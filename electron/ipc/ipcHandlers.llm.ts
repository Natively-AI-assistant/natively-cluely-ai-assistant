...
export function registerLLMHandlers(appState: AppState): void {
  ...
  safeHandle("get-intelligence-context", async () => {
    try {
      const intManager = appState.getIntelligenceManager();
      return { context: intManager.getFormattedContext(), lastAssistantMessage: intManager.getLastAssistantMessage(), activeMode: intManager.getActiveMode() };
    } catch (error: any) {
      throw error;
    }
  });
  ...
  safeHandle("generate-clarify", async () => {
    ...
    if (!result) {
      ...
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('intelligence-error', { error: 'Could not generate clarification', mode: 'clarify' });
      }
    }
