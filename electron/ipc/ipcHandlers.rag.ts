...
export function registerRAGHandlers(appState: AppState): void {
  ...
  safeHandle("rag:cancel-query", async (_, { meetingId, global }: { meetingId?: string; global?: boolean } = {}) => {
    if (meetingId) {
      activeRAGQueries.get(`meeting-${meetingId}`)?.abort();
      activeRAGQueries.delete(`meeting-${meetingId}`);
    } else if (global) {
      activeRAGQueries.forEach((_, key) => {
        if (key.startsWith('global-')) {
          activeRAGQueries.get(key)?.abort();
          activeRAGQueries.delete(key);
        }
      });
    } else {
      activeRAGQueries.forEach((controller) => controller.abort());
  ...
  safeHandle("rag:get-queue-status", async () => {
    ...
    return { pending: status.pending || 0, processing: status.processing || 0, completed: status.completed || 0, failed: status.failed || 0 };
  });
