...
export class IntelligenceEngine extends EventEmitter {
     ...
     */
    async runWhatShouldISay(question?: string, confidence: number = 0.8, imagePaths?: string[], options?: { speculative?: boolean; skipCooldown?: boolean; screenContext?: ScreenContext; promptInstruction?: string; domContext?: string }): Promise<string | null> {
        const now = Date.now();
        ...
        try {
            ...
            const stream = this.whatToAnswerLLM.generateStream(preparedTranscript, temporalContext, intentResult, imagePaths, screenContext, options?.promptInstruction, options?.domContext);
