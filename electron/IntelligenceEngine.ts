...
// Local stand-in for premium ContextAssembler.PromptAssemblyResult. The public
// tree has no premium/, so importing that module fails typecheck:electron.
// Shape is the fields this file actually reads on the grounding result.
...
function withTimeout<T, F = T>(
	...
	ms: number,
	fallback: F,
): Promise<{ value: T | F; timedOut: boolean }> {
...
function detectRefinementIntent(userText: string): {
	...
	const refinementPatterns = [
		...
		{
			pattern: /give me an example|provide an instance/i,
			intent: 'add_example',
		},
		{
			pattern: /make it more confident|be more assertive|sound stronger/i,
			intent: 'more_confident',
		},
		{
...
export class IntelligenceEngine extends EventEmitter {
	...
	private readonly whatToAnswerBackgroundCancellationTokens =
	...
	private questionLedgerShadow:
		| import('./llm/questionLedger').QuestionLedger
		| null = null;
	 ...
	 * the legacy gate.
	 */
	private static shouldJitForAnswerType(
	...
	private static isNonAnswerSentinel(answer: string): boolean {
		...
		const normalized = answer
			...
			.replace(/[.!?]+$/g, '');
		return (
			normalized === 'nothing actionable right now' ||
	...
	private static isFalseNoContentClaim(answer: string): boolean {
		...
		const sentinelBody = t
			...
			.trim();
		return (
			/^(?:hey\s+\w+,\s*)?(?:your\s+)?phone'?s?\s+interviewer\s+audio\s+is\s+coming\s+through,?\s*but\s+i\s+haven'?t\s+picked\s+up\s+(?:any|a)\s+question\s+yet\.?$/i.test(
				sentinelBody,
			) ||
			/^there'?s\s+nothing\s+captured\s+to\s+summarize\s+yet\.?$/i.test(
				sentinelBody,
			) ||
			/^i\s+don'?t\s+have\s+a\s+specific\s+question\s+or\s+topic\s+to\s+clarify\s+from\s+what'?s\s+captured\s+right\s+now\.?$/i.test(
				sentinelBody,
			) ||
			/^i\s+haven'?t\s+(?:picked\s+up|caught|heard)\s+(?:any|a)\s+question\s+yet\.?$/i.test(
				sentinelBody,
			) ||
	...
	private static sanitizeManualContextText(
		text: string,
		maxChars: number,
	): string {
	...
	private buildActionContextWithManualFallback(
		lastSeconds: number,
	): string | null {
		const transcriptContext = this.buildPreparedTranscriptContext(lastSeconds);
		if (
			transcriptContext &&
			transcriptContext.trim().length >=
				IntelligenceEngine.TRANSCRIPT_CONTEXT_SUBSTANTIAL_CHARS
		)
		...
		if (manualContext) {
			if (transcriptContext?.trim()) {
				const supplementalTranscript = IntelligenceEngine.escapeXmlText(
	 ...
	 */
	getLastTraceSnapshot(): {
		requestId: string;
		timings: Record<string, number>;
	} | null {
		if (!this.lastTrace) return null;
		return {
			requestId: this.lastTrace.requestId,
			timings: this.lastTrace.snapshot(),
		};
	}
	...
	private static hasQuestionSignal(text: string): boolean {
		...
		return /\b(what|how|why|where|when|which|who|can you|could you|tell me|explain|describe|walk me through|talk me through)\b/i.test(
			text,
		);
	}
	...
	private maybeSpeculate(segment: TranscriptSegment): void {
		...
		this.speculativeTimer = setTimeout(() => {
			...
			console.log(
				`[IntelligenceEngine] Speculative inference fired on interim`,
				{ length: text.length, confidence },
			);
			this.runWhatShouldISay(text, confidence || 0.8, undefined, {
				speculative: true,
			}).catch((err) =>
				console.error('[IntelligenceEngine] Speculative run error:', err),
			);
	...
	handleTranscript(
		...
		if (segment.final && result) {
			try {
				if (isIntelligenceFlagEnabled('questionLedgerShadow')) {
					if (!this.questionLedgerShadow) {
						const { QuestionLedger } =
							require('./llm/questionLedger') as typeof import('./llm/questionLedger');
				...
				/* shadow only — never affects the transcript path */
			}
		}
		...
		if (segment.final) {
			try {
				...
				console.warn(
					'[IntelligenceEngine] detectAndEmitDynamicActions failed',
		...
		if (
			...
			const { isRefinement, intent } = detectRefinementIntent(
				segment.text.trim(),
			);
			if (isRefinement) {
				void this.runFollowUp(intent, segment.text.trim()).catch((err) =>
					console.error('[IntelligenceEngine] Follow-up run error:', err),
				);
			}
		}
	}
	...
	async handleSuggestionTrigger(trigger: SuggestionTrigger): Promise<void> {
		...
		if (plannerDecision.kind === 'silent') {
			console.log('[IntelligenceEngine] Planner stayed silent', {
		...
		if (this.speculativeText !== null) {
			...
			if (!stale) {
				const similarity = speculativeQuestionSimilarity(
				...
				if (similarity >= this.SPECULATIVE_SIMILARITY_THRESHOLD) {
					console.log(
						`[IntelligenceEngine] Speculative stream accepted (Jaccard=${similarity.toFixed(2)}) — continuing`,
					);
					this.lastTriggerTime = Date.now();
					return;
				}
				console.log(
					`[IntelligenceEngine] Speculative stream rejected (Jaccard=${similarity.toFixed(2)}) — restarting`,
				);
			} else {
				console.log(
					`[IntelligenceEngine] Speculative result discarded (expired=${expired}, noQuestion=${!trigger.lastQuestion})`,
				);
				this.speculativeText = null;
				this.speculativeTextExpiry = Infinity;
			}
	...
	private async planSuggestionTrigger(
		...
		return planNextAssistantAction({
			...
			transcriptContext,
			intentResult,
			hasRecentAssistantResponse:
				this.session.getAssistantResponseHistory().length > 0,
	...
	private async runPlannerDecision(
	...
	async runAssistMode(): Promise<string | null> {
		...
		try {
			...
			const insight = await this.assistLLM.generate(
				context,
				controller.signal,
				assistV3 ?? undefined,
			);
	...
	async runWhatShouldISay(
		...
		const strictDocumentGroundedActive =
			(snapshotModeInfo as any)?.strictDocumentGroundedActive === true;
		...
		const docGroundedEnforcementActive =
		...
		const snapshotModesManager = (() => {
			try {
				const { ModesManager } =
					require('./services/ModesManager') as typeof import('./services/ModesManager');
		...
		const snapshotMode =
			snapshotModesManager && snapshotModeInfo?.id
				? snapshotModesManager.getModeSnapshot(snapshotModeInfo.id)
				: null;
		const snapshotReferenceFiles =
			snapshotMode && snapshotModesManager
				? Object.freeze(
						snapshotModesManager
							.getReferenceFiles(snapshotMode.id)
		...
		const snapshotProfileFacts =
			(snapshotKnowledge as any)?.activeResume?.structured_data ?? null;
		const snapshotJobDescriptionFacts =
			(snapshotKnowledge as any)?.activeJD?.structured_data ?? null;
		...
		const recordWtaCancellation = () => {
			try {
				const reason =
					whatToAnswerCancellationToken.signal.reason === 'engine_reset'
						? 'engine_reset'
						: 'superseded';
				wtaTrace
					.setCorrelation({ aborted: true, errorCategory: reason })
					.lifecycle('cancelled', { reason, finalAction: 'discard' });
				commitTrace(wtaTrace);
			} catch {
				/* trace never affects cancellation */
			}
		};
		...
		try {
			...
			if (lastInterim && lastInterim.text.trim().length > 0) {
				...
				if (!isDuplicate) {
					console.log(`[IntelligenceEngine] Injecting interim transcript`, {
						length: lastInterim.text.length,
					});
					contextItems.push({
						role: 'interviewer',
						...
						...(lastInterim.punctuationSource
							? { punctuationSource: lastInterim.punctuationSource }
							: {}),
					});
				}
			}
			...
			const transcriptTurns = contextItems.map((item) => ({
				...
				...(item.punctuationSource
					? { punctuationSource: item.punctuationSource }
					: {}),
			}));

			let preparedTranscript = prepareTranscriptForWhatToAnswer(
				transcriptTurns,
				12,
			);
			...
			if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
				try {
					const rawCharLen = transcriptTurns.reduce(
						(n, t) => n + (t.text?.length || 0),
						0,
					);
					console.log(
						'[TRACE:LONGCTX] question_extracted',
						JSON.stringify({
					...
					console.warn('[TRACE:LONGCTX] question_extracted logging failed', e);
				}
			}
			...
			const _wtaHasVisualContext =
				(imagePaths?.length ?? 0) > 0 ||
				Boolean(options?.screenContext) ||
				Boolean(options?.domContext);
			if (
				!question?.trim() &&
				!extractedQuestion.latestQuestion &&
				!lastInterviewerTurn &&
				!preparedTranscript.trim() &&
				!_wtaHasVisualContext
			) {
				if (isSpeculative) return null;
				this.setMode('idle');
				const noContextMsg =
					"I don't have anything to answer yet — no conversation, screen capture, or question has come in. Ask something or start the meeting audio, then press again.";
			...
			try {
				if (isIntelligenceFlagEnabled('liveTranscriptBrain')) {
					const brain = new LiveTranscriptBrain(
						this.session as any,
						extractLatestQuestion as any,
					);
					const brainQ = brain.getCurrentQuestion(180);
					wtaTrace.noteContext({
						source: 'live_transcript_brain',
						trustLevel: 'low',
						requested: true,
						retrieved: Boolean(brainQ),
						included: false,
						reason:
							brainQ &&
							extractedQuestion.latestQuestion &&
							brainQ !== extractedQuestion.latestQuestion
								? 'brain_question_divergence'
								: 'brain_parity',
					});
				}
			} catch {
				/* shadow brain is observe-only; never affects the answer */
			}
			...
			if (!question && extractedQuestion.latestQuestion) {
				try {
					...
					const priorInterviewer = [...transcriptTurns]
						.reverse()
						.find(
							(t) =>
								t.role === 'interviewer' &&
								t.text.trim().toLowerCase() !== latestQ,
						);
					...
					let fr: ReturnType<typeof resolveFollowUpOrClarify> & {
					...
					const lsmConfig = resolveLiveSessionMemoryConfig(
						this.currentSessionId ?? undefined,
					);
					piTelemetry.emit('wta_live_session_memory_enabled', {
						enabled: lsmConfig.enabled,
						reason: lsmConfig.reason,
						rolloutPercent: lsmConfig.rolloutPercent,
						bucket: lsmConfig.bucket,
					...
					if (lsmConfig.enabled) {
						...
						const memWindowSource = this.session.getDurableContext(
							this.LIVE_MEMORY_WINDOW_SECONDS,
						);
						const memWindowTurns = memWindowSource.map((item) => ({
							role: item.role,
							text: item.text,
							t: Math.floor(item.timestamp / 1000),
						}));
						const latestTurnSec = Math.floor(
							(transcriptTurns[transcriptTurns.length - 1]?.timestamp ??
								Date.now()) / 1000,
						);
						...
						fr = resolveFollowUpOrClarify({
							...
							surface: 'what_to_answer',
							hasPriorContext:
								Boolean(priorInterviewer?.text) ||
								Boolean(extractedQuestion.followUpTarget),
						});
					}
					...
					if (fr.isClarification && fr.clarificationText && !isSpeculative) {
						piTelemetry.emit('wta_context_free_clarification', {
							surface: 'what_to_answer',
							via: (fr as any).resolvedVia ?? 'clarification',
						});
						this.session.addAssistantMessage(
							fr.clarificationText,
							undefined,
							'what_to_answer',
						);
						this.emit(
							'suggested_answer',
							fr.clarificationText,
							extractedQuestion.latestQuestion || 'inferred',
							0.9,
							generationId,
						);
						this.setMode('idle');
					...
					if (
						...
						if (fr.resolvedEntity)
							extractedQuestion.followUpTarget = fr.resolvedEntity;
						trace.mark('repair_used', {
							reason:
								via === 'session_memory'
									? 'session_memory_followup'
									: 'followup_resolved',
							resolved: fr.reason,
						});
						...
						piTelemetry.emit('wta_live_followup_resolved', {
							via: via ?? 'prior_turn',
							answerType: fr.resolvedAnswerType,
					...
					const entityRecallSucceeded = Boolean(
						fr &&
						!fr.isClarification &&
						fr.confidence >= 0.7 &&
						(fr as any).recalledEntity,
					);
					if (
						!question &&
						extractedQuestion.isFollowUp &&
						!entityRecallSucceeded
					) {
						try {
							const { recallLongRangeContext } =
								require('./llm/longRangeTranscriptRecall') as typeof import('./llm/longRangeTranscriptRecall');
							const durableWindow = this.session.getDurableContext(
								this.LIVE_MEMORY_WINDOW_SECONDS,
							);
							const recentWindowCutoffMs =
								transcriptTurns.length > 0
									? transcriptTurns[0].timestamp
									: Date.now();
							...
							try {
								...
								/* default: not negotiation → comp stays gated */
							}
							...
							if (recall.block) {
								...
								trace.mark('repair_used', {
									reason: 'long_range_lexical_recall',
								...
								if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
									console.log(
										'[TRACE:LONGCTX] long_range_recall_fired',
										JSON.stringify({
											question: extractedQuestion.latestQuestion,
											matchCount: recall.matchCount,
							...
							/* fallback is best-effort; never blocks the answer */
						}
					}
				} catch {
					/* keep extractor result */
				}
			}
			...
			try {
				if (
					this.questionLedgerShadow &&
					isIntelligenceFlagEnabled('questionLedgerShadow')
				) {
					const rankedAsks = this.questionLedgerShadow.rankActiveAsks(
						Date.now(),
					);
					...
					const norm = (t: string) =>
						t
							...
							.trim();
					wtaTrace.noteContext({
						source: 'question_ledger_shadow',
						trustLevel: 'low',
						requested: true,
						retrieved: Boolean(topAsk),
						included: false,
						reason:
							topAsk && extractedQuestion.latestQuestion
								? norm(topAsk.standaloneText) ===
									norm(extractedQuestion.latestQuestion)
									? 'ledger_parity'
				...
				/* shadow only — never affects the answer */
			}
			...
			const wtaResolvedQuestionForKicks =
			...
			const intentPromise = classifyIntent(
				...
				(): { intent: 'general'; confidence: number; answerShape: string } => ({
					intent: 'general',
					confidence: 0.4,
					answerShape: 'Concise, direct answer to the question.',
				}),
			);
			...
			const modeContextPromise: Promise<string> =
				options?.activeSkill ||
				docGroundedEnforcementActive ||
				!wtaPrefetchDecision.allowed
					...
					: (async () => {
							try {
								...
								if (
									typeof mm.buildRetrievedActiveModeContextBlockHybrid ===
									'function'
								) {
			...
			try {
				...
				const _wtaSourceContract =
					(snapshotModeInfo as any)?.sourceContract ?? null;
				if (_wtaSourceContract) {
					const {
						resolveExplicitSourceRequests: _r,
						resolveTurnSourceDecision: _d,
					} =
					...
					_wtaTurnSourceDecision = _tsd.resolveTurnSourceDecision({
						...
						availability: {
							hasReferenceFiles: Boolean(
								(snapshotModeInfo as any)?.hasReferenceFiles,
							),
							hasProfileFacts: Boolean(
								(_wtaOrchAvail as any)?.activeResume?.structured_data,
							),
							hasJobDescription: Boolean(
								(_wtaOrchAvail as any)?.activeJD?.structured_data,
							),
							hasLiveTranscript: true,
							hasMeetingRag: false,
						},
			...
			if (_wtaTurnSourceDecision) {
				...
				if (_wtaTurnSourceDecision.allowedEvidenceKinds.length > 0) {
					...
					wtaDecisionAllowsCandidateProfile =
						wtaDecisionAllowsCandidateProfile &&
						(_wtaTurnSourceDecision.allowedEvidenceKinds.includes(
							'profile_resume',
						) ||
							_wtaTurnSourceDecision.allowedEvidenceKinds.includes(
								'projects',
			...
			try {
				...
				if (
					...
					const isNegotiationButActuallyJdFact =
						extracted.questionType === 'negotiation' &&
						isJdFactualLookupNotNegotiationAdvice(
							extracted.latestQuestion || '',
						);
					const groundable =
						extracted.detectedSpeaker === 'interviewer' &&
					...
					const questionIsTranscriptQuestion =
						Boolean(question) &&
					...
					if (groundable && (!question || questionIsTranscriptQuestion)) {
						...
						const { value: knowledge, timedOut: groundingTimedOut } =
							await withTimeout(
								orchestrator.processQuestion(lookupQ),
								GROUNDING_BUDGET_MS,
								null,
							);
						if (groundingTimedOut) {
							trace.mark('degraded_context', {
								...
								budgetMs: GROUNDING_BUDGET_MS,
							});
							console.warn(
								`[IntelligenceEngine] Profile grounding exceeded ${GROUNDING_BUDGET_MS}ms — proceeding without it`,
							);
						} else {
						...
						if (
							knowledge &&
							knowledge.factualRecall === true &&
							!knowledge.liveNegotiationResponse
						) {
							...
							if (candidateProfile) {
								console.log(
									'[IntelligenceEngine] Grounded what-to-answer in candidate profile',
									{
				...
				console.warn(
					'[IntelligenceEngine] Profile grounding skipped:',
					groundErr?.message,
				);
			}
			...
			try {
				...
				const { buildTurnContractIfEnabled, allowsEvidence: coAllowsEvidence } =
					contextOsStatic;
				// MUST match wtaTurnQuestion / canonicalTurn's expression below
				...
				var _wtaHasProfile: boolean | undefined = Boolean(
					(_wtaOrchForAvail as any)?.activeResume?.structured_data,
				);
				var _wtaHasJd: boolean | undefined = Boolean(
					(_wtaOrchForAvail as any)?.activeJD?.structured_data,
				);
				...
				const {
					resolveExplicitSourceRequest: _wtaResolveSwitch,
					resolveExplicitSourceRequests: _wtaResolveSwitches,
					toLegacyUserExplicitSource: _wtaToLegacySwitch,
				...
				const _wtaUserExplicitSource =
					_wtaExplicitSwitch === 'job_description'
						? 'profile'
						: _wtaExplicitSwitch;
				const _wtaSourceContract =
					(snapshotModeInfo as any)?.sourceContract ?? null;
				...
				const _wtaContract = buildCustomModeExecutionContract({
					...
					persistedSourceAuthority:
						(snapshotModeInfo as any)?.sourceContract?.sourceAuthority ?? null,
				...
				wtaProfileAllowed =
					getSourceOwnerEnforcementStage() === 'off'
				...
				if (_wtaEarlyContract) {
					...
					const contractAllowsCandidateProfileEarly =
						coAllowsEvidence(_wtaEarlyContract, 'profile_resume') ||
						coAllowsEvidence(_wtaEarlyContract, 'profile_project');
					wtaProfileAllowed =
						wtaProfileAllowed && contractAllowsCandidateProfileEarly;
				}
			} catch {
				/* keep legacy doc-grounded guard */
			}
			...
			const _jitAnswerType = (() => {
				try {
					return _wtaPlan?.answerType ?? null;
				} catch {
					return null;
				}
			})();
			const _jdShapeAllowed =
				_jitAnswerType !== null &&
				IntelligenceEngine.shouldJitForAnswerType(_jitAnswerType) &&
			...
			if (
				...
				try {
					...
					const _c3HasJd = (() => {
						try {
							return _wtaHasJd ?? false;
						} catch {
							return false;
						}
					})();
					const _c3HasRefFiles = (() => {
						try {
							return Boolean((snapshotModeInfo as any)?.hasReferenceFiles);
						} catch {
							return false;
						}
					})();
					...
					try {
						...
						_c3TurnPlan = planTurn({
							question:
								extractedQuestion.latestQuestion || lastInterviewerTurn || '',
					...
					const identityQ =
						extractedQuestion.detectedSpeaker === 'interviewer' &&
					...
					const seedCandidateBackground =
						_c3TurnPlan.answerDirectives.seedCandidateBackground;
					if (
						(resume || jd) &&
						(identityQ ||
							IntelligenceEngine.shouldJitForAnswerType(jitAnswerType))
					) {
						const { selectManualProfileEvidence } =
							await import('./llm/manualProfileIntelligence');
						...
						if (evidence) {
							...
							trace.mark('repair_used', {
								reason: 'identity_jit_evidence_grounding',
								evidenceItems: evidence.items.length,
								promptChars: jit.promptChars,
							});
						}
					}
				} catch (fbErr: any) {
					console.warn(
						'[IntelligenceEngine] identity fast-path grounding skipped:',
						fbErr?.message,
					);
				}
			}
			...
			const canonicalTurn = resolveCanonicalTurn({
				...
				explicitRequests: (() => {
					try {
						...
						return resolveExplicitSourceRequests(
							question ||
								extractedQuestion.latestQuestion ||
								lastInterviewerTurn ||
								'',
			...
			wtaTrace.noteContext({
				...
				included: false,
				reason:
					canonicalTurn.turnSourceDecision?.reasonCode ??
					'legacy_source_contract_absent',
			});
			...
			try {
				...
				recordLegacyTurn({
					requestId: String(
						(canonicalTurn as any).turnId ?? `wta-${Date.now()}`,
					),
					surface: 'what-to-answer',
					scope: {
						userId: 'local',
						meetingId: _s?.getMeetingMetadata?.()?.id ?? undefined,
					},
					originalQuestion: _q,
					resolvedQuestion: _q,
					modeId: (this.getActiveModeInfo() as any)?.templateType ?? undefined,
					groundingPolicy: (canonicalTurn.sourceAuthority ??
						undefined) as never,
			...
			try {
				const {
					buildCustomModeExecutionContract: _bldC,
				} = require('./llm/customModeExecutionContract');
				const { buildTurnContractIfEnabled } = contextOsStatic;
				const _wtaQ2 = wtaTurnQuestion;
				const _hasProfile2 = Boolean(
					(this.llmHelper.getKnowledgeOrchestrator?.() as any)?.activeResume
						?.structured_data,
				);
				const {
					resolveExplicitSourceRequest: _wtaResolveSwitch2,
					toLegacyUserExplicitSource: _wtaToLegacySwitch2,
				} = require('./intelligence/context-os/explicitSourceSwitch');
				const _wtaUserExplicitSource2 = _wtaToLegacySwitch2(
					_wtaResolveSwitch2(String(_wtaQ2)),
				);
				...
				wtaTurnContract = buildTurnContractIfEnabled({
					...
					activeModeName: snapshotModeInfo?.name ?? null,
					sourceAuthority:
						canonicalTurn.sourceAuthority ?? _legacyContract2.sourceAuthority,
					answerType: answerPlan.answerType,
					...
					allowedExplicitSwitches:
						snapshotSourceContract?.allowedExplicitSwitches ?? null,
					turnId: _wtaTurnId,
				});
				if (wtaTurnContract) {
					...
					const contractAllowsProfileWta =
						allowsEvidence(wtaTurnContract, 'profile_resume') ||
					...
					if (!contractAllowsProfileWta && candidateProfile) {
						...
						candidateProfile = '';
						trace.mark('context_selected', {
							via: 'context_os_profile_suppressed',
				...
				if (isIntelligenceFlagEnabled('trace')) {
					console.warn(
						'[CONTEXT-OS] WTA contract build skipped (non-fatal):',
						contextOsWtaErr?.message,
					);
				}
			}
			...
			const WTA_COORDINATOR_KINDS = new Set([
				'reference_files',
				'profile_resume',
				'projects',
				'profile_jd',
			]);
			const wtaCoordinatorInScope =
				Boolean(canonicalTurn.turnSourceDecision) &&
				canonicalTurn.requiredEvidenceKinds.length > 0 &&
				canonicalTurn.requiredEvidenceKinds.every((kind) =>
					WTA_COORDINATOR_KINDS.has(kind),
				) &&
				canonicalTurn.requiredEvidenceKinds.some(
					(kind) =>
						kind === 'profile_resume' ||
						kind === 'projects' ||
						kind === 'profile_jd',
			...
			if (
				...
				try {
					const { TurnEvidenceCoordinator, ProfileEvidenceService } =
						contextOsStatic;
					...
					const { classifyQuestion } =
						require('./services/knowledge/QuestionClassifier') as typeof import('./services/knowledge/QuestionClassifier');
					const { queryOkfCards } =
						require('./services/knowledge/OkfRetriever') as typeof import('./services/knowledge/OkfRetriever');
					const { KnowledgeManager } =
						require('./services/knowledge/KnowledgeManager') as typeof import('./services/knowledge/KnowledgeManager');
					...
					const retrieveReferenceEvidence =
						canonicalTurn.requiredEvidenceKinds.includes('reference_files')
							? async () => {
									...
									const resolver = new EvidenceResolver({
										...
										getReferenceFiles: (modeId: string) =>
											(modeId === snapshotMode.id
												? snapshotReferenceFiles.map((file) => ({ ...file }))
												: []) as any,
										hybridRetriever: {
					...
					const WTA_COORDINATOR_BUDGET_MS = documentGroundedCustomModeActive
						? 2000
						: 1000;
					const coordinatorResult = await Promise.race([
						coordinator.resolve({
							...
							retrieveProfileEvidence: () =>
								profileService.retrieveEvidence({
									question: wtaTurnQuestion,
									contract: wtaTurnContract!,
						...
						new Promise<null>((resolve) =>
							setTimeout(() => resolve(null), WTA_COORDINATOR_BUDGET_MS),
						),
					]);
					if (!coordinatorResult)
						throw new Error(
							`TurnEvidenceCoordinator exceeded ${WTA_COORDINATOR_BUDGET_MS}ms budget`,
						);
					if (isWtaSuperseded()) {
					...
					wtaContextOsGeneration = {
						...
						modeSnapshot: {
							...
							sourceAuthority:
								canonicalTurn.sourceAuthority ?? 'ask_if_ambiguous',
						},
					...
					wtaContextOsGeneration = undefined;
					console.warn(
						'[CONTEXT-OS] WTA TurnEvidenceCoordinator skipped (non-fatal):',
				...
				try {
					...
					const clarify = wtaOwnershipDecision?.shouldClarifyInsteadOfProfile
						...
						: buildSourceClarification({
								...
								hasProfileFacts: Boolean(
									(this.llmHelper.getKnowledgeOrchestrator?.() as any)
										?.activeResume?.structured_data,
								),
								hasLiveTranscript: true, // WTA is always transcript-driven
							});
					this.session.addAssistantMessage(
					...
					this.emit(
						'suggested_answer',
						clarify,
						extractedQuestion.latestQuestion || question || 'inferred',
						0.9,
						generationId,
					);
					...
					if (isIntelligenceFlagEnabled('trace')) {
						logContextOsTrace(
							buildContextOsTrace({
								...
								sourceAuthority: canonicalTurn.sourceAuthority ?? 'legacy',
								question: String(
									extractedQuestion.latestQuestion || question || '',
								),
								usedSources: [],
								finalAction: 'clarify',
							}),
					...
					if (isIntelligenceFlagEnabled('trace')) {
						console.warn(
							'[CONTEXT-OS] WTA clarification short-circuit skipped (non-fatal):',
							clarErr?.message,
						);
					}
				}
			...
			const isCoding =
				!isSpeculative && isCodingAnswerType(answerPlan.answerType);
			...
			const { StreamingSpecStripper } = isCoding
				? (require('./llm/codingContract') as typeof import('./llm/codingContract'))
				: { StreamingSpecStripper: null as any };
			const specStripper:
				| import('./llm/codingContract').StreamingSpecStripper
				| null = isCoding ? new StreamingSpecStripper() : null;
			...
			const wtaV3Prompt = await (async () => {
				try {
					...
					const _v3 = await buildV3Prompt({
						...
						hasScreenContext:
							Boolean(options?.screenContext) || (imagePaths?.length ?? 0) > 0,
						...
						personaBase: ({ codingTask }: { codingTask: boolean }) => {
							try {
								const { resolveV2SystemPrompt, v2TierForPromptTier } =
									require('./llm/promptSystemV2') as typeof import('./llm/promptSystemV2');
								const { resolveCodingPromptSignals } =
									require('./llm/codingPromptSignals') as typeof import('./llm/codingPromptSignals');
								...
								const _screenIsSubject =
									((imagePaths?.length ?? 0) > 0 || _screenText.length > 0) &&
									(!answerPlan.question?.trim() ||
					...
					if (_v3) {
						...
						console.log(
							`[IntelligenceEngine] WTA V3 prompt in effect: answerability=${_v3.answerability} evidence=${_v3.evidenceCount} fallback=${_v3.fallbackUsed}`,
						);
					}
					return _v3
						? {
								...
								evidenceCount: _v3.evidenceCount,
								answerability: _v3.answerability,
							}
						: undefined;
				} catch {
					return undefined;
			...
			const requestSnapshot: WhatToAnswerRequestSnapshot = Object.freeze({
				...
				generationId,
				...(wtaContextOsGeneration
					? { contextOsGeneration: wtaContextOsGeneration }
					: {}),
			...
			const usingLocalLlm =
				typeof (this.llmHelper as any).isUsingOllama === 'function'
			...
			const raceOutcome = await raceStreamWithDeadline({
				...
				onFirstUsefulTimeout: () => {
					liveDeadlineFired = true;
					trace.mark('provider_timeout', {
						budgetMs: firstUsefulDeadline,
						answerType: answerPlan.answerType,
					});
				},
				onStallTimeout: () => {
				...
				onToken: (token: string) => {
					...
					if (codingGate) {
						...
						if (
							streamingTokenBuffer.length >= STREAMING_SAFE_PREFIX_CHARS &&
							...
							try {
								const { stripLeadingNoActionSentinel } =
									require('./llm/promptSystemV2') as typeof import('./llm/promptSystemV2');
								visiblePrefix =
									stripLeadingNoActionSentinel(visiblePrefix) || visiblePrefix;
							} catch {
								/* emit unmodified */
							}
			...
			if (streamAborted) {
				console.log(
					'[IntelligenceEngine] _what_to_say stream aborted by new generation',
				);
			}
			...
			let wtaWriteDecision = decideSessionWritePolicy({
			...
			if (
				...
				if (fullAnswer.trim().length < STREAMING_SAFE_PREFIX_CHARS) {
					...
					trace.mark('fallback_answer_used', {
						answerType: answerPlan.answerType,
						finalGenerationMode: 'provider_error_no_answer',
					});
				}
			...
			if (streamAborted) {
				...
				if (openedStreamRow)
					this.emit('suggested_answer_discard', 'superseded');
				if (isSpeculative) {
					this.speculativeText = null;
					this.speculativeTextExpiry = Infinity;
			...
			if (fullAnswer) {
				...
				if (recoveredAnswer) {
					trace.mark('repair_used', {
						reason: 'json_envelope_answer_recovered',
						answerType: answerPlan.answerType,
					});
			...
			const jsonAnswerLikelyAnswerTypes =
				isCodingAnswerType(answerPlan.answerType) ||
			...
			if (
				fullAnswer &&
				(isLeakedSchemaStub(fullAnswer) ||
					(!jsonAnswerLikelyAnswerTypes && isLeakedJsonEnvelope(fullAnswer)))
			) {
				const stubFallback =
					answerPlan.answerType === 'general_meeting_answer' ||
					answerPlan.answerType === 'lecture_answer'
				...
				this.emit(
					'suggested_answer',
					stubFallback,
					question || extractedQuestion.latestQuestion || 'inferred',
					confidence,
					generationId,
				);
				this.setMode('idle');
			...
			if (fullAnswer && isLeakedInternalTagBlock(fullAnswer)) {
				...
				trace.mark('fallback_answer_used', {
					answerType: answerPlan.answerType,
					reason: 'leaked_internal_tag_block',
					finalGenerationMode: 'provider_error_no_answer',
				});
			...
			const liveExplicitCodingContract =
			...
			const structureRepairSuppressedByDeadline =
				liveDeadlineFired && emittedStreamingToken;
			if (structureRepairSuppressedByDeadline && !structureValidation.ok) {
				trace.mark('validation_completed', {
					reason: 'structure_repair_skipped_deadline_truncated',
				});
			}
			if (
				!structureRepairSuppressedByDeadline &&
				!structureValidation.ok &&
				structureValidation.repaired
			) {
			...
			if (
				!isCodingAnswerType(answerPlan.answerType) &&
				!TECHNICAL_ANSWER_TYPES_EXCLUDED_FROM_SCAFFOLD_EXTRACTION.has(
					answerPlan.answerType,
				)
			) {
				const extracted = detectAndExtractScaffoldMisfire(
					answerPlan.answerType,
					fullAnswer,
				);
				if (extracted) {
					trace.mark('repair_used', {
						reason: 'scaffold_misfire_extracted',
						answerType: answerPlan.answerType,
					});
			...
			if (
				...
				try {
					if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
						try {
							console.log(
								'[TRACE:LONGCTX] scaffold_contamination_discard',
								JSON.stringify({
							...
							console.warn(
								'[TRACE:LONGCTX] scaffold_contamination_discard logging failed',
								e,
							);
						}
					}
					trace.mark('repair_used', {
						reason: 'scaffold_contamination_detected',
						answerType: answerPlan.answerType,
					});
					wtaTrace.lifecycle('repairing', {
						reason: 'scaffold_contamination',
						repairCount: 1,
					});
					const safeScaffoldQuestion =
						IntelligenceEngine.sanitizeManualContextText(
							scaffoldQuestion,
							1000,
						);
					const hasCandidateProfileForScaffold = Boolean(
						candidateProfile && candidateProfile.trim().length > 0,
					);
					...
					const scaffoldRepairPrompt = [
						...
						IntelligenceEngine.escapeXmlText(
							'Your previous response leaked internal planning notes and template headings (e.g. "## Approach") instead of a clean spoken answer. Rewrite it as a direct, natural first-person answer to the question below, with no headings, no meta-commentary about how you are structuring the answer, and no notes to yourself. Ground every claim in candidate_facts if provided.',
						),
					...
					try {
						await raceStreamWithDeadline({
							...
							firstUsefulDeadlineMs: this.llmHelper.isUsingOllama()
								? LIVE_LOCAL_FIRST_USEFUL_TIMEOUT_MS
								: 7000,
							interTokenStallMs: LIVE_INTER_TOKEN_STALL_MS,
							isUsefulYet: () => scaffoldRepaired.length >= 5,
							shouldAbort: () =>
								scaffoldRepaired.length > 1800 ||
								whatToAnswerCancellationToken.signal.aborted ||
								isWtaSuperseded(),
							onToken: (tok: string) => {
								scaffoldRepaired += tok;
							},
						});
					} catch {
						/* keep original fullAnswer on repair failure */
					}
					const scaffoldRepairedTrim = scaffoldRepaired.trim();
					if (
						scaffoldRepairedTrim.length >= 5 &&
						this.currentGenerationId === generationId
					) {
						...
						const stillContaminated = hasUnrecoveredScaffoldContamination(
							answerPlan.answerType,
							scaffoldRepairedTrim,
						);
						if (
							!stillContaminated &&
							...
							trace.mark('repair_used', {
								reason: 'scaffold_contamination_regenerated',
							});
						} else {
							trace.mark('validation_completed', {
								reason: 'scaffold_contamination_repair_rejected',
							});
						}
					} else {
						trace.mark('validation_completed', {
							reason: 'scaffold_contamination_repair_empty',
						});
					}
				} catch (scaffoldErr: any) {
					console.warn(
						'[IntelligenceEngine] scaffold contamination guard skipped:',
						scaffoldErr?.message || scaffoldErr,
					);
				}
			}
			...
			try {
				...
				if (
					...
					if (docQuestion) {
						...
						const hasOkfEvidence =
							/STRUCTURED KNOWLEDGE CARDS|Direct quote|knowledge_card/i.test(
								docContextBlock,
							);
							...
							if (firstCheck.action === 'refuse') {
								...
								try {
									...
									wtaRepairSystemPrompt = appendCustomModeSystemPromptLayer({
										...
										modePromptSuffix: mm.getActiveModeSystemPromptSuffix?.(
											_activeModeRow?.id,
										),
										pinnedInstructions: mm.getActiveModePinnedInstructions?.(
											answerPlan.answerType,
											_activeModeRow?.id,
										),
									...
									wtaRepairSystemPrompt = undefined;
								}
								let repaired = '';
								try {
									await raceStreamWithDeadline({
								...
								const wtaRepairIsLengthDowngrade =
									wtaOriginalForCompare.length >= 150 &&
									repairedTrim.length < wtaOriginalForCompare.length * 0.6;
								if (
									repairedTrim.length >= 5 &&
									...
									trace.mark('repair_used', {
										reason: 'doc_grounded_repair_applied',
										originalReason: firstCheck.reason,
									});
								} else if (
									firstCheck.reason === 'empty_or_greeting' ||
									firstCheck.reason === 'false_refusal_evidence_exists' ||
									wtaRepairIsLengthDowngrade
								) {
									...
									trace.mark('validation_completed', {
										reason: 'doc_grounded_repair_rejected_keep_original',
										originalReason: firstCheck.reason,
									});
								} else {
									fullAnswer =
										'I could not find that in the retrieved sections of the document.';
									trace.mark('repair_used', {
										reason: 'doc_grounded_safe_refusal_after_repair_reject',
										originalReason: firstCheck.reason,
									});
								}
							}
				...
				console.warn(
					'[IntelligenceEngine] document-grounded WTA validation skipped:',
					docGroundedValidationErr?.message || docGroundedValidationErr,
				);
			}
			...
			try {
				...
				if (
					profileLoaded &&
					contractPermitsProfileRepair &&
					answerPlan.voicePerspective === 'first_person_candidate'
				) {
					...
					const evidenceIsSubstantial =
						candidateProfile.length >= 600 &&
						!candidateProfile.trim().startsWith('<candidate_identity_fact>');
					const criticalViolation = pv.violations.find(
						(v) =>
							v.severity === 'error' &&
					...
					if (criticalViolation && this.currentGenerationId === generationId) {
						...
						const repairInstruction =
							pv.repairInstruction || buildProfileRepairInstruction(pv as any);
						...
						if (repairedTrim.length >= 5) {
							...
							if (profileVerdict.accepted) {
								...
								trace.mark('validation_completed', {
									reason: 'profile_repair_rejected',
									code: criticalViolation.code,
									rejection: profileVerdict.reason,
								});
							}
						}
					}
				...
				console.warn(
					'[IntelligenceEngine] profile repair failed (non-fatal):',
					profileRepairErr?.message || profileRepairErr,
				);
			}
			...
			if (CANDIDATE_VOICE_ANSWER_TYPES.has(answerPlan.answerType)) {
				try {
					...
					if (sani.repaired && !sani.needsFallback) {
						...
						trace.mark('repair_used', {
							reason: 'candidate_sanitizer',
							markers: sani.removedMarkers.length,
						});
						...
						trace.mark('repair_used', {
							reason: 'candidate_sanitizer_needs_fallback',
							markers: sani.removedMarkers.length,
						});
					...
					console.warn(
						'[IntelligenceEngine] candidate sanitizer skipped:',
						saniErr?.message,
					);
				}
			}
			...
			if (
				...
				if (
					...
					trace.mark('repair_used', {
						reason: 'product_about_refusal_repaired',
					});
				}
			}
				...
				try {
					...
					if (mis.isMisfire) {
						...
						trace.mark('repair_used', {
							...
							misfireReason: mis.reason,
						});
					}
				} catch (avErr: any) {
					console.warn(
						'[IntelligenceEngine] assistant-voice guard skipped:',
						avErr?.message,
					);
				}
			}
			...
			if (
				...
				if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
					try {
						console.log(
							'[TRACE:LONGCTX] false_no_content_claim_discard',
							JSON.stringify({
								question:
									question ||
									extractedQuestion.latestQuestion ||
						...
						console.warn(
							'[TRACE:LONGCTX] false_no_content_claim_discard logging failed',
							e,
						);
					}
				}
			...
			if (IntelligenceEngine.isNonAnswerSentinel(fullAnswer)) {
				...
				if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
					try {
						console.log(
							'[TRACE:LONGCTX] nonanswer_sentinel_discard',
							JSON.stringify({
								question:
									question ||
									extractedQuestion.latestQuestion ||
						...
						console.warn(
							'[TRACE:LONGCTX] nonanswer_sentinel_discard logging failed',
							e,
						);
					}
				}
				if (!isSpeculative) {
					const honestFallback =
						"I don't have enough from the conversation to answer that specific point yet.";
					fullAnswer = honestFallback;
					console.log(
						'[FIX:longsession-nonanswer-fallback]',
						JSON.stringify({
							answerType: answerPlan?.answerType,
					...
					trace.mark('fallback_answer_used', {
						answerType: answerPlan?.answerType,
						finalGenerationMode: 'nonanswer_sentinel_fallback',
					});
					if (openedStreamRow) {
			...
			try {
				if (
					...
					const {
						...
						shouldAttemptClauseRepair,
						buildClauseRepairInstruction,
					} =
					...
					if (
						...
						const clauseRepairInstruction = buildClauseRepairInstruction(
							coverage.missing,
						);
						const safeCoverageQuestion =
							IntelligenceEngine.sanitizeManualContextText(
								coverageQuestion,
								1000,
							);
						...
						if (additionTrim.length >= 5) {
							...
							const reAssessed = assessAnswerCoverage(
								coverageQuestion,
								appended,
							);
							const coverageVerdict = acceptRepairedAnswer({
								...
								repaired: appended,
								stillInvalid:
									reAssessed.missing.length >= coverage.missing.length,
							});
							if (coverageVerdict.accepted) {
								fullAnswer = coverageVerdict.text;
								trace.mark('repair_used', {
									reason: 'clause_coverage_appended',
									missing: coverage.missing.length,
								});
								piTelemetry.emit('wta_clause_coverage', {
									incomplete: true,
								...
								trace.mark('validation_completed', {
									reason: 'clause_coverage_repair_rejected',
									rejection: coverageVerdict.reason,
								});
							}
						}
					}
				...
				console.warn(
					'[IntelligenceEngine] clause-coverage check failed (non-fatal):',
			...
			if (
				...
				try {
					...
					if (!isIntelligenceFlagEnabled('answerRelevanceGuardLive')) {
						this.pendingObserveOnlyRelevanceCheck = checkAnswerRelevance(
							relevanceQuestion,
							fullAnswer,
						)
							.then((relevance) => {
								if (
									...
									if (process.env.NATIVELY_TRACE_LONGCTX === '1') {
										try {
											console.log(
												'[TRACE:LONGCTX] answer_relevance_discard',
												JSON.stringify({
											...
											console.warn(
												'[TRACE:LONGCTX] answer_relevance_discard logging failed',
												e,
											);
										}
									}
									trace.mark('validation_completed', {
										reason: 'answer_relevance_observe_only',
										confidence: relevance.confidence,
									});
								}
							})
							.catch((relevanceErr: any) => {
								console.warn(
									'[IntelligenceEngine] answer relevance guard skipped:',
									relevanceErr?.message || relevanceErr,
								);
							});
					} else {
						...
						if (
							...
							});
							{
								...
								const repairPrompt = [
									'<rewrite_instructions note="follow these; never repeat or quote them in your output">',
									IntelligenceEngine.escapeXmlText(
										'Your previous response did not address the question below at all. Answer it directly and specifically, grounding every claim in candidate_facts if provided. Speak as if answering aloud in conversation — short clauses, no heavy markdown formatting, no LaTeX notation, no headings — natural first-person spoken delivery, the way a thoughtful candidate would in a real interview.',
									),
									'</rewrite_instructions>',
									...(hasCandidateProfile
										? [
								...
								if (
									...
									const reCheck = await checkAnswerRelevance(
										relevanceQuestion,
										repairedTrim,
									);
									// Whole-answer artifact re-check (found 2026-07-19, see
									...
									if (relevanceVerdict.accepted) {
										...
										trace.mark('repair_used', {
											reason: 'answer_relevance_regenerated',
										});
									} else {
										trace.mark('validation_completed', {
											reason: 'answer_relevance_repair_rejected',
											rejection: relevanceVerdict.reason,
										});
									}
								} else {
									trace.mark('validation_completed', {
										reason: 'answer_relevance_repair_empty',
									});
								}
							} // end repair body
						}
			...
			if (isSpeculative) {
				this.lastTriggerTime = Date.now();
				this.speculativeTextExpiry =
					this.lastTriggerTime + this.triggerCooldown + 500;
				this.setMode('idle');
				return fullAnswer;
			}
			...
			if (codingGate) {
				...
				const tail = specStripper
					? specStripper.push(gatedTail) + specStripper.finish()
					: gatedTail;
				if (tail)
					this.emit(
						'suggested_answer_token',
			...
			if (!isCoding && finalWtaAnswer) {
				try {
					...
					if (
						...
						console.log(
							'[TRACE:LONGCTX] fabricated_transcript_preamble_stripped',
							JSON.stringify({
								rawChars: finalWtaAnswer.length,
					...
					if (
						...
						BOLD_PSEUDO_HEADER_RE.test(cleaned)
					) {
						const speakable = compressToSpeakable(cleaned);
						if (speakable.trim().length >= 40) cleaned = speakable;
					}
					if (cleaned.trim().length >= 10 && cleaned !== finalWtaAnswer)
						finalWtaAnswer = cleaned;
				} catch {
					/* cleanup never blocks the answer */
				}
			}
			try {
				...
				/* normalizer never blocks the answer */
			}
			...
			try {
				...
				if (finalWtaAnswer !== fullAnswer)
					wtaTrace.noteFallback('output_shape_normalized');
				wtaTrace.lifecycle('completed', {
					answerType: answerPlan.answerType,
					finalAction: 'answer',
			...
			try {
				recordAttribution({
					...
					surface: 'what_to_answer',
					live_transcript_brain_used: isIntelligenceFlagEnabled(
						'liveTranscriptBrain',
					),
					live_transcript_brain_mode: isIntelligenceFlagEnabled(
						'liveTranscriptBrain',
					)
						? 'shadow'
						: 'off',
					...
					prompt_assembler_v2_mode: isIntelligenceFlagEnabled(
						'promptAssemblerV2',
					)
						? 'shadow'
						: 'off',
				...
				/* attribution never affects the answer */
			}
			...
			if (isCoding && isCodeVerificationEnabled()) {
				...
				this.whatToAnswerBackgroundCancellationTokens.add(
				...
				void this.maybeVerifyCoding(
					...
					this.whatToAnswerBackgroundCancellationTokens.delete(
						verificationCancellationToken,
					);
				});
			}
			...
			try {
				wtaTrace
					.setCorrelation({
						errorCategory: (error as Error)?.name || 'handler_error',
					})
				...
				/* trace never affects the fallback */
			}
			if (isSpeculative) {
				this.speculativeText = null;
				this.speculativeTextExpiry = Infinity;
	...
	private async maybeVerifyCoding(
		...
		try {
			...
			const outcome = await verifyCodingAnswer({
				...
				onEvent: (name, props) => {
					try {
						trace.mark(name as any, props);
					} catch {
						/* telemetry never breaks verify */
					}
				},
			});
			...
			console.warn(
				'[IntelligenceEngine] coding verification skipped (non-fatal):',
				e?.message,
			);
		}
	}
	...
	private v3ModeRetrievalContext(): {
		...
		port: unknown;
		conversationWindow: (sec: number) => string;
	} | null {
		try {
			const {
			...
			const _modeId = resolveModeIdOrWarn(
				_mi ? raw : null,
				'engine/transcript-surface',
			...
			try {
				if (policy.profileSources?.length) {
					...
					const collected = collectV3ProfileSources(
						this.llmHelper.getKnowledgeOrchestrator?.() ?? null,
					);
			...
			const meetingId =
				(this.session as any)?.getMeetingMetadata?.()?.id ?? null;
			...
			try {
				...
				/* meeting/profile combination is additive — mode port alone still answers */
			}
			...
			return {
				...
				attachedFileNames: (_files as Array<{ fileName?: string }>)
					.map((f) => f.fileName ?? '')
					.filter(Boolean),
				profileSourceCount,
				...
				conversationWindow: (sec: number) =>
					String((this.session as any)?.getFormattedContext?.(sec) ?? '').slice(
						-2400,
					),
			};
		} catch {
	...
	private async buildV3ForTranscriptSurface(
		tag: 'assist' | 'clarify' | 'brainstorm' = 'assist',
		...
		try {
			...
			const {
				resolveQuestion,
			} = require('./context-intelligence/question/question-resolver');
			...
			const _v3 = await buildV3Prompt({
				...
				scope: {
					...
					sessionId: ctx.meetingId ?? undefined,
				},
	...
	async runFollowUp(
		...
		console.log(
			`[IntelligenceEngine] runFollowUp called with intent: ${intent}`,
		);
		const lastMsg = this.session.getLastAssistantMessage();
		if (!lastMsg) {
			console.warn(
				'[IntelligenceEngine] No lastAssistantMessage found for follow-up',
			);
			...
			this.emit(
				'refined_answer',
				"There's no answer yet to follow up on — ask something first.",
				intent,
			);
			return null;
		}
		...
		try {
			...
			const context =
				this.buildPreparedTranscriptContext(120) ||
				this.session.getFormattedContextWithInterim(60);
			...
			try {
				...
				const fuContract = this.buildRecapFollowUpContract(
					'follow_up',
					String(refinementRequest || ''),
				);
				if (fuContract) {
					const switchTo = contextOs.detectFollowUpSourceSwitch(
						String(refinementRequest || ''),
					);
					if (
						switchTo &&
						switchTo !==
							(fuContract.sourceOwner === 'reference_files'
								? 'reference_files'
								: fuContract.sourceOwner)
					) {
						const line =
							"Switching sources needs a fresh question — ask it directly and I'll answer from " +
							(switchTo === 'profile'
								? 'your profile.'
								: switchTo === 'reference_files'
					...
					followUpContractRule =
						contextOs.buildFollowUpContractRule(fuContract);
				}
			} catch {
				/* Context OS is additive — never break follow-up */
			}
			...
			const stream = this.followUpLLM.generateStream(
				...
				context,
				followUpContractRule
					? { contractRule: followUpContractRule }
					: undefined,
			);
			let streamAborted = false;
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] _follow_up stream aborted by new generation',
					);
			...
			if (!streamAborted && fullRefined) {
				...
				const displayQuestion =
					userRequest || intentMap[intent] || `Refining: ${intent}`;
	...
	private buildRecapFollowUpContract(
		...
		try {
			...
			const docInfo =
				ModesManager.getInstance().getActiveModeDocumentGroundingInfo?.() ??
				null;
			...
			const legacy = buildCustomModeExecutionContract({
				...
				isDocGroundedCustomModeActive:
					docInfo?.documentGroundedCustomModeActive === true,
				...
				hasLongTermMemory: false,
				persistedSourceAuthority:
					docInfo?.sourceContract?.sourceAuthority ?? null,
			});
	...
	async runRecap(): Promise<string | null> {
		...
		try {
			...
			if (!context) {
				...
				const fallback =
					"There's nothing to recap yet — start a conversation and I'll summarize it here.";
			...
			try {
				const contextOs = contextOsStatic;
				const recapContract = this.buildRecapFollowUpContract(
					'recap',
					'Recap the conversation so far',
				);
				if (recapContract)
					recapContractRule = contextOs.buildRecapContractRule(recapContract);
			} catch {
				/* Context OS is additive — never break recap */
			}

			const generationId = ++this.currentGenerationId;
			let fullSummary = '';
			const stream = this.recapLLM.generateStream(
				context,
				recapContractRule ? { contractRule: recapContractRule } : undefined,
			);
			let streamAborted = false;
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] _recap stream aborted by new generation',
					);
					await stream.return(undefined);
					streamAborted = true;
					break;
	...
	async runClarify(): Promise<string | null> {
		...
		try {
			...
			const context =
				rawContext ||
				'[No transcript or recent manual answer available yet. Generate an opening clarifying question to understand the scope and constraints of the upcoming problem.]';

			const generationId = ++this.currentGenerationId;
			let fullClarification = '';
			const clarifyV3 = await this.buildV3ForTranscriptSurface('clarify');
			const stream = this.clarifyLLM.generateStream(
				context,
				clarifyV3 ?? undefined,
			);
			let streamAborted = false;
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] _clarify stream aborted by new generation',
					);
					await stream.return(undefined);
					streamAborted = true;
					break;
	...
	async runFollowUpQuestions(): Promise<string | null> {
		...
		try {
			if (!this.followUpQuestionsLLM) {
				console.error(
					'[IntelligenceEngine] FollowUpQuestionsLLM not initialized',
				);
				this.setMode('idle');
				return null;
			}
			...
			if (!context) {
				console.warn(
					'[IntelligenceEngine] No transcript or recent manual answer available for follow-up questions',
				);
				this.setMode('idle');
				...
				this.emit(
					...
					"There's no conversation yet to suggest questions from — start talking and try again.",
				);
				return null;
			}
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] _follow_up_questions stream aborted by new generation',
					);
					await stream.return(undefined);
					streamAborted = true;
					break;
	...
	async runManualAnswer(question: string): Promise<string | null> {
		...
		try {
			...
			try {
				...
				recordLegacyTurn({
					...
					scope: {
						userId: 'local',
						meetingId:
							(this.session as any)?.getMeetingMetadata?.()?.id ?? undefined,
					},
					...
					answerability: 'NONE',
					legacyPath:
						'IntelligenceEngine.runManualAnswer (no source authority)',
				});
			} catch {
				/* observability must never break an answer */
			}
			...
			if (_v3) {
				...
				const _docEnforced =
					(activeModeInfo as any)?.strictDocumentGroundedActive === true ||
					(activeModeInfo?.documentGroundedCustomModeActive === true &&
						Boolean((activeModeInfo as any)?.hasReferenceFiles));
				const context =
					_docEnforced || isCodingAnswerType(answerPlan.answerType)
						? undefined
	...
	async runCodeHint(
		...
		try {
			...
			const questionContext =
				problemStatement ?? sessionQuestion.question ?? null;
			...
			const transcriptContext =
				questionContext === null ? this.session.getFormattedContext(180) : null;

			console.log(
				`[IntelligenceEngine] Code hint — question source: ${questionContext ? (questionSource ?? 'passed') : 'none'}, transcript lines: ${transcriptContext ? transcriptContext.split('\n').length : 0}, images: ${imagePaths?.length ?? 0}`,
			);
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] code_hint stream aborted by new generation',
					);
					await stream.return(undefined);
					streamAborted = true;
					break;
			...
			if (!fullHint || fullHint.trim().length < 5) {
				fullHint =
					"I couldn't detect any code in the screenshot. Try screenshotting your code editor directly.";
			}
	...
	async runBrainstorm(
		...
		try {
			...
			const stream = this.brainstormLLM.generateStream(
				context,
				imagePaths,
				brainstormV3 ?? undefined,
			);
			let streamAborted = false;
			...
			for await (const token of stream) {
				if (this.currentGenerationId !== generationId) {
					console.log(
						'[IntelligenceEngine] brainstorm stream aborted by new generation',
					);
					await stream.return(undefined);
					streamAborted = true;
					break;
	...
	private getActiveModeId(): string {
		try {
			const { ModesManager } =
				require('./services/ModesManager') as typeof import('./services/ModesManager');
			return (
				ModesManager.getInstance().getActiveMode()?.templateType || 'general'
			);
		} catch {
			return 'general';
		}
