...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle("open-external", async (event, url: string) => {
    try {
      // Protocol-gated shell open to prevent invalid protocols (like macOS system prefs)
      // from triggering unexpected behavior (like opening Microsoft Store) on other OSs.
      if (url.startsWith("x-apple.systempreferences:")) {
        if (process.platform === "darwin") {
          await shell.openExternal(url);
        } else {
          console.warn(`[IPC] Blocked macOS-specific protocol on ${process.platform}: ${url}`);
          // On non-macOS, fall back to Natively's settings window
  ...
  safeHandle("open-privacy-settings", async (_, type: "microphone" | "screen") => {
    if (process.platform === "darwin") {
      ...
      const url = type === "screen" ? "ms-settings:sound" : "ms-settings:privacy-microphone";
      await shell.openExternal(url);
    }
  });
  ...
  safeHandle("generate-code-hint", async (_, imagePaths?: string[], problemStatement?: string) => {
    try {
      ...
      const resolvedImagePaths: string[] =
        imagePaths && imagePaths.length > 0 ? imagePaths : appState.getScreenshotQueue();
  ...
  safeHandle("profile:get-profile", async () => {
    try {
      ...
      const dbData = {
        customNotes: db.getCustomNotes(),
        persona: db.getPersona(),
      };

      if (!orchestrator) return { ...dbData, hasProfile: false };
      
      const profileData = orchestrator.getProfileData();
      return { ...profileData, ...dbData };
    } catch (error: any) {
      console.error("[IPC] profile:get-profile failed:", error);
      return null;
    }
  });
  ...
  safeHandle("profile:get-persona", async () => {
    try {
      return DatabaseManager.getInstance().getPersona();
    } catch (error: any) {
      return "";
    }
  });
