...
import type {
	...
	SafeWebsiteMetadata,
} from './services/browser-context/types';
...
} from './services/skills/SkillValidator';
...
import {
	...
	buildPriorCodingContextBlock as buildPriorCodingBlockForV3,
...
import {
	...
	type FinalGenerationMode,
	type SessionWriteDecision,
} from './llm/FinalAnswerGenerationPolicy';
...
import {
	...
	getSourceOwnerEnforcementStage,
} from './intelligence/intelligenceFlags';
import {
	recordAttribution,
	hindsightModeFor,
	type AttributionInput,
} from './intelligence/IntelligenceAttribution';
import {
	routeContext,
	isBackwardLookingQuery,
} from './intelligence/ContextRouter';
import {
	SearchOrchestrator,
	type SearchCandidate,
} from './intelligence/SearchOrchestrator';
...
export function initializeIpcHandlers(appState: AppState): void {
	...
	const refreshRuntimeDefaultIfUnavailable = async (): Promise<
		string | null
	> => {
		try {
			...
			try {
				...
				codexSignedIn =
					CodexOAuthService.getInstance().getStatus().signedIn === true;
			} catch {
				/* optional */
			}
			...
			const isKnownGroqModel = (modelId: string): boolean => {
				...
				); // Groq-hosted OpenAI OSS models, not OpenAI API models.
			};
			...
			const modelAvailable = (modelId: string): boolean => {
				...
				if (
					disabledProviders.includes(family) ||
					disabledProviders.includes(modelId)
				)
					return false;
				...
				if (optInFamily) {
					...
					enabledForFamily.length > 0 &&
					!enabledForFamily.includes(modelId)
				)
					return false;

				if (modelId === 'natively') return has(cm.getNativelyApiKey());
				if (modelId.startsWith('codex-cli'))
					return codexConfig.enabled === true && codexSignedIn;
			...
			if (has(cm.getLitellmBaseURL())) {
				try {
					const baseURL = (
						cm.getLitellmBaseURL() || 'http://localhost:4000/v1'
					).replace(/\/+$/, '');
					...
					if (resp.ok) {
						...
						const preferredBare = preferred?.startsWith('litellm/')
							? preferred.slice('litellm/'.length)
							: preferred;
						const chosen =
							preferredBare && ids.includes(preferredBare)
								? preferredBare
								: ids[0];
						if (chosen) litellmFallbackModel = `litellm/${chosen}`;
					}
				} catch {
					/* LiteLLM fallback discovery best-effort */
				}
			}
			...
			const next = modelAvailable('natively')
				? 'natively'
				: modelAvailable('gemini-3.7-flash')
					? 'gemini-3.7-flash'
					: modelAvailable('gpt-5.4')
						? 'gpt-5.4'
						: modelAvailable('claude-sonnet-4-6')
							? 'claude-sonnet-4-6'
							: modelAvailable('llama-3.3-70b-versatile')
								? 'llama-3.3-70b-versatile'
								: modelAvailable('deepseek-v4-flash')
									? 'deepseek-v4-flash'
									: codexConfig.enabled === true &&
										  codexSignedIn &&
										  modelAvailable('codex-cli')
										? 'codex-cli'
										: litellmFallbackModel &&
											  modelAvailable(litellmFallbackModel)
											? litellmFallbackModel
											: allProviders.find((p: any) => modelAvailable(p?.id))
													?.id || null;
			if (!next) {
				...
				console.warn(
					'[IPC] refreshRuntimeDefaultIfUnavailable: no available model (all providers disabled or unconfigured)',
				);
				return null;
			}
			...
			console.warn(
				'[IPC] default model availability refresh failed:',
				error?.message || error,
			);
			return null;
		}
		...
		const normalized = String(text || '')
			...
			.map((line) => {
				const stripped = line.replace(
					/^\s*\[(?:[A-Z][A-Z0-9 _-]*|SYSTEM|DEVELOPER|USER|ASSISTANT|ME|INTERVIEWER|RECENT|NEW|IMPORTANT|INSTRUCTION|CONTEXT|TRANSCRIPT|TOOL|PROMPT|HUMAN|AI|BOT|GPT|OVERRIDE)[^\]]*\]\s*:?\s*/i,
					'',
				);
				return stripped === line
					? line
					: `quoted previous content: ${stripped || '(context header removed)'}`;
			})
			.join('\n')
			.trim();
		const clipped =
	...
	safeHandle('test-release-fetch', async () => {
		try {
			...
			const notes = await ReleaseNotesManager.getInstance().fetchReleaseNotes(
				'latest',
				true,
			);
	...
	safeHandle(
		...
		async (_event, opts?: { budgets?: number[]; repeats?: number }) => {
			try {
				const llmHelper = appState.processingHelper?.getLLMHelper?.();
				if (!llmHelper) return { ok: false, error: 'LLMHelper unavailable' };
				const {
					runThinkingBudgetBench,
				} = require('./services/dev/ThinkingBudgetBench');
				...
				return {
					...
					path:
						...
						'/thinking-budget-bench-results.json',
				};
			} catch (err: any) {
	...
	safeHandle('set-ai-response-language', async (_, language: string) => {
		...
		if (!language || typeof language !== 'string' || !language.trim()) {
			console.warn(
				'[IPC] set-ai-response-language: invalid or empty language received, ignoring.',
			);
		...
		if (llmHelper) {
			...
			console.log(
				`[IPC] AI response language updated to: ${sanitizedLanguage}`,
			);
		} else {
			console.warn(
	...
	safeHandle(
		...
		async (event, payload: { panelRight?: number }) => {
			const overlayWin = appState.getWindowHelper().getOverlayWindow();
			if (!overlayWin || overlayWin.isDestroyed()) return;
			if (overlayWin.webContents.id !== event.sender.id) return;
	...
	safeHandle(
		'overlay-hover-interactive',
		async (event, interactive: boolean) => {
			const overlayWin = appState.getWindowHelper().getOverlayWindow();
			if (!overlayWin || overlayWin.isDestroyed()) return;
			if (overlayWin.webContents.id !== event.sender.id) return;
	...
	safeHandle('overlay-ui-action', async (event, action: { type?: string }) => {
		...
		const fromAux =
			...
			(toggleWin &&
				!toggleWin.isDestroyed() &&
				toggleWin.webContents.id === event.sender.id);
	...
	safeHandle(
		...
		async (
			event,
			delta: { dx?: number; dy?: number; phase?: 'start' | 'move' | 'end' },
		) => {
			const helper = appState.getWindowHelper();
			const pillWin = helper.getPillWindow();
			const fromPill =
	...
	safeHandle(
		'set-window-mode',
		async (event, mode: 'launcher' | 'overlay', inactive?: boolean) => {
	...
	safeHandle(
		...
		async (
			...
			try {
				...
				console.log(`[IPC] gemini - chat response received`, {
	...
	const _chatStreamsBySender = new Map<
		number,
		{ streamId: number; controller: AbortController }
	>();
	...
	const { AnswerDiversityGuard } =
		require('./llm/answerPolish') as typeof import('./llm/answerPolish');
	...
	const { ConversationMemoryService } =
		require('./intelligence/ConversationMemoryService') as typeof import('./intelligence/ConversationMemoryService');
	...
	const { CodingConversationState } =
		require('./intelligence/CodingConversationState') as typeof import('./intelligence/CodingConversationState');
	...
	const _geminiChatStreamHandler = async (
		...
		const { ForegroundGate } =
			require('./services/ForegroundGate') as typeof import('./services/ForegroundGate');
		try {
			const llmHelper = appState.processingHelper.getLLMHelper();
			...
			_chatStreamsBySender.set(senderId, {
				streamId: myStreamId,
			...
			// True only when the user typed /skill or $skill. Default-applied
			// enabled skills must not skip the identity probe.
			let skillWasExplicit = false;
			const skillPrefixMatch =
			...
			if (skillPrefixMatch) {
				try {
					...
					if (skill) {
						...
						skillStrippedMessage =
							strippedQuery || `Please help me with the ${skill.name} skill.`;
					...
					console.warn(
						'[IPC] Skill lookup failed:',
						skillErr?.message || skillErr,
					);
					event.sender.send(
						'gemini-stream-error',
						`Skill lookup failed: ${skillErr?.message || 'unknown error'}`,
						{ streamId: myStreamId },
					);
					return null; // sibling error paths return null; handler is typed `| null`
				}
			...
			if (!skillPromptBlock) {
				try {
					skillPromptBlock =
						SkillsManager.getInstance().buildDefaultChatSkillPromptBlock();
				} catch (skillErr: any) {
					console.warn(
						'[IPC] Default skill injection failed:',
						skillErr?.message || skillErr,
					);
				}
			}
			...
			try {
				const {
					isContextIntelligenceV3Enabled,
				...
				const callerOwnsPrompt =
					options?.skipSystemPrompt === true && Boolean(context);
				if (!callerOwnsPrompt && isContextIntelligenceV3Enabled()) {
					const {
						buildV3Prompt,
					} = require('./context-intelligence/orchestration/engine-bridge');
					const {
						resolveModePolicy,
						isModeId,
						resolveModeIdOrWarn,
					} = require('./context-intelligence/policies/mode-policy-registry');
					const { ModesManager } = require('./services/ModesManager');
					...
					const modeId = resolveModeIdOrWarn(
						modeInfo ? rawMode : null,
						'ipc/manual-chat',
						{ quietWhenAbsent: true },
					);
					const policy = resolveModePolicy(modeId);

					const files = modeInfo?.id
						? (mm.getReferenceFiles?.(modeInfo.id) ?? [])
						: [];
					...
					const {
						createModeRetrievalPort,
						attachmentSourceTypeExtensions,
					...
					const extraSourceTypes = attachmentSourceTypeExtensions(
						modeId,
						files,
					);
					const effectiveAllowedSourceTypes = [
					...
					try {
						const {
							getContextDebugLevel,
						} = require('./context-intelligence/debug/debug-config');
						if (getContextDebugLevel() !== 'off') {
							const {
								sourceTypeForFile,
								detectDocumentStatus,
							} = require('./context-intelligence/retrieval/mode-retrieval-port');
							v3DebugSources = (files as Array<Record<string, unknown>>).map(
								(f) => ({
									id: String(f.id ?? ''),
									role: sourceTypeForFile(
										f.fileName as string | undefined,
										f.content as string | undefined,
										effectiveAllowedSourceTypes,
									),
									name: f.fileName,
									...(detectDocumentStatus(f.content as string | undefined)
										? {
												status: detectDocumentStatus(
													f.content as string | undefined,
												),
											}
										: {}),
					...
					// rule (06 §4).
					const v3MeetingId =
						(appState.getIntelligenceManager?.() as any)
							?.getSessionTracker?.()
							?.getMeetingMetadata?.()?.id ?? null;
					const ragForV3 = appState.getRAGManager?.();
					const wantsMeeting =
						policy.allowedSourceTypes.includes('MEETING_TRANSCRIPT') &&
						Boolean(v3MeetingId) &&
						Boolean(ragForV3?.getRetriever);
					...
					try {
						if (policy.profileSources?.length) {
							...
							} = require('./services/knowledge/v3ProfileSources');
							const collected = collectV3ProfileSources(
								llmHelper.getKnowledgeOrchestrator?.() ?? null,
							);
							if (collected.docs.length) {
								const {
									createProfileRetrievalPort,
								} = require('./context-intelligence/retrieval/profile-retrieval-port');
						...
						console.warn(
							'[V3] profile hydration failed — continuing with mode attachments only:',
							(profErr as Error)?.message ?? profErr,
						);
					}
					...
					const port =
						v3Ports.length > 1
							? combineRetrievalPorts(v3Ports as never[])
							: modePort;
					...
					const composed = await buildV3Prompt({
						...
						codingTask: (() => {
							try {
								...
								return undefined;
							} // fall back to the bridge's own check
						})(),
						...
						attachedFileNames: (files as Array<{ fileName?: string }>)
							...
							.filter(Boolean),
						profileSourceCount:
							v3ProfileCounts.profileResume +
							v3ProfileCounts.profileJd +
							v3ProfileCounts.profileFact,
						...
						personaBase: ({ codingTask }: { codingTask: boolean }) => {
							...
							let bareCode = false;
							if (
								isBareCodeRequest(v3Question) ||
								isCodingContinuation(v3Question)
							) {
								try {
									...
									const lastAnywhere = appState
										.getIntelligenceManager?.()
										?.getLastAssistantMessage?.();
									bareCode = isBareCodeRequest(v3Question);
									if (
										typeof lastAnywhere === 'string' &&
										lastAnywhere.trim().length > 40 &&
										...
										priorProblem =
											...
											buildPriorCodingBlockForV3({
												userMessage:
													'(the question answered just before this one)',
												assistantAnswer: lastAnywhere,
											});
									}
								} catch {
									/* guard only; never blocks the answer */
								}
							}
							...
							const codingSignals = (() => {
								try {
									...
									if (
										...
										(!v3Question.trim() ||
											require('./llm/codingPromptSignals').isDeicticAsk(
												v3Question,
											))
									) {
										return {
											codingTask: true,
											codingTaskKind: 'dsa',
										} as import('./llm/codingPromptSignals').CodingPromptSignals;
									}
									return resolved;
								} catch {
									return {
							...
							const base = resolveV2SystemPrompt({
								...
								tier: v2TierForPromptTier(llmHelper?.getPromptTier?.()),
								activeMode: modeInfo,
								// Default overlay chat is Meeting Copilot (JS, Core Answer).
								// The DSA contract lists ```python first and forces six
								// interview headings, which overrides the skill. Keep the
								// contract only when the user explicitly invoked /skill.
								codingTask: skillWasExplicit
									? codingTask || codingSignals.codingTask || !!priorProblem
									: false,
								...
								codingFormat:
									priorProblem && bareCode
										? 'code_only'
										: codingSignals.codingFormat,
					...
					if (composed) {
						...
						const v3DebugCollector = (() => {
							try {
								...
								const {
									getTurnCollector,
								} = require('./context-intelligence/debug/turn-collector');
						...
						try {
							v3DebugCollector?.recordGenerationStart({
								provider: 'llmHelper',
							});
						} catch {
							/* noop */
						...
						try {
							for await (const tok of v3Stream.stream) {
								...
								if (!v3SawFirstToken) {
									...
									try {
										v3DebugCollector?.recordFirstToken();
									} catch {
										/* noop */
									}
								}
								finalText += tok;
								event.sender.send('gemini-stream-token', tok, {
									streamId: myStreamId,
								});
							}
						} catch (streamErr) {
						...
						// the record guard below stays as defense in depth for writes.
						{
							...
							try {
								liveModeIdAtEmit = mm.getActiveMode()?.id ?? null;
							} catch {
								/* fail open — emit-guard only */
							}
							if (liveModeIdAtEmit !== v3RequestModeId) {
								console.warn(
									'[IPC] stale stream suppressed: mode changed mid-generation',
									{
										streamId: myStreamId,
						...
						if (v3Truncated) {
							console.warn(
								'[IPC] manual chat answer is INCOMPLETE — not storing it as conversation history',
								{
									streamId: myStreamId,
						...
						event.sender.send('gemini-stream-done', {
							...
							incompleteReason: v3Truncated
								? v3Stream.outcome.reason
								: undefined,
						});
						finishDebug(
							finalText,
							!v3Truncated,
							v3Truncated ? 'stream_truncated' : null,
						);
						...
						try {
							liveModeIdAtRecord = mm.getActiveMode()?.id ?? null;
						} catch {
							/* record-guard only */
						}
						if (liveModeIdAtRecord === (manualActiveMode?.id ?? null)) {
							...
							if (v3Truncated) {
								console.warn(
									'[IPC] truncated answer — recording the user turn but skipping answer-side history/memory sinks',
									{ streamId: myStreamId },
								);
							}
				...
				console.error(
					'[ContextIntelligenceV3] manual-chat path failed, falling back to legacy:',
					v3Err?.message || v3Err,
				);
				try {
					require('./context-intelligence/observability/rollout-metrics').recordV3Fallback(
						'manual-chat-ipc',
						v3Err,
					);
				} catch {
			...
			try {
				if (!_convoCleanupRegistered.has(senderId)) {
					...
					event.sender?.once?.('destroyed', () => {
						...
						try {
							_manualConversationMemory.clearSession(String(senderId));
						} catch {
							/* noop */
						}
						try {
			...
			// !skillWasExplicit: an explicit /skill turn must never be hijacked by
			// the canned identity reply. Default-injected enabled skills still allow
			// the probe. This was previously incidental — the probe ran on the raw
			...
			if (
				...
				const { resolveIdentityProbe } =
					require('./llm/manualIdentityRouting') as typeof import('./llm/manualIdentityRouting');
				let probeProfileReady = false;
				try {
					const orchProbe = llmHelper.getKnowledgeOrchestrator?.();
					probeProfileReady = profileFactsReady(
						(orchProbe as any)?.activeResume?.structured_data ?? null,
					);
				} catch {
					/* no profile — assistant reply stands */
				}
			...
			let manualActiveMode: import('./llm/modeProfiles').ActiveModeInfo | null =
				null;
			try {
				const { ModesManager } = require('./services/ModesManager');
				manualActiveMode = ModesManager.getInstance().getActiveModeInfo();
			} catch {
				/* mode prior unavailable — planAnswer stays mode-blind */
			}
			...
			let manualSourceContract:
				| import('./llm/customModeExecutionContract').CustomModeExecutionContract
				| null = null;
			let manualOwnership:
				| import('./llm/sourceOwnership').SourceOwnershipDecision
				| null = null;
			const _hasProfileFactsForTurn = Boolean(
				llmHelper.getKnowledgeOrchestrator?.()?.activeResume?.structured_data,
			);
			...
			let _userExplicitSource:
				| 'reference_files'
				| 'profile'
				| 'transcript'
				| null = null;
			let manualTurnSourceDecision:
				| import('./llm/turnSourceDecision').TurnSourceDecision
				| null = null;
			try {
				const {
					buildCustomModeExecutionContract,
					logArbitratedContract,
				} = require('./llm/customModeExecutionContract');
				const { resolveTurnSourceDecision } =
					require('./llm/turnSourceDecision') as typeof import('./llm/turnSourceDecision');
				const { resolveSourceOwnership } = require('./llm/sourceOwnership');
				const _docGrounded =
					manualActiveMode?.documentGroundedCustomModeActive === true;
				const _hasRefFiles = Boolean(
					(manualActiveMode && (manualActiveMode as any).hasReferenceFiles) ??
					false,
				);
				const _hasCustomPrompt = Boolean(
					(manualActiveMode && (manualActiveMode as any).hasCustomPrompt) ??
					false,
				);
				const _hasLiveTranscript = Boolean(
					intelligenceManager.getFormattedContext(100)?.trim(),
				);
				...
				const _hasLongTermMemory = Boolean(
				...
				const {
					resolveExplicitSourceRequest,
					resolveExplicitSourceRequests,
					toLegacyUserExplicitSource,
				} = require('./intelligence/context-os/explicitSourceSwitch');
				const _explicitRequests = resolveExplicitSourceRequests(
					String(message || ''),
				);
				const _explicitSwitch = resolveExplicitSourceRequest(
					String(message || ''),
				);
					...
					? resolveTurnSourceDecision({
							...
							availability: {
								...
								hasJobDescription: Boolean(
									_orchForAvailability?.activeJD?.structured_data,
								),
				...
				try {
					const {
						recordLegacyTurn,
					} = require('./context-intelligence/observability/legacy-trace');
					recordLegacyTurn({
						...
						groundingPolicy: (_activeSourceContract?.sourceAuthority ??
							undefined) as never,
						retrievalPath: 'GROUNDED',
						legacyPath:
							'ipcHandlers.gemini-chat-stream resolveTurnSourceDecision',
					});
				} catch {
					/* observability must never break an answer */
				}
				...
				manualSourceContract = buildCustomModeExecutionContract({
					...
					persistedSourceAuthority:
						manualActiveMode?.sourceContract?.sourceAuthority ?? null,
				...
				if (isIntelligenceFlagEnabled('trace')) {
					// manualOwnership! below: assigned unconditionally at 1790-1797
					console.log(
						'[SOURCE-OWNERSHIP]',
						JSON.stringify({
							...
							shouldClarifyInsteadOfProfile:
								manualOwnership!.shouldClarifyInsteadOfProfile,
				...
				if (isIntelligenceFlagEnabled('trace')) {
					console.warn(
						'[SOURCE-ARBITER] skipped (non-fatal):',
						arbiterErr?.message,
					);
				}
			}
			...
			let turnContract:
				| import('./intelligence/context-os').TurnContextContract
				| null = null;
			...
			let manualContextOsGeneration:
				| import('./intelligence/context-os').ContextOsGenerationContext
				| null = null;
			try {
				if (manualSourceContract) {
					const { buildTurnContractIfEnabled } =
					...
					turnContract = buildTurnContractIfEnabled({
						...
						hasReferenceFiles: Boolean(
							(manualActiveMode as any)?.hasReferenceFiles,
						),
					...
					if (
						turnContract &&
						isIntelligenceFlagEnabled('contextOsImpossibleStateGateShadow')
					) {
						try {
							const { ProfileEvidenceService, checkImpossibleEvidenceState } =
								require('./intelligence/context-os') as typeof import('./intelligence/context-os');
							const orchestratorForShadow =
								llmHelper.getKnowledgeOrchestrator?.();
							const shadowResume =
								(orchestratorForShadow as any)?.activeResume?.structured_data ??
								null;
							const shadowJd =
								(orchestratorForShadow as any)?.activeJD?.structured_data ??
								null;
							...
							const shadowResult = checkImpossibleEvidenceState(
								shadowPack,
								answerPlan.profileContextPolicy,
							);
							...
							if (!shadowResult.ok) {
								console.warn('[IMPOSSIBLE-STATE-GATE-SHADOW] violation', {
									...
									violationSourceKinds: shadowResult.violations.map(
										(v) => v.sourceKind,
									),
							...
							if (isIntelligenceFlagEnabled('trace')) {
								console.warn(
									'[IMPOSSIBLE-STATE-GATE-SHADOW] skipped (non-fatal):',
									shadowGateErr?.message,
								);
							}
						}
					}
				...
				if (isIntelligenceFlagEnabled('trace')) {
					console.warn(
						'[CONTEXT-OS] contract build skipped (non-fatal):',
						contextOsErr?.message,
					);
				}
			}
			let isCodingChat = isCodingAnswerType(answerPlan.answerType);
			chatTrace.mark('answer_type_selected', {
				answerType: answerPlan.answerType,
				isCoding: isCodingChat,
			});
			piTelemetry.emit('pi_answer_plan_created', {
				answerType: answerPlan.answerType,
				surface: 'manual',
				isCoding: isCodingChat,
			...
			let explicitCodingContract: ExplicitCodingContract =
				detectExplicitCodingContract(message);
					...
					if (
						...
						if (!isCodingChat) {
							...
							iTrace.noteContext({
								...
								retrieved: true,
								included: true,
								reason: 'coding_followup_prior_problem',
							});
						}
			...
			}
			{
				...
				const wantsOriginalProblem =
					convMemOn && _manualCodingState.isOriginalProblemQuery(message);
				...
				if ((looksLikeCodingFollowup || wantsOriginalProblem) && convMemOn) {
					try {
						const priorCoding = _manualConversationMemory.getLastCodingTurn(
							String(senderId),
						);
						const resolvedProblem = _manualCodingState.resolveProblemFor(
							String(senderId),
							message,
						);
						if (
							priorCoding &&
							priorCoding.userMessage &&
							priorCoding.assistantAnswer
						) {
							if (
								wantsOriginalProblem &&
								resolvedProblem?.isOriginal &&
							...
							chatTrace.mark('coding_followup_resolved' as any, {
								explicitContract: explicitCodingContract || 'none',
							});
							piTelemetry.emit('pi_context_policy_applied', {
								answerType: answerPlan.answerType,
								via: wantsOriginalProblem
									? 'coding_original_recall'
									: 'coding_followup',
								profilePolicy: 'forbidden',
							});
						}
					} catch {
						/* memory recall never blocks the answer */
					}
				}
			}
			...
			const _attr: AttributionInput = {
				...
				context_router_mode: isIntelligenceFlagEnabled('contextRouterV2')
					? 'shadow'
					: 'off',
			...
			try {
				if (isIntelligenceFlagEnabled('contextRouterV2')) {
					...
					const routerProfileAvailable = profileFactsReady(
						(orchRouter as any)?.activeResume?.structured_data ?? null,
					);
					const routerDecision = routeContext(
						{
							...
							jdAvailable: Boolean(
								(orchRouter as any)?.activeJD?.structured_data,
							),
						},
						iTrace,
					);
					...
					const liveWantsProfile =
						routerProfileAvailable &&
						(answerPlan.profileContextPolicy === 'required' ||
							answerPlan.requiredContextLayers.some(
								(l) => l === 'stable_identity' || l === 'resume' || l === 'jd',
							));
				...
				/* shadow routing is observe-only; never affects the answer */
			}
			...
			if (
				...
				(!turnContract ||
					turnContract.memoryReadPolicy.allowPriorAssistantFacts)
			) {
				try {
					const prior = _manualConversationMemory.resolveSameSession(
						String(senderId),
						message,
					);
			...
			if (
				...
				try {
					const prior =
						_manualConversationMemory.resolveSameSession(
							String(senderId),
							message,
						) ||
						(() => {
							const a = _manualConversationMemory.getLastAssistantAnswer(
								String(senderId),
							);
							return a
								? ({ userMessage: '', assistantAnswer: a } as any)
								: null;
						})();
					if (prior && prior.assistantAnswer) {
						...
						iTrace.noteContext({
							...
							retrieved: true,
							included: true,
							reason: 'refinement_followup',
						});
						_attr.conversation_memory_used = true;
						_attr.conversation_memory_turns_used = 1;
						piTelemetry.emit('pi_context_policy_applied', {
							answerType: answerPlan.answerType,
							via: 'refinement_followup',
							profilePolicy: answerPlan.profileContextPolicy,
						});
					}
				} catch {
					/* refinement recall never blocks the answer */
				}
			}
			if (!context && !autoContextSnapshot && isBareFollowUp(message)) {
				...
				try {
					const { ModesManager } = require('./services/ModesManager');
					const tpl =
						ModesManager.getInstance().getActiveModeInfo()?.templateType;
				...
				chatTrace.mark('response_completed', {
					chars: clarification.length,
					deterministic: true,
				});
				...
				iTrace
					...
					.noteFallback('context_free_clarification');
				commitTrace(iTrace);
				_emitAttr({
					answer_type: 'follow_up_answer',
					conversation_memory_used: Boolean(context),
				});
				return null;
			}
			...
			if (
				...
				try {
					const { buildSourceClarification } =
						require('./intelligence/context-os') as typeof import('./intelligence/context-os');
					...
					const clarifyWrite = decideSessionWritePolicy({
						finalGenerationMode: 'source_safe_refusal',
						validationOk: true,
					...
					iTrace
						...
						.noteFallback('context_os_clarification');
					if (isIntelligenceFlagEnabled('trace')) {
						const { buildContextOsTrace, logContextOsTrace } =
							require('./intelligence/context-os') as typeof import('./intelligence/context-os');
						logContextOsTrace(
							buildContextOsTrace({
								contract: turnContract,
								sourceAuthority:
									manualSourceContract?.sourceAuthority ?? 'ask_if_ambiguous',
								question: String(message || ''),
								usedSources: [],
								finalAction: 'clarify',
							}),
					...
					if (isIntelligenceFlagEnabled('trace')) {
						console.warn(
							'[CONTEXT-OS] clarification short-circuit skipped (non-fatal):',
							clarErr?.message,
						);
					}
				}
			...
			const _contractAllowsProfile = (() => {
				...
				try {
					...
					return (
						allowsEvidence(turnContract, 'profile_resume') ||
			...
			const _impossibleStateGateAllowsProfile = (() => {
				...
				try {
					...
					const gateResume =
						(orchestratorForGate as any)?.activeResume?.structured_data ?? null;
					const gateJd =
						(orchestratorForGate as any)?.activeJD?.structured_data ?? null;
					...
					const gateResult = checkImpossibleEvidenceState(
						gatePack,
						answerPlan.profileContextPolicy,
					);
					...
					if (enforceableViolations.length > 0) {
						if (isIntelligenceFlagEnabled('trace')) {
							console.warn(
								'[IMPOSSIBLE-STATE-GATE-ENFORCE] blocked profile injection',
								{
			...
			const sourceOwnershipAllowsProfile =
				(manualOwnership && !_ownerEnforcementOff
					? manualOwnership.profileAllowed
					: legacyDocGuardEligible) &&
				_contractAllowsProfile &&
				_impossibleStateGateAllowsProfile;
			...
			const KNOWN_COORDINATOR_KINDS_EARLY = new Set([
				'reference_files',
				'profile_resume',
				'projects',
				'profile_jd',
			]);
			const coordinatorWouldHandleMixedTurn =
				Boolean(manualTurnSourceDecision) &&
				manualTurnSourceDecision!.requiredEvidenceKinds.length > 0 &&
				manualTurnSourceDecision!.requiredEvidenceKinds.every((k) =>
					KNOWN_COORDINATOR_KINDS_EARLY.has(k),
				) &&
				manualTurnSourceDecision!.requiredEvidenceKinds.includes(
					'reference_files',
				) &&
				manualTurnSourceDecision!.requiredEvidenceKinds.some(
					(k) =>
						k === 'profile_resume' || k === 'projects' || k === 'profile_jd',
				);
			const coordinatorReachableForThisTurn =
			...
			let selectedProfileEvidence:
				| import('./llm/manualProfileIntelligence').ManualProfileRouteResult
				| null = null;
			let profileJitPrompt: ReturnType<typeof buildProfileJitPrompt> | null =
				null;
			...
			if (
				...
				try {
					...
					iTrace
						...
						.noteFallback('source_switch_clarification');
					if (isIntelligenceFlagEnabled('trace')) {
						console.log(
							'[SOURCE-GUARD] blocked source=profile reason=explicit_profile_ask_in_reference_mode',
							{
								owner: manualOwnership.owner,
					...
					console.warn(
						'[SOURCE-GUARD] clarify emit skipped (non-fatal):',
						clarErr?.message,
					);
				}
			}
			...
			if (profileEvidenceEligible) {
				try {
					...
					const allowedSourceKinds: string[] | undefined =
						manualTurnSourceDecision
							...
							: undefined;
					const { route: evidenceRoute, routeLog } =
						buildManualProfileEvidenceRoute({
							question: message,
							orchestrator,
							source: 'manual_input',
					...
					if (evidenceRoute || routeLog.profileFactsReady) {
						console.log(
							'[ProfileIntelligence] manual evidence route',
							routeLog,
						);
					}
					if (evidenceRoute) {
						...
						try {
							...
							} = require('./llm/manualProfileIntelligence');
							const {
								buildActiveProfileContext,
								summarizeActiveProfileContext,
							} = require('./llm/ActiveProfileContext');
							...
							chatTrace.mark(
								'profile_evidence_diagnostics' as any,
								evidenceDiagnostics,
							);
							if (isIntelligenceFlagEnabled('trace')) {
								console.log(
									'[ProfileIntelligence] evidence diagnostics',
									evidenceDiagnostics,
								);
							}
						...
						iTrace.noteContext({
							...
							retrieved: true,
							included: true,
							reason: 'manual_jit_evidence_selection',
						});
						_attr.profile_tree_used = true;
						_attr.profile_tree_fast_path_used = false;
						_attr.structured_resume_used = evidenceRoute.items.some(
							(item) =>
								item.sourceKind === 'profile_resume' ||
								item.sourceKind === 'projects',
						);
					}
				} catch (profileRouteError: any) {
					console.warn(
						'[ProfileIntelligence] manual evidence preflight failed; falling back to generic chat:',
						profileRouteError?.message || profileRouteError,
					);
				}
			}
			...
			if (!isCodingChat) {
				try {
					...
					const wantsProfileContext = answerPlan.requiredContextLayers.some(
						(layer) =>
					...
					if (wantsProfileContext || profileReady) {
						console.log('[ProfileIntelligence] manual route', {
							...
							questionHash: crypto
								...
								.slice(0, 12),
							answerType: answerPlan.answerType,
							selectedContextLayers: wantsProfileContext
								? answerPlan.requiredContextLayers
								: [],
							...
							promptContainsProfileContext: Boolean(
								profileReady && wantsProfileContext,
							),
						});
					}
				} catch {
			...
			const isContractEnforced = CONTRACT_ENFORCED_TYPES.has(
			...
			if (isCodingChat) {
				...
				if (explicitCodingContract) {
					...
					const codingContract = buildCodingContractPrompt(
					...
					context = codingPriorProblemBlock
						? `${codingContract}\n\n${codingPriorProblemBlock}`
						: codingContract;
				} else if (planIsCodingType) {
					// Plain coding question (no constraint) → the EXACT proven path, byte unchanged.
					const baseContract = formatAnswerPlanForPrompt(
						answerPlan,
						isCodeVerificationEnabled(),
					);
					context = codingPriorProblemBlock
				...
				console.log(
					'[IPC] Coding contract enforced; rolling context excluded',
					{
						answerType: answerPlan.answerType,
						explicitContract: explicitCodingContract || 'none',
				...
				console.log(
					'[IPC] Answer-contract enforced; rolling context excluded',
					{
						answerType: answerPlan.answerType,
					},
				);
				...
				if (_stripFires) {
					...
					if (isIntelligenceFlagEnabled('trace')) {
						console.log(
							'[SOURCE-GUARD] blocked source=prior_assistant_facts reason=document_grounded_contract',
							{
								answerType: answerPlan.answerType,
								modeId: manualActiveMode?.id,
								strippedLength:
									autoContextSnapshot.length - snapshotForContext.length,
							},
						);
					}
				}
			...
			const wantsCandidateContract =
				CANDIDATE_CONTRACT_TYPES.has(answerPlan.answerType) ||
			...
			if (
				...
				const contractWithVoice = humanize
					? `${candidateContract}\n\n${humanize}`
					: candidateContract;
				context = context
					? `${contractWithVoice}\n\n${context}`
				...
				try {
					...
					if (profileFactsReady(resumeA)) {
						...
						const jdLayerRouted =
							answerPlan.requiredContextLayers.includes('jd') &&
							!answerPlan.forbiddenContextLayers.includes('jd');
						const jdHasContent =
							Boolean(jdA) &&
							(jdA.title ||
								jdA.company ||
								jdA.description_summary ||
								(Array.isArray(jdA.requirements) &&
									jdA.requirements.length > 0) ||
								(Array.isArray(jdA.responsibilities) &&
									jdA.responsibilities.length > 0) ||
								(Array.isArray(jdA.technologies) &&
									jdA.technologies.length > 0) ||
								(Array.isArray(jdA.keywords) && jdA.keywords.length > 0));
						_attr.structured_jd_used = jdLayerRouted && jdHasContent;
						_attr.hybrid_rag_used =
							answerPlan.requiredContextLayers.includes('resume') ||
							answerPlan.requiredContextLayers.includes('jd');
					}
				} catch {
					/* attribution only */
			...
			_attr.hindsight_enabled =
				_hsMemoryOn && isIntelligenceFlagEnabled('hindsightLiveRecall');
			...
			const _hindsightOwnerAllows =
				manualOwnership && !_ownerEnforcementOff
					? manualOwnership.owner === 'mixed' ||
						manualOwnership.owner === 'transcript'
					: true;
			...
			const _contractAllowsHindsight = turnContract
				? turnContract.memoryReadPolicy.allowHindsight
				: true;
			if (
				!isCodingChat &&
				!isContractEnforced &&
				...
				try {
					...
					const ltm = LongTermMemoryService.fromFlags({
						hindsight: { ..._liveHsCfg, timeoutMs: 800 },
					});
					if (ltm.enabled) {
						const t0 = Date.now();
						const memories = await ltm.recallRelevantMemory(
							message,
							{ userId: _HM.getInstance().localUserId() },
							...
							if (turnContract) {
								try {
									const {
										...
										require('./intelligence/context-os') as typeof import('./intelligence/context-os');
									memBlock = renderHindsightRecallBlock(
										toRecalledMemoryEvidence(memories, turnContract),
									);
								} catch {
									memBlock = '';
								}
						...
						try {
							const { intelligenceMetrics } =
								require('./intelligence/IntelligenceMetrics') as typeof import('./intelligence/IntelligenceMetrics');
							intelligenceMetrics.timing('hindsight_recall_ms', recallMs);
							intelligenceMetrics.rate(
								'memory_recall_empty_rate',
								facts.length === 0,
							);
						} catch {
							/* metrics never affect the answer */
						}
						...
						if (isIntelligenceFlagEnabled('trace')) {
							console.log('[HindsightLiveRecall]', {
								ms: recallMs,
								facts: facts.length,
								injected: facts.length > 0,
							});
						}
						iTrace.noteContext({
							...
							trustLevel: 'medium',
							requested: true,
							retrieved: facts.length > 0,
							included: facts.length > 0,
							reason: 'live_backward_recall',
						});
					}
				} catch (recallErr: any) {
					console.warn(
						'[HindsightLiveRecall] skipped (non-fatal):',
						recallErr?.message,
					);
				}
			}
			...
			const docGroundedOrUnknown =
				manualActiveMode == null ||
				manualActiveMode.documentGroundedCustomModeActive === true;
			...
			const ownershipAllowsProfileEvidence =
				(manualOwnership ? manualOwnership.profileAllowed : true) &&
				_contractAllowsProfile;
			...
			const KNOWN_COORDINATOR_KINDS = new Set([
				'reference_files',
				'profile_resume',
				'projects',
				'profile_jd',
			]);
			const coordinatorInScopeKinds =
				Boolean(manualTurnSourceDecision) &&
				manualTurnSourceDecision!.requiredEvidenceKinds.length > 0 &&
				manualTurnSourceDecision!.requiredEvidenceKinds.every((k) =>
					KNOWN_COORDINATOR_KINDS.has(k),
				) &&
				manualTurnSourceDecision!.requiredEvidenceKinds.some(
					(k) =>
						k === 'profile_resume' || k === 'projects' || k === 'profile_jd',
			...
			if (
				...
				try {
					const {
						...
						packGovernsGeneration,
					} =
						require('./intelligence/context-os') as typeof import('./intelligence/context-os');
					const { ModesManager } = require('./services/ModesManager');
					const modesMgr = ModesManager.getInstance();
					const orchestrator = llmHelper.getKnowledgeOrchestrator?.();
					const activeResumeStructured =
						(orchestrator as any)?.activeResume?.structured_data ?? null;
					const activeJdStructured =
						(orchestrator as any)?.activeJD?.structured_data ?? null;
					...
					const retrieveReferenceEvidence = needsReference
						? async () => {
								const { EvidenceResolver } =
									require('./intelligence/context-os/EvidenceResolver') as typeof import('./intelligence/context-os/EvidenceResolver');
								const {
									classifyQuestion,
								} = require('./services/knowledge/QuestionClassifier');
								const {
									queryOkfCards,
								} = require('./services/knowledge/OkfRetriever');
								const {
									KnowledgeManager,
								} = require('./services/knowledge/KnowledgeManager');
								const activeModeRow = modesMgr.getActiveMode?.();
								if (!activeModeRow) {
									const { emptyEvidencePack } =
										require('./intelligence/context-os/evidencePack') as typeof import('./intelligence/context-os/evidencePack');
								...
								const resolver = new EvidenceResolver({
									...
									getReferenceFiles: (modeId: string) =>
										modesMgr.getReferenceFiles(modeId),
									hybridRetriever: {
										retrieveHybrid: (m: any, files: any, opts: any) =>
											modesMgr.retrieveHybridRaw(m, files, opts),
									},
									knowledgeManager: {
										getPackForFile: (fileId: string) =>
											KnowledgeManager.getInstance().getPackForFile(fileId),
									},
									classifyQuestion,
									queryOkfCards,
								});
								const resolution = await resolver.resolve({
									...
									sourceContract: _tc,
									activeMode: {
										modeId: activeModeRow.id,
										modeUniqueId: activeModeRow.id,
									},
									requestedProperty: _tc.requestedProperty,
									transcript: context,
								});
					...
					const coordinatorResult = await Promise.race([
						...
						new Promise<null>((resolve) =>
							setTimeout(() => resolve(null), COORDINATOR_BUDGET_MS),
						),
					]);
					if (!coordinatorResult) {
						throw new Error(
							`TurnEvidenceCoordinator exceeded ${COORDINATOR_BUDGET_MS}ms budget`,
						);
					}
					...
					iTrace.noteContext({
						...
						included:
							coordinatorResult.pack.answerPolicy === 'answer' ||
							coordinatorResult.pack.answerPolicy === 'answer_with_uncertainty',
						reason:
							coordinatorResult.failures.length > 0
					...
					console.warn(
						'[CONTEXT-OS] TurnEvidenceCoordinator skipped (non-fatal):',
						coordinatorErr?.message || coordinatorErr,
					);
				}
			}
			...
			if (
				!isCodingChat &&
				!selectedProfileEvidence &&
				!coordinatorGovernedProfileEvidence &&
				answerPlan.profileContextPolicy !== 'forbidden' &&
				!docGroundedOrUnknown &&
				ownershipAllowsProfileEvidence
			) {
				try {
					const { retrieveProfileEvidence } =
						require('./services/knowledge/OkfProfileRetriever') as typeof import('./services/knowledge/OkfProfileRetriever');
					const profileEvidence = retrieveProfileEvidence({
						...
						documentGroundedActive:
							manualActiveMode?.documentGroundedCustomModeActive === true,
						hasExplicitPlan: true,
					});
					if (profileEvidence.allowed && profileEvidence.block) {
						context = context
							? `${profileEvidence.block}\n\n${context}`
							: profileEvidence.block;
						iTrace.noteContext({
							source: 'okf_profile_cards',
							trustLevel: 'high',
							requested: true,
							retrieved: true,
							included: true,
							reason: 'profile_policy_allowed',
						});
						piTelemetry.emit('pi_okf_profile_evidence_assembled', {
							cardCount: profileEvidence.cardCount,
							profilePolicy: answerPlan.profileContextPolicy,
							answerType: answerPlan.answerType,
						});
					}
					...
					console.warn(
						'[OkfProfile] profile card retrieval skipped (non-fatal):',
						okfErr?.message || okfErr,
					);
				}
			}
			...
			try {
				...
				const ignoreKnowledge =
					isCodingChat || isSafetyAnswer ? true : options?.ignoreKnowledgeMode;
				iTrace
					.lifecycle('evidence_selected', {
						...
						sourceKinds:
							selectedProfileEvidence?.items.map((item) => item.sourceKind) ??
				...
				chatTrace.mark('provider_request_started', {
					ignoreKnowledgeMode: Boolean(ignoreKnowledge),
				});
						...
						...(manualActiveMode?.documentGroundedCustomModeActive === true
							? {
									followUpReferentHint:
										(
											intelligenceManager.getLastAssistantMessage(
												'manual_chat',
											) || ''
										).trim() || undefined,
								}
								...
								? {
										contextOsGeneration: (manualContextOsGeneration = {
											...
											modeSnapshot: {
												...
												modeName: manualActiveMode?.name ?? null,
												sourceAuthority:
													manualSourceContract?.sourceAuthority ??
													'ask_if_ambiguous',
											},
				...
				const { StreamingSpecStripper } =
					require('./llm/codingContract') as typeof import('./llm/codingContract');
				const chatSpecStripper = isCodingChat
					? new StreamingSpecStripper()
					: null;
				const sendChunk = (chunk: string) => {
					const visible = chatSpecStripper
						? chatSpecStripper.push(chunk)
						: chunk;
					if (!visible) return;
				...
				const REFUSAL_SNIFF_RE =
					/not (?:directly )?(?:mentioned|in (?:the|my) (?:uploaded|seminar|thesis|retrieved) (?:material|sections?|document)|found in|present in)|^I could not find\b|^this is not directly mentioned/i;
				const deferFirstPaintEligible =
				...
				const {
					couldBecomeNoActionSentinel: _couldBeNoAction,
					stripLeadingNoActionSentinel: _stripNoAction,
				} =
				...
				const sendChunkGated = (chunk: string) => {
					if (!deferFirstPaint && !sentinelHold) {
						sendChunk(chunk);
						return;
					}
				...
				await raceStreamWithDeadline({
					...
					onFirstUsefulTimeout: () => {
						chatTrace.mark('provider_timeout', { reason: 'first_useful' });
					},
					onStallTimeout: () => {
						chatTrace.mark('provider_timeout', { reason: 'inter_token_stall' });
					},
				...
				if (manualSuperseded) {
					iTrace
						.setCorrelation({ aborted: true, errorCategory: 'superseded' })
						.lifecycle('cancelled', {
							reason: 'superseded',
							finalAction: 'discard',
						});
				...
				if (codingGate) {
					...
					const tail = chatSpecStripper
						? chatSpecStripper.push(gatedTail) + chatSpecStripper.finish()
						: gatedTail;
					if (tail) {
				...
				if (!manualFirstUseful && !fullResponse.trim()) {
					const fb =
						answerPlan.answerType === 'general_meeting_answer' ||
						answerPlan.answerType === 'lecture_answer'
					...
					fullResponse = fb;
					sendChunk(fb);
					chatTrace.mark('fallback_answer_used' as any, {
						answerType: answerPlan.answerType,
						finalGenerationMode,
					});
				}
				...
				const { stripVerificationSpec: _stripSpec } =
					require('./llm/codingContract') as typeof import('./llm/codingContract');
				...
				if (!isCodingChat)
					fullResponse = _stripNoAction(fullResponse) || fullResponse;
				...
				if (isCodingChat) {
					...
					try {
						const looksMeta =
							/<verification_spec>|<answer_contract>|I notice the contract|your message (was|got) cut off|please share (the|your) (problem|prompt)|paste the prompt/i.test(
								fullResponse,
							);
						...
						if (
							...
							piTelemetry.emit('pi_coding_meta_retry_attempted', {
								answerType: answerPlan.answerType,
							});
							console.warn(
								'[IPC] coding answer looks like meta-reply, regenerating once',
								{ snippetLen: fullResponse.length },
							);
							...
							await raceStreamWithDeadline({
								stream: llmHelper.streamChat(
									regenPrompt,
									undefined,
									codingPriorProblemBlock || undefined,
									undefined,
									true,
									true,
									[],
								...
								firstUsefulDeadlineMs: usingLocalLlm
									? LIVE_LOCAL_FIRST_USEFUL_TIMEOUT_MS
									: 8000,
							...
							if (
								...
								if (isCodingChat) fullResponse = _stripSpec(fullResponse);
								piTelemetry.emit('pi_coding_meta_retry_succeeded', {
									answerType: answerPlan.answerType,
								});
							} else {
								...
								piTelemetry.emit('pi_coding_meta_retry_no_accept', {
									answerType: answerPlan.answerType,
									regenLen: regenTrim.length,
								});
							}
						}
					} catch (metaRetryErr: any) {
						console.warn(
							'[IPC] coding meta-reply retry skipped:',
							metaRetryErr?.message,
						);
					}
					...
					try {
						...
						if (
							...
							piTelemetry.emit('pi_context_policy_applied', {
								answerType: answerPlan.answerType,
								via: 'code_truncation_detected',
								markerCount: completeness.issues.length,
							});
							console.warn(
								'[IPC] code-only answer looks truncated, regenerating once',
								{ issues: completeness.issues.map((i) => i.code) },
							);
							const regenContract = explicitCodingContract
								? buildCodingContractPrompt(explicitCodingContract)
								: buildCodingContractPrompt(null);
							...
							if (
								regenTrim.length >= 20 &&
								checkCodeCompleteness(regenTrim).ok
							) {
								fullResponse = regenTrim;
								finalText = regenTrim;
								piTelemetry.emit('pi_context_policy_applied', {
									answerType: answerPlan.answerType,
									via: 'code_regenerated_complete',
								});
							}
						}
					} catch (completenessErr: any) {
						console.warn(
							'[IPC] code completeness check skipped:',
					...
					try {
						...
						if (!profileValidation.ok) {
							console.warn(
								'[ProfileIntelligence] profile evidence violations',
								{
									answerType: answerPlan.answerType,
						...
						const _docGroundedBlocksRepair =
							manualActiveMode?.documentGroundedCustomModeActive;
						const critical =
							profileAvailable &&
							answerPlan.profileContextPolicy === 'required' &&
							...
							profileValidation.violations.find(
								(v) => v.severity === 'error' && CRITICAL_CODES.has(v.code),
							);
						if (
							_docGroundedBlocksRepair &&
							profileAvailable &&
							answerPlan.profileContextPolicy === 'required' &&
							isIntelligenceFlagEnabled('trace')
						) {
							console.log(
								'[SOURCE-GUARD] blocked source=profile_resume reason=document_grounded_contract',
								{
									answerType: answerPlan.answerType,
									modeId: manualActiveMode?.id,
						...
						if (
							...
							try {
								...
								try {
									facts =
										(await orch2?.processQuestion?.(message))?.contextBlock ||
										'';
								} catch {
									/* best effort */
								}
								...
								const repairInstruction =
									critical.code === 'unsupported_metric'
										? profileValidation.repairInstruction ||
											buildProfileRepairInstruction({
												ok: false,
												violations: [critical],
											} as any)
								...
								if (repairedTrim.length >= 5) {
									...
									const stillCritical = reCheck.violations.some(
										(v) => v.severity === 'error' && CRITICAL_CODES.has(v.code),
									);
									...
									if (verdict.accepted) {
										...
										console.warn(
											'[ProfileIntelligence] manual profile repair applied',
											{ code: critical.code },
										);
									} else {
										console.warn(
											'[ProfileIntelligence] manual profile repair REJECTED',
											{ code: critical.code, reason: verdict.reason },
										);
									}
								}
							} catch (repairErr: any) {
								console.warn(
									'[ProfileIntelligence] manual profile repair failed (non-fatal):',
						...
						console.warn(
							'[ProfileIntelligence] profile output validation failed (non-fatal):',
				...
				if (answerPlan.profileContextPolicy === 'forbidden') {
					try {
						...
						const resumeC =
							(orchC as any)?.activeResume?.structured_data ?? null;
						const profileTokens = resumeC
							? {
									firstName:
										(resumeC.identity?.name || resumeC.name || '')
											.trim()
											.split(/\s+/)[0] || undefined,
									projects: (resumeC.projects || [])
										.map((p: any) => (p?.name || '').split(/[–—-]/)[0].trim())
										.filter((s: string) => s.length >= 3),
									companies: (resumeC.experience || [])
										.map((e: any) => (e?.company || '').trim())
										.filter((s: string) => s.length >= 3),
								}
							: undefined;
						const profileExplicitlyInvited =
							/\b(use|using|with|in|from)\s+(my|your|the)\s+(natively|project|portfolio)\b|\bin natively\b|\b(my|your) natively project\b/i.test(
								message,
							);
						const codeLeak = validateProfileOutput({
							answer: fullResponse,
							plan: answerPlan,
							profileAvailable: Boolean(resumeC),
							candidateDirected: false,
							profileTokens,
							profileExplicitlyInvited,
						}).violations.find(
							(v) => v.code === 'profile_token_in_coding_answer',
						);
						if (codeLeak) {
							const tokens = [
								profileTokens?.firstName,
								...(profileTokens?.projects || []),
								...(profileTokens?.companies || []),
							].filter((t): t is string => !!t);
							const stripped = stripProfileTokensFromCoding(
								fullResponse,
								tokens,
							);
							...
							const stillLeaks = reCheck.violations.some(
								(v) => v.code === 'profile_token_in_coding_answer',
							);
							if (!stillLeaks && stripped.trim().length >= 20) {
								...
								finalText = stripped;
								console.warn(
									'[ProfileIntelligence] stripped stray profile token from a profile-forbidden answer',
									{ answerType: answerPlan.answerType },
								);
							}
						}
						...
						console.warn(
							'[ProfileIntelligence] forbidden-answer leak validation skipped:',
							codeLeakErr?.message,
						);
					}
				}
				...
				try {
					if (isIntelligenceFlagEnabled('profileTreeV2')) {
						const guard = ProfileTreeService.getCandidatePerspectiveGuard(
							manualActiveMode?.templateType,
							message,
						);
					...
					/* guard never blocks the answer */
				}
				if (
					CANDIDATE_VOICE_ANSWER_TYPES.has(answerPlan.answerType) ||
					_perspectiveExpectsCandidate
				) {
					try {
						...
						if (isNonCandidateContent) {
							piTelemetry.emit('pi_coding_skip_candidate_sanitizer', {
								answerType: answerPlan.answerType,
							});
						} else {
							...
							if (sani.repaired && !sani.needsFallback) {
								...
								piTelemetry.emit('pi_candidate_sanitizer_applied', {
									answerType: answerPlan.answerType,
									repaired: true,
									needsFallback: false,
									markerCount: sani.removedMarkers.length,
								});
								console.warn(
									'[ProfileIntelligence] sanitized assistant-meta tail from candidate answer',
									{
										answerType: answerPlan.answerType,
										markers: sani.removedMarkers,
									},
								);
								...
								if (manualActiveMode?.documentGroundedCustomModeActive) {
									console.warn(
										'[SourceGuard] blocked profile fallback reason=document_grounded_contract',
										{ answerType: answerPlan.answerType },
									);
								} else {
									...
									sessionWriteDecision = decideSessionWritePolicy({
										...
										validationOk: false,
										criticalViolations: [
											'assistant_identity_misfire_no_jit_answer',
										],
										sourceContractHonored: false,
									});
									console.warn(
										'[ProfileIntelligence] candidate answer was all assistant-meta; deterministic profile fallback blocked',
										{ answerType: answerPlan.answerType },
									);
								}
							}
							...
							if (
								...
								/^\s*(?:I(?:'m| am) Natively[.,]?\s*(?:an? AI assistant[.,]?\s*)?)?I\s+(?:cannot|can\s?not|can'?t)\s+share\s+that(?:\s+information)?\s*\.?\s*$/i.test(
									fullResponse.trim(),
								)
							) {
								const honest =
								...
								piTelemetry.emit('pi_context_policy_applied', {
									answerType: answerPlan.answerType,
									via: 'product_about_refusal_repaired',
								});
								console.warn(
									'[ProfileIntelligence] product-about stock refusal replaced with honest no-context line',
									{ answerType: answerPlan.answerType },
								);
							}
						...
						console.warn(
							'[ProfileIntelligence] candidate sanitizer skipped:',
							saniErr?.message,
						);
					}
				}
				...
				if (
					!isCodingChat &&
					ASSISTANT_VOICE_ANSWER_TYPES.has(answerPlan.answerType)
				) {
					try {
						...
						if (misfire.isMisfire) {
							...
							piTelemetry.emit('pi_assistant_voice_misfire_repaired', {
								answerType: answerPlan.answerType,
								reason: misfire.reason,
							});
							console.warn(
								'[ProfileIntelligence] assistant-voice identity/refusal misfire replaced with honest line',
								{ answerType: answerPlan.answerType, reason: misfire.reason },
							);
							fullResponse = honest;
							finalText = honest;
						}
						...
						console.warn(
							'[ProfileIntelligence] assistant-voice guard skipped:',
							avErr?.message,
						);
					}
				}
				...
				try {
					if (humanizeDirectiveFor(answerPlan.answerType)) {
						...
						if (filler.hasFiller) {
							console.warn(
								'[HumanLikeness] corporate filler detected in candidate answer',
								{
									answerType: answerPlan.answerType,
									count: filler.count,
									phrases: filler.matches.slice(0, 5),
								},
							);
							piTelemetry.emit('pi_context_policy_applied', {
								answerType: answerPlan.answerType,
								via: 'corporate_filler_detected',
								markerCount: filler.count,
							});
						}
					}
				} catch {
					/* detection never affects the answer */
				}
				...
				if (!isCodingChat) {
					try {
						...
						const humanized = humanizeForAnswerType(
							answerPlan.answerType,
							fullResponse,
						);
						...
						if (answerPlan.answerType === 'technical_concept_answer') {
							const simpleRequested =
								...
								/\b(simple|simply|beginner|eli5|like i'?m (?:5|five)|layman)\b/i.test(
									message,
								);
							...
							const tech = compressTechnicalConcept(
								fullResponse,
								simpleRequested,
							);
							if (tech.changed && tech.text.trim().length >= 20) {
								...
								piTelemetry.emit('pi_context_policy_applied', {
									answerType: answerPlan.answerType,
									via: 'technical_concept_flattened',
								});
							}
						}
						...
						const budget = applySpeakabilityBudget(
							fullResponse,
							answerPlan.answerType,
						...
						piTelemetry.emit('pi_context_policy_applied', {
							answerType: answerPlan.answerType,
							via: 'speakability_measured',
							speakabilityClass: budget.speakability_class,
							markerCount: budget.spoken_word_count,
						});
						...
						const structureRequested = [
							...
							'exam',
							'notes',
						].includes(answerPlan.answerStyle as string);
						if (hasVisibleScaffold && !structureRequested) {
							...
							if (speakable.length >= 40) {
								...
								finalText = speakable;
								piTelemetry.emit('pi_scaffold_compressed', {
									answerType: answerPlan.answerType,
								});
							}
						}
						...
						try {
							...
							const resumeD =
								(orchD as any)?.activeResume?.structured_data ?? null;
						...
						const verdict = _manualDiversityGuard.check(
							fullResponse,
							answerPlan.answerType,
							message,
							{ availableProjects },
						...
						if (verdict.repeated) {
							piTelemetry.emit('pi_answer_repeated', {
								answerType: answerPlan.answerType,
							...
							if (
								verdict.reason === 'same_opening_window' ||
								verdict.reason === 'same_first_sentence'
							) {
								const varied = varySpokenOpening(
									fullResponse,
									_manualDiversityGuard.size,
								);
								if (
									varied !== fullResponse &&
									!_manualDiversityGuard.check(
										varied,
										answerPlan.answerType,
										message,
										{ availableProjects },
							...
							if (repaired !== fullResponse) {
								...
								piTelemetry.emit('pi_context_policy_applied', {
									answerType: answerPlan.answerType,
									via: 'repetition_guard_repaired',
								});
							}
						}
						_manualDiversityGuard.record(
							fullResponse,
							answerPlan.answerType,
							message,
							{ availableProjects },
						...
						console.warn(
							'[ProfileIntelligence] answer polish skipped:',
							polishErr?.message,
						);
					}
				}
				...
				if (
					...
					console.log('[SOURCE-GUARD] doc-grounded-validator-gate', {
						...
						reason: _docGateFires
							? 'post_stream_validator_required'
							: 'answer_type_not_doc_grounded',
					});
				}
				...
				if (
					...
					try {
						const GREETING_RE =
							/^\s*(?:hey|hi|hello)[!,.]?\s*(?:there)?[!,.]?\s*(?:what would you like help with|how can i help|what can i (?:help|do)(?: you with| for you)?|how may i (?:help|assist))\b/i;
						...
						const priorAnswer = (
							intelligenceManager.getLastAssistantMessage('manual_chat') || ''
						).trim();
						const isGreeting =
							GREETING_RE.test(trimmed) ||
							/what would you like help with/i.test(trimmed);
						const isEmpty = trimmed.length < 8;
						const isExactRepeat =
							priorAnswer.length > 0 && trimmed === priorAnswer;
						...
						const _capturedRawBlock =
							manualContextOsGeneration?.retrievedBlockRaw;
						if (_governedPack && _governedPack.items.length > 0) {
							docContextBlock = _governedPack.items
								.map(
									(it) =>
										`[Section: ${it.pointer?.section || it.sourceId}]\n${it.text}`,
								)
								.join('\n\n');
							...
							try {
								...
								try {
									const {
										expandQueryWithHints,
									} = require('./llm/documentGroundedPrompt');
									_valRetrievalQuery = expandQueryWithHints(
										String(message || ''),
									);
								} catch {
									_valRetrievalQuery = message;
								}
								docContextBlock =
									ModesManager.getInstance().buildRetrievedActiveModeContextBlock(
										_valRetrievalQuery,
										undefined,
										DOC_GROUNDED_TOKEN_BUDGET,
										'lecture_answer',
										true,
										undefined,
								...
								console.warn(
									'[DocGrounded] re-retrieval for validator failed (non-fatal):',
									reErr?.message,
								);
							}
						}
							...
							try {
								if (isIntelligenceFlagEnabled('okfHybridRetrieval')) {
									...
									if (activeMode) {
										...
										const {
											assembleEvidence,
										} = require('./services/knowledge/EvidenceAssembler');
										const referenceFiles =
											_MM.getInstance().getReferenceFiles?.(activeMode.id) ||
											[];
										...
										for (const file of referenceFiles) {
											...
											const scored = queryOkfCards(
												pack,
												message,
												classification,
												{ topN: 6, fileId: file.id },
											);
											for (const { card } of scored)
												cardTexts.push(`${card.title}\n${card.body}`);
								...
								console.warn(
									'[DocGrounded] OKF card augmentation for validator skipped (non-fatal):',
									okfRetryErr?.message,
								);
							}
						...
						try {
							if (isIntelligenceFlagEnabled('okfHybridRetrieval')) {
								...
								if (activeMode2) {
									const {
										KnowledgeManager: _KM2,
									} = require('./services/knowledge/KnowledgeManager');
									const targetEntities =
										classifyQuestion(message).targetEntities || [];
									...
									const addName = (raw: string) => {
										...
										for (const w of name.split(/[^a-z0-9]+/)) {
											if (w.length >= 5 && !GATE_GENERIC_TOKENS.has(w))
												packNameTokens.add(w);
										}
									};
									for (const file of _MM2
										.getInstance()
										.getReferenceFiles?.(activeMode2.id) || []) {
									...
									highSignalEntities = targetEntities.filter((e: string) =>
										packWholeNames.has(e.toLowerCase()),
									);
								}
							}
						} catch {
							/* best effort — falls back to empty sets, same as OKF-off behavior */
						}
						let isFalseRefusal = false;
						const falseRefusalRepairEnabled = isIntelligenceFlagEnabled(
						...
						try {
							...
							if (
								saysNotMentioned &&
								docContextBlock &&
								falseRefusalRepairEnabled
							) {
								const qTerms: string[] = (
									message.match(/\b[A-Za-z0-9][A-Za-z0-9-]*[A-Za-z0-9]\b/g) ||
									[]
								)
								...
								const wholeNameHit =
									present.some((t: string) =>
										packWholeNames.has(t.toLowerCase()),
									) ||
									highSignalEntities.some((e) =>
										packWholeNames.has(e.toLowerCase()),
									);
								const tokenHits = new Set(
									present
										.filter((t: string) => packNameTokens.has(t.toLowerCase()))
								...
								const governedRefusal =
									manualContextOsGeneration?.govern === true &&
									manualContextOsGeneration?.evidencePack?.answerPolicy ===
								...
								if (shouldRepair) {
									isFalseRefusal = true;
									console.warn(
										'[DocGrounded] false-refusal detected — question matches a real document topic, triggering regen',
										{
									...
									console.warn(
										'[DocGrounded] "not mentioned" left as honest refusal (no whole-name hit, <2 title tokens)',
										{
											questionTerms: present.slice(0, 8),
											wholeNameHit,
											tokenHits: [...tokenHits].slice(0, 8),
							...
							/* never throws into the answer path */
						}
						...
						try {
							...
							const answerIsRefusalLike =
								isFalseRefusal ||
								(isSystemOwnRefusalPhrase && trimmed.length < 120) ||
								(trimmed.length < 120 &&
									/^(?:\s*I could not find|.*\bnot (?:directly )?(?:mentioned|found|present)\b)/i.test(
										trimmed,
									) &&
									!/\d[\d,]*(?:\.\d+)?\s?(?:gb|mb|hz|kg|mm|%|dof|steps?|episodes?)/i.test(
										trimmed,
									));
						...
						try {
							if (
								...
								const answerIsRefusal =
									isAssistantRefusal(trimmed) ||
									/^\s*(?:there is |there's )?no (?:information|mention|data)\b/i.test(
										trimmed,
									);
								if (!answerIsRefusal) {
									const {
										textCanProveProperty,
									...
									if (
										!textCanProveProperty(
											docContextBlock,
											turnContract.requestedProperty,
										)
									) {
										propertyUnsupported = true;
										propertyRefusalLine = buildInsufficientPropertyAnswer({
											property: turnContract.requestedProperty,
										});
										piTelemetry.emit('pi_doc_grounded_validation_failed', {
						...
						if (
							...
							try {
								const {
									detectUnsupportedDocumentAnswer: _detectUnsupported,
								} = require('./llm/documentGroundedPrompt');
								const unsupportedCheck = _detectUnsupported({
									answer: trimmed,
									retrievedBlock: docContextBlock,
								});
								if (
									unsupportedCheck.unsupported &&
									unsupportedCheck.reason === 'unsupported_named_entity'
								) {
								...
								entityUnsupported = false;
							}
						}

						const reason = propertyUnsupported
							? null // handled above — no regen can conjure missing evidence
							: isGreeting
								? 'greeting'
								: isEmpty
									? 'empty'
									: isExactRepeat
										? 'exact_repeat_of_prior_answer'
										: isFalseRefusal
											? 'false_refusal'
											: entityUnsupported
						...
						if (reason) {
							console.warn('[DocGrounded] answer validation failed', {
								reason,
								answerType: answerPlan.answerType,
							});
							...
							try {
								...
								try {
									...
									regenSystemPrompt = appendCustomModeSystemPromptLayer({
										...
										modePromptSuffix: _regenBaseIsV2
											? undefined
											: _mm.getActiveModeSystemPromptSuffix?.(
													manualActiveMode?.id ?? undefined,
												),
										pinnedInstructions: _mm.getActiveModePinnedInstructions?.(
											answerPlan.answerType,
											manualActiveMode?.id ?? undefined,
										),
										isActiveCustomMode:
											manualActiveMode?.isCustom === true ||
											_mm.isCustomMode?.(manualActiveMode),
									});
								} catch {
									regenSystemPrompt = undefined;
								}
								await raceStreamWithDeadline({
								...
								console.warn(
									'[DocGrounded] regeneration failed (non-fatal):',
							...
							const regenIsStillRefusing =
								(reason === 'false_refusal' || reason === 'incomplete') &&
								(isAssistantRefusal(regenTrim) ||
									/^\s*(?:the (?:material|document|excerpts?) )?do(?:es)? not (?:specify|mention|state|provide)\b/i.test(
										regenTrim,
									));
							...
							if (reason === 'incomplete' && regenTrim) {
								try {
									incompleteRegenFabricates = completenessRegenFabricates(
										regenTrim,
										docContextBlock,
									);
									...
									// original's.
									const {
										extractNumericUnitTokens: _ext,
										detectIncompleteSubQuestionAnswer: _subQ,
									} = require('./llm/documentGroundedPrompt');
									const regenVals: Set<string> = _ext(regenTrim);
									const originalVals: Set<string> = _ext(trimmed);
									const recoveredAnyMissingValue =
										incompleteMissing.length === 0 ||
									...
									const regenSubQCheck =
										incompleteSubQuestions.length > 0
											...
											: { missing: [] };
									const recoveredAnySubQuestion =
										incompleteSubQuestions.length === 0 ||
										(regenSubQCheck.missing as string[]).length <
											incompleteSubQuestions.length;
									const noNumericCoverageRegression =
										regenVals.size >= originalVals.size;
									incompleteRecoveredValue =
										recoveredAnyMissingValue &&
										recoveredAnySubQuestion &&
										noNumericCoverageRegression;
								} catch {
									incompleteRegenFabricates = false;
									incompleteRecoveredValue = false;
								}
							}
							...
							if (
								...
								try {
									...
									if (isIntelligenceFlagEnabled('trace')) {
										console.warn(
											'[SOURCE-GUARD] regen-contract check skipped (non-fatal):',
											regenContractErr?.message,
										);
									}
								}
							}
							...
							if (regenValid) {
								...
								console.warn('[DocGrounded] regeneration applied', {
									reason,
									chars: regenTrim.length,
								});
								...
								console.warn(
									'[DocGrounded] completeness re-ask did not cleanly improve — keeping original answer',
									{ missing: incompleteMissing.slice(0, 6) },
								);
								...
								console.warn(
									'[DocGrounded] false-refusal regen did not cleanly improve on a substantial original — keeping original answer',
								...
								console.warn(
									'[DocGrounded] regeneration did not recover — shipping safe failure line, blocked from SessionTracker',
									{ reason },
								);
							}
						}
						...
						console.warn(
							'[DocGrounded] validator skipped (non-fatal):',
							dgErr?.message || dgErr,
						);
					}
				}
				...
				try {
					const { shouldSuppressModelOutput: _isNoActionOut } =
						require('./llm/promptSystemV2') as typeof import('./llm/promptSystemV2');
					if (
						_isNoActionOut(rawResponseForVerify) ||
						_isNoActionOut(fullResponse)
					) {
						const fb =
							"I don't have enough context from the conversation to answer that yet.";
						fullResponse = fb;
						...
						chatTrace.mark('fallback_answer_used' as any, {
							answerType: answerPlan.answerType,
							finalGenerationMode,
							reason: 'no_action_sentinel',
						});
					}
				} catch {
					/* non-fatal — sentinel guard must never break chat */
				}
				...
				try {
					const {
						ModesManager: _MmDoneEmitGuard,
					} = require('./services/ModesManager');
					liveModeIdAtDoneEmit =
						_MmDoneEmitGuard.getInstance().getActiveMode()?.id ?? null;
				} catch {
					/* fail open — emit-guard only */
				}
				...
				if (liveModeIdAtDoneEmit !== (manualActiveMode?.id ?? null)) {
					...
					chatTrace.mark('response_completed', {
						...
						repaired: Boolean(finalText),
					});
					chatTrace.finish({ chars: fullResponse.length });
					iTrace
						.setProvider({ provider: 'llm', model: undefined })
						.lifecycle('completed', {
							answerType: answerPlan.answerType,
							finalAction: 'answer',
					...
					if (
						...
						if (
							turnContract &&
							isIntelligenceFlagEnabled('contextOsMemorySafetyEnabled')
						) {
							try {
								const { buildAssistantClaims, parseModeSnippets } =
									require('./intelligence/context-os') as typeof import('./intelligence/context-os');
								...
								const verifyPack: import('./intelligence/context-os').EvidencePack =
									(manualContextOsGeneration?.govern
										? manualContextOsGeneration?.evidencePack
										: null) ??
									((): import('./intelligence/context-os').EvidencePack => {
										const evItems = (() => {
										...
										return {
											...
											rejected: [],
											coverage: {
												hasDirectEvidence: evItems.length > 0,
												propertySatisfied: false,
												entityMatched: evItems.length > 0,
												sourceOwnerSatisfied: true,
												confidence: evItems.length ? 0.6 : 0,
											},
											conflicts: [],
								...
								dbm.saveTurnContextContract({
									...
									requestedProperty: turnContract.requestedProperty,
									allowedSources: turnContract.allowedSources.map(
										(c) => c.sourceKind,
									),
								...
								if (isIntelligenceFlagEnabled('trace')) {
									console.warn(
										'[CONTEXT-OS] claim persistence skipped (non-fatal):',
										claimErr?.message,
									);
								}
							}
						...
						if (isIntelligenceFlagEnabled('conversationMemoryV2')) {
							try {
								...
								const {
									ModesManager: _ModesManagerForRecordGuard,
								} = require('./services/ModesManager');
								const liveModeIdAtRecord =
									_ModesManagerForRecordGuard.getInstance().getActiveMode()
										?.id ?? null;
								if (liveModeIdAtRecord === (manualActiveMode?.id ?? null)) {
									_manualConversationMemory.record({
										sessionId: String(senderId),
								...
								/* memory recording never affects the answer */
							}
						}
					}
					...
					_emitAttr({
						assistant_voice_guard_triggered:
							Boolean(finalText) && _attr.assistant_voice_guard_triggered,
					});
					...
					if (
						...
						void (async () => {
							try {
								const { verifyCodingAnswer } =
									await import('./llm/codeVerification/verifyCodingAnswer');
								const { stripVerificationSpec } =
									await import('./llm/codingContract');
								...
								console.warn(
									'[IPC] chat coding verification skipped (non-fatal):',
									verifyErr?.message,
								);
							}
						})();
				...
				try {
					...
					piTelemetry.emit('pi_provider_error_classified', {
						kind: klass.kind,
						outage: klass.isOutage,
					...
					if (
						...
						_emitAttr({
							...
							profile_tree_fast_path_used: false,
							structured_resume_used: false,
						});
						return null;
					}
				} catch (classifyErr: any) {
					console.warn(
						'[IPC] provider-error classify/fallback skipped:',
						classifyErr?.message,
					);
				}
				if (_chatStreamsBySender.get(senderId)?.streamId === myStreamId) {
			...
			try {
				iTrace
					.setCorrelation({ errorCategory: error?.name || 'handler_error' })
					.noteError(error?.name || 'handler_error')
					.lifecycle('failed', {
						errorCategory: error?.name || 'handler_error',
						finalAction: 'retry',
					});
				commitTrace(iTrace);
			} catch {
				/* trace must never mask the real error */
			}
			throw error;
		} finally {
	...
	safeHandle(
		...
		async (_, retention: 'forever' | '7d' | '30d' | 'never') => {
	...
	safeHandle(
		...
		async (_, scopes: Record<string, boolean>) => {
			...
			const merged = mergeProviderDataScopes(
				settings.get('providerDataScopes'),
				scopes,
			);
	...
	safeHandle('intelligence-flags:get', async () => {
		try {
			const {
				...
				require('./intelligence/intelligenceFlags') as typeof import('./intelligence/intelligenceFlags');
			return intelligenceFlagKeys().map((key) => {
				const meta = intelligenceFlagMeta(key);
				return {
					key,
					enabled: isIntelligenceFlagEnabled(key),
	...
	safeHandle(
		...
		async (_, { key, value }: { key: string; value: boolean | null }) => {
			try {
				const {
					setIntelligenceFlag,
					isIntelligenceFlagEnabled,
				...
				if (
					typeof key !== 'string' ||
					!intelligenceFlagKeys().includes(key as any)
				)
					return { success: false, error: 'unknown_flag' };
				if (value !== null && typeof value !== 'boolean')
					return { success: false, error: 'invalid_value' };
				const ok = setIntelligenceFlag(
					key as any,
	...
	safeHandle('hindsight-config:get', async () => {
		try {
			...
			const available = cfg
				? (await hm.healthCheck()) || hm.isAvailable()
				: false;
		...
		async (
			...
			try {
				...
				if (typeof cfg?.baseUrl === 'string')
					sm.set('hindsightBaseUrl', cfg.baseUrl.trim());
				...
				if (typeof cfg?.apiKey === 'string' && cfg.apiKey.trim())
					sm.set('hindsightApiKey', cfg.apiKey.trim());
				if (typeof cfg?.autoStart === 'boolean')
					sm.set('hindsightAutoStart', cfg.autoStart);
				if (typeof cfg?.serverCommand === 'string')
					sm.set('hindsightServerCommand', cfg.serverCommand.trim());
				if (typeof cfg?.llmProvider === 'string')
					sm.set('hindsightLlmProvider', cfg.llmProvider.trim());
				...
				if (sm.get('hindsightExplicitlyDisabled') === true)
					sm.set('hindsightExplicitlyDisabled', false);
				...
				try {
					...
					console.warn(
						'[HindsightConfig] post-save start() failed (non-fatal):',
						e?.message,
					);
				}
		...
		try {
			...
			if (hm.isAuthFailed?.()) {
				try {
					...
					BrowserWindow.getAllWindows().forEach((win) => {
						if (!win.isDestroyed()) {
							win.webContents.send('hindsight-status', {
								state: 'auth-failed',
								reason:
									'Cloud key rejected (401/403) — check your Hindsight Cloud account key',
								at: Date.now(),
							});
						}
					});
	...
	safeHandle('open-hindsight-log', async () => {
		try {
			const { HindsightManager } =
				require('./services/HindsightManager') as typeof import('./services/HindsightManager');
			const logPath =
				HindsightManager.getInstance().getServerLogPath?.() ?? null;
			...
			if (!fs.existsSync(logPath)) {
				try {
					...
					/* read-only fs — openPath will surface */
				}
			}
	...
	safeHandle('context-debug:get-config', async () => {
		try {
			const {
				describeContextDebugConfig,
			} = require('./context-intelligence/debug/debug-config');
			const {
				getContextDebugLogDirectory,
				getContextDebugWriter,
			} = require('./context-intelligence/debug/jsonl-writer');
			...
			return {
				...
				currentFile:
					cfg.level !== 'off'
						? (getContextDebugWriter()?.getCurrentFilePath() ?? null)
						: null,
			};
		} catch (e) {
			return {
	...
	safeHandle('context-debug:export', async () => {
		try {
			...
			if (!fs.existsSync(file))
				return {
					ok: false,
					error: 'no debug records written this session yet',
				};
	...
	safeHandle('get-local-fallback-preflight', async () => {
		const {
			getLatestLocalFallbackPreflight,
		} = require('./services/LocalFallbackPreflight');
	...
	safeHandle('run-local-fallback-preflight', async () => {
		const llmHelper = appState.processingHelper.getLLMHelper();
		const {
			runLocalFallbackPreflight,
		} = require('./services/LocalFallbackPreflight');
		return runLocalFallbackPreflight({
			ollamaSelected: llmHelper.isUsingOllama?.() === true,
		});
	});
	...
	let localFallbackCache: {
		at: number;
		value: { text: boolean; vision: boolean };
	} | null = null;
	let localFallbackInFlight: Promise<{
		text: boolean;
		vision: boolean;
	}> | null = null;
	...
	safeHandle('get-local-fallback-status', async () => {
		...
		if (
			localFallbackCache &&
			now - localFallbackCache.at < LOCAL_FALLBACK_TTL_MS
		) {
	...
	safeHandle('switch-to-ollama', async (_, model?: string, url?: string) => {
		try {
			...
			if (
				status.health === 'missing_optional_dependency' ||
	...
	safeHandle('ensure-ollama-running', async () => {
		try {
			...
			return {
				success: status.health === 'ready' || status.health === 'degraded',
				status,
			};
		} catch (error: any) {
	...
	safeHandle('set-gemini-api-key', async (_, apiKey: string) => {
		try {
					...
					ragManager.initializeEmbeddings({
						...
						providerDataScopes: (() => {
							try {
								const {
									SettingsManager,
								} = require('./services/SettingsManager');
								return SettingsManager.getInstance().get('providerDataScopes');
							} catch {
								return undefined;
							}
			...
			if (keyChanged) {
				try {
					require('./services/HindsightManager')
						.HindsightManager.getInstance()
						.notifyHindsightOfKeyChange('Gemini');
				} catch {
					/* optional */
				}
		...
		try {
			...
			if (keyChanged) {
				try {
					require('./services/HindsightManager')
						.HindsightManager.getInstance()
						.notifyHindsightOfKeyChange('Groq');
				} catch {
					/* optional */
				}
				await refreshRuntimeDefaultIfUnavailable();
		...
		try {
			...
			if (keyChanged) {
				try {
					require('./services/HindsightManager')
						.HindsightManager.getInstance()
						.notifyHindsightOfKeyChange('OpenAI');
				} catch {
					/* optional */
				}
				await refreshRuntimeDefaultIfUnavailable();
		...
		try {
			...
			if (keyChanged) {
				try {
					require('./services/HindsightManager')
						.HindsightManager.getInstance()
						.notifyHindsightOfKeyChange('Claude');
				} catch {
					/* optional */
				}
				await refreshRuntimeDefaultIfUnavailable();
		...
		try {
			...
			if (keyChanged) {
				try {
					require('./services/HindsightManager')
						.HindsightManager.getInstance()
						.notifyHindsightOfKeyChange('DeepSeek');
				} catch {
					/* optional */
				}
				await refreshRuntimeDefaultIfUnavailable();
	...
	safeHandle(
		...
		async (
			_,
			config: { apiKey: string; baseURL: string; maxTokens?: number },
		) => {
			try {
				const { CredentialsManager } = require('./services/CredentialsManager');
				const cm = CredentialsManager.getInstance();
				...
				if (changed) {
					try {
						require('./services/HindsightManager')
							.HindsightManager.getInstance()
							.notifyHindsightOfKeyChange('LiteLLM');
					} catch {
						/* optional */
					}
					await refreshRuntimeDefaultIfUnavailable();
	...
	const discoverLitellmModels = async (
		timeoutMs: number,
	): Promise<string[]> => {
		const { CredentialsManager } = require('./services/CredentialsManager');
		const cm = CredentialsManager.getInstance();
	...
	safeHandle('set-disabled-providers', async (_, providers: string[]) => {
		try {
			const { CredentialsManager } = require('./services/CredentialsManager');
			CredentialsManager.getInstance().setDisabledProviders(
	...
	safeHandle(
		'set-cloud-enabled-models',
		async (_, provider: string, models: string[]) => {
			try {
				const { CredentialsManager } = require('./services/CredentialsManager');
				CredentialsManager.getInstance().setCloudEnabledModels(
					provider,
		...
		try {
			...
			if (apiKey) {
				try {
					...
					const result =
						await LicenseManager.getInstance().activateWithApiKey(apiKey);
					if (result.success) {
						console.log(
							'[IPC] set-natively-api-key: Pro auto-activated via API plan.',
						);
						...
						console.log(
							'[IPC] set-natively-api-key: Pro not activated —',
							result.error,
						);
					}
				} catch (e: any) {
		...
		async (_event, opts?: { force?: boolean }) => {
			...
			try {
				...
				if (
					!opts?.force &&
					cached &&
					Date.now() - cached.ts < USAGE_CACHE_TTL_MS
				) {
					return cached.data;
				}
	...
	safeHandle('trial:get-local', async () => {
		try {
			...
			const token = cm.getTrialToken();
			if (!token)
				return { hasToken: false, trialClaimed: cm.getTrialClaimed() };
			return {
				hasToken: true,
				trialClaimed: true,
	...
	safeHandle('review:dismiss-forever', async () => {
		try {
			...
			const hwid = await getReviewHardwareId();
			svc
				.reportEvent(apiKey, hwid, { type: 'dont_show_again' })
				.catch(() => {});
			return { ok: true };
		} catch (error: any) {
			return { ok: false, error: error?.message || 'unknown' };
			...
			try {
				...
				const name =
					typeof payload?.name === 'string'
						? payload.name.replace(/[<>]/g, '').trim().slice(0, 80)
						: null;
				const role =
					typeof payload?.role === 'string'
						? payload.role.replace(/[<>]/g, '').trim().slice(0, 80)
						: null;
				const company =
					typeof payload?.company === 'string'
						? payload.company.replace(/[<>]/g, '').trim().slice(0, 80)
						: null;
	...
	safeHandle('trial:end-byok', async () => {
		try {
          ...
          `);
				...
				console.warn(
					'[IPC] trial:end-byok: SQLite wipe partial error:',
					dbErr.message,
				);
			}
			...
			try {
				const { ProfilePackBuilder } =
					require('./services/knowledge/ProfilePackBuilder') as typeof import('./services/knowledge/ProfilePackBuilder');
				ProfilePackBuilder.getInstance().deleteAllProfilePacks();
			} catch (piiErr: any) {
				console.warn(
					'[IPC] trial:end-byok: profile OKF pack wipe failed:',
					piiErr?.message || piiErr,
				);
			}
	...
	safeHandle('trial:wipe-profile-data', async () => {
		try {
          ...
          `);
				...
				console.warn(
					'[IPC] trial:wipe-profile-data: SQLite wipe partial error:',
					dbErr.message,
				);
			}
			...
			try {
				...
				console.warn(
					'[IPC] trial:wipe-profile-data: profile OKF pack wipe failed:',
					piiErr?.message || piiErr,
				);
			}
	...
	const validateCurlProviderPayload = (
		provider: unknown,
		...
		try {
			...
			return {
				...
				sttGroqKey: creds.groqSttApiKey
					? `sk-...${creds.groqSttApiKey.slice(-4)}`
					: '',
				sttOpenaiKey: creds.openAiSttApiKey
					? `sk-...${creds.openAiSttApiKey.slice(-4)}`
					: '',
				sttDeepgramKey: creds.deepgramApiKey
					? `sk-...${creds.deepgramApiKey.slice(-4)}`
					: '',
				sttElevenLabsKey: creds.elevenLabsApiKey
					? `sk-...${creds.elevenLabsApiKey.slice(-4)}`
					: '',
				sttAzureKey: creds.azureApiKey
					? `sk-...${creds.azureApiKey.slice(-4)}`
					: '',
				sttIbmKey: creds.ibmWatsonApiKey
					? `sk-...${creds.ibmWatsonApiKey.slice(-4)}`
					: '',
				sttSonioxKey: creds.sonioxApiKey
					? `sk-...${creds.sonioxApiKey.slice(-4)}`
					: '',
		...
		async (
			...
			try {
				...
				if (!key) {
					return {
						success: false,
						error: 'No API key available. Please save a key first.',
					};
				}
	...
	safeHandle(
		...
		async (
			...
			try {
				...
				if (!persisted) {
					CredentialsManager.getInstance().emitStorageStatusDiagnostic(
						'stt_save_failed',
					);
	...
	const sttKeyPersistenceWarning = (
		apiKey: string,
		persisted: boolean,
	...
	safeHandle('set-openai-stt-base-url', async (_, url: string) => {
		try {
			...
			if (!urlCheck.isValid) {
				console.warn('[IPC] Blocked set-openai-stt-base-url', {
					reason: urlCheck.reason,
				});
				return {
					success: false,
					error: `Invalid STT base URL: ${urlCheck.reason}`,
				};
			}
			const { CredentialsManager } = require('./services/CredentialsManager');
	...
	safeHandle('set-ibmwatson-region', async (_, region: string) => {
		try {
			...
			if (!isValidSttRegion(region)) {
				console.warn(
					'[IPC] Blocked set-ibmwatson-region: invalid region shape',
				);
	...
	safeHandle(
		...
		async (
			...
			try {
				...
				// set-*-region setters.
				if (
					(provider === 'azure' || provider === 'ibmwatson') &&
					!isValidSttRegion(region)
				) {
					...
					return await new Promise<{ success: boolean; error?: string }>(
						(resolve) => {
							...
							const timeout = setTimeout(() => {
								ws.close();
								console.error(
									'[IPC] Deepgram test failed: Connection timed out',
								);
						...
						(resolve) => {
							...
							ws.on('close', (code: number) => {
								...
								if (!resolved && code !== 1000) {
										...
										success: false,
										error: `Server closed connection (code ${code})`,
									});
								}
							});
						},
	...
	safeHandle('local-whisper-get-models', async () => {
		try {
			...
			} = require('./audio/whisper/modelLanguageSupport');
		...
		async (
			...
			try {
				...
				if (family === 'embeddings') {
					...
					} = require('./rag/providers/LocalEmbeddingProvider');
					clearLocalEmbeddingPoison();
					return { success: true };
				}
				if (family === 'reranker') {
				...
				return {
					success: false,
					error: `No poison reset path for family '${family}'`,
				};
			} catch (e: any) {
	...
	safeHandle('local-whisper-set-model', async (_, modelId: string) => {
		try {
			...
			if (!MODEL_CATALOG_IDS.has(modelId)) {
				return {
					success: false,
					error: `Unknown local Whisper model: ${modelId}`,
				};
			}
	...
	safeHandle(
		...
		async (
			_,
			cfg: { enabled?: boolean; micModelId?: string; systemModelId?: string },
		) => {
			try {
				const sm = SettingsManager.getInstance();
				const { MODEL_CATALOG_IDS } = require('./audio/whisper/modelManager');
				if (
					typeof cfg?.micModelId === 'string' &&
					cfg.micModelId &&
					!MODEL_CATALOG_IDS.has(cfg.micModelId)
				) {
					return {
						success: false,
						error: `Unknown local Whisper mic model: ${cfg.micModelId}`,
					};
				}
				if (
					typeof cfg?.systemModelId === 'string' &&
					cfg.systemModelId &&
					!MODEL_CATALOG_IDS.has(cfg.systemModelId)
				) {
					return {
						success: false,
						error: `Unknown local Whisper system model: ${cfg.systemModelId}`,
					};
				}
		...
		async (_event, modelId: string) => {
			try {
				const {
					...
					resolveLocalModelProviderName,
				} = require('./services/LocalModelDownloadService');
				const r = LocalModelDownloadService.getInstance().start(
					resolveLocalModelProviderName(modelId),
					modelId,
				);
				// Preserve the original return shape: the panel treats 'already-downloading'
				// as a non-error success.
				if (r.alreadyDownloading)
	...
	safeHandle(
		...
		async (_event, modelId: string) => {
			try {
				...
				return LocalModelDownloadService.getInstance().cancel(
					resolveLocalModelProviderName(modelId),
					modelId,
				);
			} catch (e: any) {
	...
	// mid-download.
	safeHandle(
		'local-whisper-get-download-state',
		async (_event, modelId?: string) => {
			try {
				const {
					LocalModelDownloadService,
	...
	safeHandle('local-whisper-preload', async (_, modelId: string) => {
		if (process.platform === 'darwin') {
			...
			if (Number.isNaN(darwinMajor) || darwinMajor < 22) {
				return {
					success: false,
					error: 'Local Whisper models require macOS 13 Ventura or later.',
				};
			}
		}
		try {
			...
			} = require('./audio/whisper/inferenceConfig');
	...
	const runCodexAuthAction = async (
		action: 'status' | 'logout' | 'login' | 'doctor',
		config?: any,
	) => {
		...
		try {
			...
			if (action === 'status') {
				...
				return {
					...
					output: status.signedIn
						? `Logged in with ChatGPT account (${status.email || 'unknown'})`
						: 'Not signed in',
					config: normalized,
				};
			}
			...
			if (action === 'login') {
				...
				try {
					...
					return {
						success: false,
						action,
						error: e?.message || 'Codex login failed',
						config: normalized,
					};
				}
			}
			...
			return {
				success: false,
				action,
				error: error.message || `Codex CLI ${action} failed.`,
			};
		}
	};

	safeHandle('codex-cli:auth-status', async (_, config?: any) =>
		runCodexAuthAction('status', config),
	);
	safeHandle('codex-cli:logout', async (_, config?: any) =>
		runCodexAuthAction('logout', config),
	);
	safeHandle('codex-cli:login', async (_, config?: any) =>
		runCodexAuthAction('login', config),
	);
	safeHandle('codex-cli:doctor', async (_, config?: any) =>
		runCodexAuthAction('doctor', config),
	);
	...
	const {
		CodexOAuthService: CodexOAuthServiceClass,
	} = require('./services/CodexOAuthService');
	const codexOAuth = CodexOAuthServiceClass.getInstance();
	const broadcastCodexLoginEvent = (
		event:
			| 'login:complete'
			| 'login:failed'
			| 'tokens:refreshed'
	...
	codexOAuth.on('login:failed', (err: Error) =>
		broadcastCodexLoginEvent('login:failed', {
	...
	safeHandle(
		'show-model-selector',
		(_, coords: { x: number; y: number; activate?: boolean }) => {
	...
	safeHandle('debug-inject-transcript', async (_event, segments: unknown) => {
			...
			im.addTranscript(
				{
					...
					text,
					timestamp:
						typeof s?.timestamp === 'number' ? s.timestamp : Date.now(),
					final: true,
		...
		console.log(
			`[TestInjection] Injected ${injected} transcript segment(s) with origin 'test'.`,
		);
	...
	safeHandle(
		...
		async (_event, { query, filters }: { query: string; filters?: any }) => {
			try {
				if (!isIntelligenceFlagEnabled('globalSearchV2'))
				...
				for (const m of meetings) {
					...
					const score = Math.min(
						1,
						hits / Math.max(1, terms.length) + phraseBonus,
					);
					// Best matching snippet for display.
					const snippet =
						haystackParts.find((p) => p.toLowerCase().includes(terms[0])) ||
						m.title ||
						m.summary ||
						'';
					candidates.push({
				...
				try {
					...
					if (
						...
						const { LongTermMemoryService } =
							require('./intelligence/memory/LongTermMemoryService') as typeof import('./intelligence/memory/LongTermMemoryService');
						const ltm = LongTermMemoryService.fromFlags({
							hindsight: { ...hsCfg, timeoutMs: 2000 },
						});
						if (ltm.enabled) {
							const memories = await ltm.recallRelevantMemory(
								q,
								{ userId: _hm.localUserId() },
								{ timeoutMs: 2000, maxResults: 8 },
							);
					...
					console.warn(
						'[GlobalSearchV2] Hindsight recall skipped (non-fatal):',
						memErr?.message,
					);
				}

				const _gsT0 = Date.now();
				const results = new SearchOrchestrator().globalSearch(
					candidates,
					{ userId: 'local' },
					filters || {},
					Date.now(),
	...
	safeHandle(
		...
		async (_event, { query }: { query: string }) => {
			try {
				if (!isIntelligenceFlagEnabled('inMeetingSearchV2'))
					return { enabled: false, results: [] };
				if (typeof query !== 'string') return { enabled: true, results: [] };
				const transcript = appState
					.getIntelligenceManager()
					.getCurrentMeetingTranscript();
				const chunks = transcript.map((t) => ({
					text: t.text,
					timestampMs: t.timestamp,
					speaker: t.speaker,
				}));
				const results = new SearchOrchestrator().inMeetingSearch(
					chunks,
					query || '',
				);
				return { enabled: true, results };
				...
				console.warn(
					'[InMeetingSearchV2] search failed (non-fatal):',
					e?.message,
				);
				return { enabled: true, results: [] };
			}
	...
	safeHandle(
		...
		async (_event, opts?: { title?: string; course?: string }) => {
			try {
				if (!isIntelligenceFlagEnabled('lectureIntelligenceV2'))
					return { enabled: false, notes: null };
				const { LectureIntelligenceService } =
					require('./intelligence/LectureIntelligenceService') as typeof import('./intelligence/LectureIntelligenceService');
				const transcript = appState
					.getIntelligenceManager()
					.getCurrentMeetingTranscript();
				...
				console.warn(
					'[LectureIntelligenceV2] notes generation failed (non-fatal):',
					e?.message,
				);
	...
	safeHandle(
		...
		async (_event, { text }: { text?: string }) => {
			try {
				if (!isIntelligenceFlagEnabled('diagramIntelligence'))
					return { enabled: false, diagram: null };
				if (text !== undefined && typeof text !== 'string')
					return { enabled: true, diagram: null };
				const { DiagramIntelligenceService } =
					require('./intelligence/DiagramIntelligenceService') as typeof import('./intelligence/DiagramIntelligenceService');
				...
				const diagram = new DiagramIntelligenceService().generate({
					text: source,
					fromSourceVisual: false,
				});
				return { enabled: true, diagram };
			} catch (e: any) {
				console.warn(
					'[DiagramIntelligence] generation failed (non-fatal):',
					e?.message,
				);
				return { enabled: true, diagram: null };
			}
	...
	safeHandle(
		...
		async (
			...
			if (!mgr)
				return { success: false, error: 'intelligence manager unavailable' };
	...
	safeHandle(
		...
		async (
			_,
			{ id, labels }: { id: string; labels: Record<string, string> },
		) => {
			if (!id || typeof id !== 'string')
				return { success: false, error: 'invalid id' };
			try {
				const {
					SpeakerLabelService,
				} = require('./services/meeting/SpeakerLabelService');
	...
	const _tracked = async <T>(
		fn: () => Promise<T>,
		failedIf?: (r: T) => boolean,
	): Promise<T> => {
		try {
		...
		async (
			...
			return _tracked(async () => {
				try {
					...
					let screenContextStatus: 'not_available' | 'available' | 'failed' =
						'not_available';
					...
					const validatedImagePaths: string[] | undefined = imagePaths?.length
						? []
						: undefined;
					...
					if (imagePaths && imagePaths.length > 0) {
						if (
							...
							console.warn(
								'[IPC] generate-what-to-say: malformed image path payload rejected',
							);
							return {
								answer: null,
						...
						try {
							...
							const localVisionAvailable =
								credentials.anyLocalVisionProviderConfigured?.() ?? false;
							...
							const sur = await sus.understand({
								...
								technicalInterviewVisionFirst:
									settings.getTechnicalInterviewVisionFirst(),
								providerPolicy: {
									localOnly:
										settings.getScreenUnderstandingMode() === 'private_vision',
									allowScreenshots: providerScopes.screenshots !== false,
									visionAvailable:
										credentials.anyVisionProviderConfigured?.() ?? true,
									localVisionAvailable,
								},
							});
							...
							visionAttempts = Array.isArray(sur.attempts)
								? sur.attempts.length
								: undefined;
							...
							console.warn(
								'[IPC] generate-what-to-say: ScreenUnderstandingService failed',
								{
					...
					let effectiveDomContext =
						typeof options?.domContext === 'string'
							...
							: undefined;
					if (
						options?.domContextEnvelope &&
						process.env.NATIVELY_BROWSER_ENVELOPE_PROMPT !== 'off'
					) {
						try {
							const envelope = sanitizeContextEnvelope(
								options.domContextEnvelope,
							);
							...
							console.warn(
								'[browser-context] envelope prompt formatting failed:',
								e,
							);
						}
					}
	...
	safeHandle('generate-clarify', async () => {
		return _tracked(
			...
			(r: any) =>
				!r || r.clarification === null || r.clarification === undefined,
		);
	});
	...
	safeHandle('dynamic-action:list', async () => {
		try {
			...
			return {
				success: true,
				actions: intelligenceManager.getActiveDynamicActions(),
			};
		} catch (error: any) {
			return {
				success: false,
	...
	safeHandle('select-service-account', async () => {
		try {
			...
			if (!verdict.usable) {
				return {
					success: false,
					error: describeServiceAccountRejection(verdict.reason),
				};
			}
			...
			if (
				...
				return {
					success: false,
					error:
						'The key file is valid, but saving it failed. It will work until you quit. ' +
		...
		return _tracked(async () => {
			try {
				...
				try {
					...
					v2EmailPrompt = resolveV2SystemPrompt({
	...
	function abortPriorRAGQueriesOfClass(
		matchesClass: (key: string) => boolean,
	): void {
	...
	safeHandle(
		...
		async (
			...
			abortPriorRAGQueriesOfClass((key) =>
				key.startsWith(`meeting-${meetingId}-`),
			);
		...
		try {
			...
			if (error.name !== 'AbortError') {
				...
				if (
					...
					console.log(
						`[RAG] JIT query failed with '${msg}', falling back to regular live chat`,
					);
					return { fallback: true };
				}
	...
	const registerSelectedProfilePath = (filePath: string): void => {
		...
		profileSelectedPaths.set(
			normalizeProfilePath(filePath),
			now + PROFILE_SELECTED_PATH_TTL_MS,
		);
	};
	...
	safeHandle('profile:upload-resume', async (_, filePath: string) => {
		try {
			...
			if (!resolvedPath) {
				console.warn(
					'[IPC] profile:upload-resume rejected: path was not produced by profile:select-file or has expired.',
				);
			...
			if (
				...
				return {
					success: false,
					error:
						'Legacy Word .doc files are not supported. Save the file as .docx and upload it again.',
				};
			}
			if (result?.success) {
				...
				try {
					...
					console.warn(
						'[IPC] profile:upload-resume: failed to auto-enable knowledge mode',
						e,
					);
				}
				const activeResume =
				...
				console.log('[ProfileIntelligence] profileFactsReady', {
					...
					experienceCount: Array.isArray(activeResume?.experience)
						? activeResume.experience.length
						: 0,
					projectCount: Array.isArray(activeResume?.projects)
					...
					skillsCount: Array.isArray(activeResume?.skills)
						...
						: activeResume?.skills && typeof activeResume.skills === 'object'
							? Object.values(activeResume.skills).reduce(
	...
	safeHandle('profile:get-status', async () => {
		try {
			...
			} = require('../premium/electron/knowledge/types');
			return {
				...
				aot_pipeline_running: Boolean(
					(orchestrator as any)?.getAOTPipeline?.()?.isRunning?.(),
				),
				resume_indexing_in_flight: Boolean(
					(orchestrator as any)?.isIngesting?.(StatusDocType.RESUME),
				),
				jd_indexing_in_flight: Boolean(
					(orchestrator as any)?.isIngesting?.(StatusDocType.JD),
				),
				...
				extractionMode: activeResume
					? (activeResume as any)?._extraction_mode === 'heuristic'
						? 'heuristic'
						: 'llm'
					: 'none',
			};
	...
	safeHandle('profile:delete', async () => {
		try {
			...
			const { deleteProfileTransactional } =
				require('./services/knowledge/deleteProfileTransactional') as typeof import('./services/knowledge/deleteProfileTransactional');
	...
	safeHandle('profile:get-company-dossier', async () => {
		try {
			...
			console.log(
				`[CompanyIntel-hydrate] activeJD.company="${company}" hasActiveJD=${!!profileData?.hasActiveJD}`,
			);
			if (!company) {
				console.log(
					'[CompanyIntel-hydrate] no company on active JD, returning null',
				);
				return null;
			}
			const dossier = orchestrator
				.getCompanyResearchEngine()
				.getCachedDossier(company);
			console.log(
				`[CompanyIntel-hydrate] getCachedDossier("${company}") → ${dossier ? `${dossier.hiring_strategy?.length || 0}b hiring + ${dossier.culture_ratings?.overall || 'n/a'}/5 culture` : 'null'}`,
			);
			return dossier;
		} catch (error: any) {
	...
	safeHandle('profile:select-file', async () => {
		try {
			const extensions = [...SAFE_DOCUMENT_EXTENSIONS].map((extension) =>
				extension.slice(1),
			);
			const result: any = await dialog.showOpenDialog({
				properties: ['openFile'],
	...
	safeHandle('profile:upload-jd', async (_, filePath: string) => {
		try {
			...
			if (!resolvedPath) {
				console.warn(
					'[IPC] profile:upload-jd rejected: path was not produced by profile:select-file or has expired.',
				);
			...
			if (result?.success) {
				...
				try {
					...
					console.warn(
						'[IPC] profile:upload-jd: failed to auto-enable knowledge mode',
						e,
					);
				}
			}
	...
	safeHandle('knowledge:export-profile-pack', async () => {
		try {
			...
			const { isOkfProfileMarkdownExportEnabled } =
				require('./intelligence/intelligenceFlags') as typeof import('./intelligence/intelligenceFlags');
			if (!isOkfProfileMarkdownExportEnabled())
				return { success: false, error: 'export_flag_off' };

			const { ProfilePackBuilder } =
				require('./services/knowledge/ProfilePackBuilder') as typeof import('./services/knowledge/ProfilePackBuilder');
			const { exportProfileBundle } =
				require('./services/knowledge/ProfileMarkdownExporter') as typeof import('./services/knowledge/ProfileMarkdownExporter');
			const { checkConformance } =
				require('./services/knowledge/OkfConformance') as typeof import('./services/knowledge/OkfConformance');
			const { piTelemetry } =
				require('./llm/piTelemetry') as typeof import('./llm/piTelemetry');
			...
			if (!resumePack && !jdPack)
				return { success: false, error: 'no_profile_pack' };

			const files = exportProfileBundle({
				resumePack,
				jdPack,
			...
			if (!conformance.conformant) {
				console.warn(
					'[IPC] knowledge:export-profile-pack: bundle NOT conformant, refusing to write',
					conformance.violations.slice(0, 5),
				);
				piTelemetry.emit('pi_okf_profile_export_completed', {
					conformant: false,
				...
				return {
					...
					violations: conformance.violations.slice(0, 10),
				};
			}
			...
			try {
				...
				/* best effort — folder still written */
			}
			piTelemetry.emit('pi_okf_profile_export_completed', {
				conformant: true,
				fileCount: files.length,
	...
	safeHandle('knowledge:list-profile-packs', async () => {
		try {
			if (!isProOrTrialActive())
				return { success: false, error: 'pro_required', packs: [] };
			const { isOkfProfileKnowledgeUiEnabled } =
				require('./intelligence/intelligenceFlags') as typeof import('./intelligence/intelligenceFlags');
			if (!isOkfProfileKnowledgeUiEnabled())
				return { success: false, error: 'ui_flag_off', packs: [] };
			const { ProfilePackBuilder } =
				require('./services/knowledge/ProfilePackBuilder') as typeof import('./services/knowledge/ProfilePackBuilder');
			const packs = ProfilePackBuilder.getInstance()
				.getAllProfilePacks()
				.map((p) => ({
					id: p.id,
					fileName: p.fileName,
					cardCount: p.stats.cardCount,
					entityCount: p.stats.entityCount,
					packVersion: p.packVersion,
					updatedAt: p.updatedAt,
					cardsByType: p.cards.reduce((acc: Record<string, number>, c) => {
	...
	safeHandle('knowledge:get-profile-pack', async (_, kind: string) => {
		try {
			...
			const { isOkfProfileKnowledgeUiEnabled } =
				require('./intelligence/intelligenceFlags') as typeof import('./intelligence/intelligenceFlags');
			if (!isOkfProfileKnowledgeUiEnabled())
				return { success: false, error: 'ui_flag_off' };
			const { ProfilePackBuilder } =
				require('./services/knowledge/ProfilePackBuilder') as typeof import('./services/knowledge/ProfilePackBuilder');
			...
			return {
				...
				pack: {
					...
					cards: pack.cards.map((c) => ({
						...
						tags: c.tags,
						entities: c.entities,
						sourceQuotes: c.sourceQuotes.map((q) => q.text),
						pii: c.pii === true,
					})),
				},
			};
	...
	safeHandle('profile:research-company', async (_, companyName: string) => {
		try {
			console.log(
				`[CompanyIntel-research] invoked for companyName="${companyName}"`,
			);
			// Premium gate
			if (!isProOrTrialActive()) {
				return {
			...
			console.log(
				`[CompanyIntel-research] engine returned dossier=${dossier ? 'YES (' + (dossier.hiring_strategy?.length || 0) + 'b)' : 'NULL'}`,
			);
			const searchQuotaExhausted =
				(engine.searchProvider as any)?.quotaExhausted === true;
	...
	const getRoleInsightService = (): any | null => {
		...
		if (
			!orchestrator ||
			typeof orchestrator.getRoleInsightService !== 'function'
		)
			return null;
		...
		try {
			...
			if (!service) {
				...
				try {
					...
					/* orchestrator itself unavailable — falses are then true */
				}
				return {
					available: false,
					hasResume,
	...
	safeHandle('roleInsight:get-report', async (event, analysisId?: string) => {
		try {
			...
			if (!service)
				return {
					success: false,
					error: 'Role Insight is not available in this build.',
				};
			const report = service.getReport(
				typeof analysisId === 'string' ? analysisId : undefined,
			);
				...
				const pump = setInterval(() => {
					try {
						...
						if (!sender.isDestroyed()) {
							sender.send('role-insight-progress', {
								stage: st.stage,
								analysing: st.analysing,
							});
						}
	...
	safeHandle('roleInsight:list-history', async () => {
		try {
			...
			if (!service)
				return {
					success: false,
					error: 'Role Insight is not available.',
					history: [],
				};
	...
	safeHandle(
		...
		async (
			event,
			options: { jobUrl?: string; skipExternalVerification?: boolean } = {},
		) => {
			try {
				if (!isProOrTrialActive()) {
					return {
						success: false,
						error:
							'Pro license required. Please activate a license key to use Role Insight.',
					};
				}
				const service = getRoleInsightService();
				if (!service)
				...
				try {
					const report = await service.analyse({
						jobUrl,
						skipExternalVerification:
							options?.skipExternalVerification === true,
					});
					return { success: true, report };
				} finally {
	...
	safeHandle('roleInsight:apply-correction', async (_, args: any) => {
		try {
			...
			const service = getRoleInsightService();
			if (!service)
				return { success: false, error: 'Role Insight is not available.' };
			if (!args?.analysisId || !args?.kind)
			...
			const report = service.applyCorrection({
				...
				evidenceText:
					typeof args.evidenceText === 'string' ? args.evidenceText : undefined,
			...
			if (!report)
				return { success: false, error: 'That analysis no longer exists.' };
			return { success: true, report };
		} catch (error: any) {
		...
		try {
			...
			if (typeof text !== 'string' || text.trim().length < 200) {
				return {
					success: false,
					error:
						'Paste the full job description — this looks too short to analyse.',
				};
			}
			const orchestrator = appState.getKnowledgeOrchestrator();
			if (!orchestrator)
			...
			if (!resolved)
				return {
					success: false,
					error: 'Could not stage the pasted job description.',
				};
			...
			if (result?.success) {
				try {
					...
					console.warn(
						'[IPC] roleInsight:paste-jd: failed to auto-enable knowledge mode',
						e,
					);
				}
			}
		...
		try {
			...
			if (typeof url !== 'string' || !/^https?:\/\//i.test(url.trim())) {
				return {
					success: false,
					error:
						'Enter a valid job posting URL starting with http:// or https://',
				};
			}
			const orchestrator = appState.getKnowledgeOrchestrator();
			if (!orchestrator)
			...
			if (!provider || typeof provider.extractUrl !== 'function') {
				return {
					success: false,
					error:
						'Reading a job URL needs a Tavily API key. Add one under Tavily Search, or paste the job description instead.',
				};
			}

			const {
				resolveFromUrl,
			} = require('../premium/electron/knowledge/roleInsight/JdSourceResolver');
			const resolvedJd = await resolveFromUrl(url.trim(), provider, '');
			if (!resolvedJd) {
				return {
					success: false,
					error:
						'That page could not be read. Paste the job description text instead.',
				};
			}
			...
			if (!staged)
				return {
					success: false,
					error: 'Could not stage the imported job description.',
				};
				...
				try {
					...
					const sources = await Promise.race([
						...
						new Promise<never>((_, reject) =>
							setTimeout(
								() => reject(new Error('screen-capture-probe-timeout')),
								5000,
							),
						),
					]);
	...
	safeHandle(
		'context-intelligence:flag-set',
		async (_, input: { enabled?: boolean | null }) => {
			try {
				const {
				...
				console.log(
					`[IPC] Context Intelligence V3 opt-in set to ${JSON.stringify(v)}`,
				);
	...
	// than remembered.
	safeHandle(
		'context-intelligence:rollout-metrics',
		async (
			_,
			input?: {
			...
			try {
				...
				} = require('./context-intelligence/observability/rollout-metrics');
				return {
					ok: true,
					metrics: getRolloutMetrics(),
					abort: evaluateAbortConditions(input ?? {}),
				};
			} catch (e: any) {
	...
	safeHandle(
		'context-intelligence:answer-policy-get',
		async (_, input: { modeId?: string; templateType?: string }) => {
			try {
				const {
					isContextIntelligenceV3Enabled,
				} = require('./context-intelligence/contracts/flag');
				const {
					resolveAnswerPolicy,
					shouldOfferAnswerPolicyControl,
					ANSWER_POLICY_LABELS,
				} = require('./context-intelligence/policies/answer-policy');
				const {
					getStoredAnswerPolicy,
				} = require('./context-intelligence/policies/answer-policy-store');
				...
				const resolved = resolveAnswerPolicy({
					modeId: templateType,
					userChoice,
				});
				return {
	...
	safeHandle(
		'context-intelligence:answer-policy-set',
		async (
			_,
			input: { modeId?: string; templateType?: string; policy?: string | null },
		) => {
			try {
				const {
					setStoredAnswerPolicy,
				} = require('./context-intelligence/policies/answer-policy-store');
				...
				if (
					p !== null &&
					p !== 'use_references_when_relevant' &&
					p !== 'only_answer_from_references'
				) {
	...
	safeHandle('modes:set-active', async (_, id: string | null) => {
		try {
			...
			{
				const { abortAndInvalidateChatStreams } =
					require('./services/chatStreamRegistry') as typeof import('./services/chatStreamRegistry');
			...
			try {
				appState.getIntelligenceManager()?.supersedeLiveAnswers?.();
			} catch {
				/* non-fatal */
			}
			...
			try {
				if (activeMode) {
					const statuses =
						ModesManager.getInstance().getReferenceFileIndexStatuses?.(
							activeMode.id,
						) ?? [];
					fileCount = statuses.length;
					indexedCount = statuses.filter(
						(st: any) => st?.status === 'indexed' || (st?.chunkCount ?? 0) > 0,
					).length;
				}
			} catch {
				/* indicator only */
			...
			if (activeMode) {
				void (async () => {
					try {
						await ModesManager.getInstance().prewarmModeReferenceIndex(
							activeMode.id,
						);
					} catch (warmErr: any) {
						console.warn(
							'[IPC] mode reference prewarm failed (non-fatal):',
							warmErr?.message,
						);
					}
					try {
	...
	safeHandle('modes:get-reference-file-status', async (_, modeId: string) => {
		try {
			...
			return {
				success: true,
				statuses:
					ModesManager.getInstance().getReferenceFileIndexStatuses(modeId),
			};
		} catch (e: any) {
	...
	safeHandle('modes:upload-reference-file', async (_, modeId: string) => {
		...
		try {
			...
			const { ingestModeReferenceFile } =
				require('./services/ModeReferenceFileIngestion') as typeof import('./services/ModeReferenceFileIngestion');
			const file = await ingestModeReferenceFile({
				...
				onIndexStatus: (status, fileId) => {
					BrowserWindow.getAllWindows().forEach((win) => {
						if (!win.isDestroyed())
							win.webContents.send('mode-file-index-status', {
								modeId,
								fileId,
								phase: status,
							});
			...
			console.error(
				'[IPC] modes:upload-reference-file error:',
				error?.message || error,
			);
			return {
				success: false,
				error:
					'Could not parse the selected file. It may be corrupt, password-protected, unsupported, or too large.',
			};
		}
	});
	...
	try {
		const {
			knowledgeIndexQueue,
		} = require('./services/knowledge/KnowledgeIndexQueue');
		if (!(global as any).__okfIndexProgressListenerAttached) {
			...
			knowledgeIndexQueue.on('progress', (progress: any) => {
				BrowserWindow.getAllWindows().forEach((win) => {
					if (!win.isDestroyed())
						win.webContents.send('knowledge-index-progress', progress);
				});
			});
		}
	} catch (e: any) {
		console.warn(
			'[IPC] KnowledgeIndexQueue progress forwarding setup failed (non-fatal):',
			e?.message,
		);
	}
		...
		try {
			...
			return {
				...
				packs: packs.map((p: any) => ({
					...
					cardCount: p.stats.cardCount,
					entityCount: p.stats.entityCount,
					relationCount: p.stats.relationCount,
					packVersion: p.packVersion,
					updatedAt: p.updatedAt,
				})),
			};
	...
	safeHandle(
		...
		async (_, params: { fileId: string; modeId: string; fileName: string }) => {
			try {
				if (!isProOrTrialActive())
					return { success: false, error: 'pro_required' };
				const {
				...
				if (!isOkfKnowledgeUiEnabled())
					return { success: false, error: 'okf_knowledge_ui_disabled' };
				const { ModesManager } = require('./services/ModesManager');
				const {
					KnowledgeManager,
				} = require('./services/knowledge/KnowledgeManager');
				const files: any[] = ModesManager.getInstance().getReferenceFiles(
					params.modeId,
				);
	...
	safeHandle('knowledge:export-pack', async (_, fileId: string) => {
		try {
			if (!isProOrTrialActive())
				return { success: false, error: 'pro_required' };
			const {
				isOkfMarkdownExportEnabled,
			} = require('./intelligence/intelligenceFlags');
			if (!isOkfMarkdownExportEnabled())
				return { success: false, error: 'okf_markdown_export_disabled' };
			const {
				KnowledgeManager,
			} = require('./services/knowledge/KnowledgeManager');
			const pack = KnowledgeManager.getInstance().getPackForFile(fileId);
			if (!pack || pack.cards.length === 0)
				return { success: false, error: 'no_pack_for_file' };
			...
			const {
				exportPack,
				exportBundleRoot,
			} = require('./services/knowledge/OkfMarkdownExporter');
			const files = [
				...exportBundleRoot([pack]),
				...exportPack(pack, {
					sourceFileId: fileId,
					sourceFileName: pack.fileName,
				}),
			];
		...
		async (
			...
			try {
				if (!isProOrTrialActive())
					return { success: false, error: 'pro_required' };
				const {
					isOkfUserEditableCardsEnabled,
				} = require('./intelligence/intelligenceFlags');
				if (!isOkfUserEditableCardsEnabled())
					return { success: false, error: 'okf_user_editable_cards_disabled' };
	...
	safeHandle(
		...
		async (_, params: { cardId: string; versionId: string }) => {
			try {
				if (!isProOrTrialActive())
					return { success: false, error: 'pro_required' };
				const {
				...
				const {
					restoreCardVersion,
				} = require('./services/knowledge/OkfCardEditor');
				const card = restoreCardVersion(params.cardId, params.versionId);
				return card
					? { success: true, card }
					: { success: false, error: 'card_or_version_not_found' };
			} catch (e: any) {
	...
	PhoneMirrorService.getInstance().onStatusChange((info) => {
		...
		if (key !== lastLauncherPhoneStatusKey) {
			...
			if (win && !win.isDestroyed()) {
				appState.recordNativeOomOutboundIpc(
					win.webContents.id,
					'phone-mirror:status',
					[launcherInfo],
				);
	...
	// by the policy engine regardless of the AI verdict.
	{
		...
		PhoneMirrorService.getInstance().setMetadataClassifier(
			async (meta: unknown) => {
				...
				if (!browserMetaClassifier) {
					browserMetaClassifier = new BrowserMetadataClassifierService(
						llmHelper,
					);
				}
	...
	safeHandle('skills:open-folder', async () => {
		try {
			...
			return {
				success: false,
				path: '',
				error: e?.message || 'failed to open skills folder',
			};
		}
	});
					...
					errors: [
						{
							field: 'structure',
							code: 'ipc_failed',
							message: e?.message || 'Skill upload failed',
						},
					],
				};
			}
	...
	safeHandle('skills:reap-stages', async () => {
		try {
			const {
				reapStaleUploadStages,
			} = require('./services/skills/SkillInstaller');
	...
	try {
		...
		console.warn(
			'[IPC] skills:reap-stages startup hook error:',
			e?.message || e,
		);
	}
	...
	safeHandle('phone-mirror:capture-tab', async (_, tabId?: number) => {
		try {
			...
			return await PhoneMirrorService.getInstance().requestDomCapture({
				tabId,
			});
		} catch (e: any) {
	...
	safeHandle('phone-mirror:request-auto-context', async () => {
		try {
			const settings =
				SettingsManager.getInstance().getBrowserContextSettings();
			...
			if (settings.autoDetectJobDescriptions)
				extraCategories.push('job_description');
			if (settings.autoDetectDeveloperDocs)
				extraCategories.push('developer_docs');
			...
			return {
				attached: false,
				reason: e?.message || 'failed to request auto context',
			};
		}
	});
	...
	safeHandle(
		...
		async (_, screenshotPath?: string) => {
			try {
				const imgPath =
					screenshotPath || (await appState.takeScreenshot(false));
	...
	PhoneMirrorService.getInstance().onPhoneCommand(async (cmd) => {
		...
		if (cmd.type === 'action') {
			...
			try {
				const { ModesManager } = require('./services/ModesManager');
				phoneDocGrounded =
					ModesManager.getInstance().getActiveModeInfo()
						?.documentGroundedCustomModeActive === true;
			} catch {
				/* mode unavailable — treat as non-doc-grounded */
			}
			...
			const phoneDocGroundedSkipRecall =
				phoneDocGrounded &&
				isIntelligenceFlagEnabled('docGroundedStrictIsolation');
			...
			try {
				...
				if (snap && snap.trim().length > 0) {
					...
					if (phoneDocGrounded && context.trim().length === 0)
						context = undefined;
				}
			} catch (ctxErr) {
				console.warn(
					'[PhoneMirror] Failed to capture pre-turn context:',
					ctxErr,
				);
			}
			...
			try {
				...
				try {
					...
					if (typeof llmMod.planAnswer === 'function') {
						const phonePlan = llmMod.planAnswer({
							...
							activeMode: (() => {
								try {
									return require('./services/ModesManager')
					...
					/* plan unavailable — fall back to no routeOptions (legacy behavior) */
				}
				...
				try {
					...
					const { resolveTurnSourceDecision } =
						require('./llm/turnSourceDecision') as typeof import('./llm/turnSourceDecision');
					const {
						resolveExplicitSourceRequest: _pResolveSwitch,
						resolveExplicitSourceRequests: _pResolveSwitches,
					} =
						require('./intelligence/context-os/explicitSourceSwitch') as typeof import('./intelligence/context-os/explicitSourceSwitch');
					const _pMode = (() => {
						try {
							return require('./services/ModesManager')
								.ModesManager.getInstance()
					...
					const _pTurnSourceDecision = _pSourceContract
						? resolveTurnSourceDecision({
								...
								availability: {
									hasReferenceFiles: Boolean(
										(_pMode as any)?.hasReferenceFiles,
									),
					...
					const _pLegacyUserExplicitSource =
						_pExplicitSwitch === 'job_description'
							? 'profile'
							: _pExplicitSwitch;
					const _pContract = buildCustomModeExecutionContract({
						...
						isDocGroundedCustomModeActive:
							_pMode?.documentGroundedCustomModeActive === true,
						...
						hasLongTermMemory: false,
						persistedSourceAuthority:
							(_pMode as any)?.sourceContract?.sourceAuthority ?? null,
					...
					const _pOwn = resolveSourceOwnership({
						...
						profileContextPolicy:
							phonePlanForOwnership?.profileContextPolicy ?? 'allowed',
						answerType: phonePlanForOwnership?.answerType ?? 'unknown_answer',
						hasProfileFacts: _pHasProfile,
						turnSourceDecision: _pTurnSourceDecision,
					});
					if (
						_pOwn.shouldClarifyInsteadOfProfile &&
						_phoneChatLatestId === myPhoneId
					) {
						...
						try {
							phoneMirror.publishToken(String(myStreamId), clarify);
						} catch (_) {}
						try {
							phoneMirror.publishDone(String(myStreamId), clarify);
						} catch (_) {}
						win?.webContents.send('gemini-stream-token', clarify, {
							streamId: myStreamId,
						});
						...
						if (isIntelligenceFlagEnabled('trace')) {
							console.log(
								'[SOURCE-GUARD] phone: blocked source=profile reason=explicit_profile_ask_in_reference_mode',
								{ owner: _pOwn.owner },
							);
						}
						return;
					...
					if (isIntelligenceFlagEnabled('trace'))
						console.warn(
							'[SOURCE-GUARD] phone ownership check skipped (non-fatal):',
							pOwnErr?.message,
						);
				}
				const stream = llmHelper.streamChat(
				...
				await raceStreamWithDeadline({
					...
					shouldAbort: () => {
						if (_phoneChatLatestId !== myPhoneId) {
							console.log(
								`[PhoneMirror] phone-chat ${myStreamId} superseded by a newer phone message, stopping.`,
							);
				...
				if (_phoneChatLatestId === myPhoneId) {
					...
					const phoneInvalid =
						...
						(phoneTrim.length < 8 ||
							...
							/^\s*(?:hey|hi|hello)[!,.]?\s*(?:there)?[!,.]?\s*(?:what would you like help with|how can i help|what can i (?:help|do))/i.test(
								phoneTrim,
							));
					if (phoneInvalid) {
						console.warn(
							'[PhoneMirror] document-grounded invalid answer blocked from SessionTracker',
							{ chars: phoneTrim.length },
						);
					}
				...
				if (_phoneChatLatestId === myPhoneId) {
					...
					win?.webContents.send(
						'gemini-stream-error',
						err?.message || 'stream error',
						{ streamId: null, source: 'phone-mirror' },
					);
				}
			}
	...
	if (process.env.NATIVELY_E2E === '1') {
		console.warn(
			'[E2E] NATIVELY_E2E=1 — registering test-only IPC handlers (must never ship enabled).',
		);
		...
		safeHandle(
			...
			async (_, params: { modeId: string; filePath: string }) => {
				try {
					...
					console.warn(
						'[E2E] parser-faithful reference upload failed:',
						error?.message,
					);
			...
			async (
				...
				try {
					...
					const file = ModesManager.getInstance().addReferenceFile({
						...
						...(typeof params.pageCount === 'number'
							? {
									...
									extractedPageCount: params.pageCount,
								}
							: {}),
					});
					...
					try {
						...
						console.warn(
							'[E2E] indexReferenceFile failed (non-fatal):',
							idxErr?.message,
						);
					}
					return { success: true, file };
		...
		safeHandle('__e2e__:reindex-embeddings', async (_, modeId: string) => {
			try {
				...
				try {
					...
					console.warn('[E2E] embedding pipeline wire failed:', pErr?.message);
				}
				await mm.retryAllLexicalOnlyFiles?.();
				await mm.prewarmModeReferenceIndex?.(modeId).catch(() => {});
				return {
					success: true,
					statuses: mm.getReferenceFileIndexStatuses?.(modeId) ?? [],
				};
			} catch (e: any) {
				return { success: false, error: e.message };
			}
		...
		safeHandle(
			...
			async (
				_,
				params: { modeId: string; grepBody?: string; titles?: string[] },
			) => {
				try {
					const modeId = typeof params === 'string' ? params : params.modeId;
					const grep = typeof params === 'object' ? params.grepBody : undefined;
					const titles =
						typeof params === 'object' && Array.isArray(params.titles)
							? params.titles.map((t) => t.toLowerCase())
							: undefined;
					const { ModesManager } = require('./services/ModesManager');
					const {
						KnowledgeManager,
					...
					for (const f of files) {
						...
						for (const c of pack.cards) {
							const matchesGrep = grep
								? c.body.toLowerCase().includes(grep.toLowerCase())
								: false;
							const matchesTitle = titles
								? titles.includes(c.title.toLowerCase())
								: false;
							const matches =
								grep || titles ? matchesGrep || matchesTitle : true;
							out.push({
								title: c.title,
								type: c.type,
								confidence: c.confidence,
			...
			async (
				...
				try {
					...
					return {
						success: true,
						block: block || '',
						retrievalConfidence: mm.getLastRetrievalConfidence?.() ?? null,
		...
		safeHandle(
			...
			async (
				...
				seg: {
					text: string;
					confidence?: number;
					priorTurns?: Array<{ speaker: string; text: string }>;
				},
			) => {
				try {
					const text = String(seg.text || '');
					const confidence =
						typeof seg.confidence === 'number' ? seg.confidence : 0.9;
					...
					const { extractLatestQuestion } =
						require('./llm/transcriptQuestionExtractor') as typeof import('./llm/transcriptQuestionExtractor');
					let ts = Date.now() - (seg.priorTurns?.length || 0) * 1000;
					const turns = [
						...(seg.priorTurns || []).map((p) => ({
							role: p.speaker === 'user' ? 'user' : 'interviewer',
							text: p.text,
							timestamp: ts++ * 1000,
						})),
					...
					const isQuestion = Boolean(
						extracted &&
						extracted.latestQuestion &&
						extracted.confidence >= LIVE_MIN_CONFIDENCE,
					);
					// Also expose the simple speculative-fire signal for reference.
					const words = text.trim().split(/\s+/).filter(Boolean);
					const hasSignal =
						text.trimEnd().endsWith('?') ||
						/\b(what|how|why|where|when|which|who|can you|could you|tell me|explain|describe|walk me through|talk me through)\b/i.test(
							text,
						);
					return {
						success: true,
		...
		safeHandle(
			...
			async (_, params: { filePath: string; docType: 'resume' | 'jd' }) => {
				try {
					const orchestrator = appState.getKnowledgeOrchestrator();
					if (!orchestrator)
			...
			try {
				...
				try {
					...
					if (kdb && typeof kdb.getAllNodes === 'function') {
						...
						embeddingSpaces = [
							...new Set(
								nodes.map((n: any) => n.embedding_space).filter(Boolean),
							),
						] as string[];
					}
				} catch {
				...
				const aot = {
					gapAnalysis: (() => {
						try {
							return Boolean(o.getGapAnalysis?.());
						} catch {
							return false;
						}
					})(),
					negotiation: (() => {
						try {
							return Boolean(o.getNegotiationScript?.());
						} catch {
							return false;
						}
					})(),
					mockQuestions: (() => {
						try {
							return Boolean(o.getMockQuestions?.());
						} catch {
							return false;
						}
					})(),
					cultureMappings: (() => {
						try {
							return Boolean(o.getCultureMappings?.());
						} catch {
							return false;
						}
					})(),
				...
				try {
					const {
						ProfilePackBuilder,
					} = require('./services/knowledge/ProfilePackBuilder');
					const packs = ProfilePackBuilder.getInstance().getAllProfilePacks();
					okfPack = packs.map((p: any) => ({
						modeId: p.modeId,
						fileName: p.fileName,
				...
				return {
					...
					resumeExperienceCount: Array.isArray(activeResume?.experience)
						? activeResume.experience.length
						: 0,
					resumeProjectCount: Array.isArray(activeResume?.projects)
						? activeResume.projects.length
						: 0,
					// Education/skills extraction visibility (E2E diagnosis of retrieval gaps).
					resumeEducationCount: Array.isArray(activeResume?.education)
						? activeResume.education.length
						: 0,
					resumeEducation: Array.isArray(activeResume?.education)
						? activeResume.education.map((e: any) => ({
						...
						: [],
					resumeSkillCount: Array.isArray(activeResume?.skills_flat)
						? activeResume.skills_flat.length
						: Array.isArray(activeResume?.skills)
							? activeResume.skills.length
							: 0,
		...
		safeHandle(
			...
			async (
				...
				return await new Promise((resolve) => {
					...
					const onClarify = (c: any) => {
						if (!latest && !nonAnswerDecision)
							nonAnswerDecision = {
								kind: 'clarify',
								text: String(c?.clarification || c || ''),
							};
					};
					const onRecap = (r: any) => {
						if (!latest && !nonAnswerDecision)
							nonAnswerDecision = {
								kind: 'recap',
								text: String(r?.summary || r || ''),
							};
					};
					const onFollowUps = (f: any) => {
						if (!latest && !nonAnswerDecision)
							nonAnswerDecision = {
								kind: 'follow_up_questions',
								text: JSON.stringify(f?.questions || f || ''),
							};
					};
					const cleanup = () => {
						try {
							im.off?.('suggested_answer', onAnswer as any);
						} catch {}
						try {
							im.off?.('suggested_answer_token', onToken as any);
						} catch {}
						try {
							im.off?.('suggested_answer_discard', onDiscard as any);
						} catch {}
						try {
							im.off?.('clarify_ready', onClarify as any);
						} catch {}
						try {
							im.off?.('recap_ready', onRecap as any);
						} catch {}
						try {
							im.off?.('follow_up_questions', onFollowUps as any);
						} catch {}
						if (settleTimer) clearTimeout(settleTimer);
						clearTimeout(timer);
					};
					...
					try {
						im.on?.('clarify_ready', onClarify as any);
					} catch {}
					try {
						im.on?.('recap_ready', onRecap as any);
					} catch {}
					try {
						im.on?.('follow_up_questions', onFollowUps as any);
					} catch {}
					...
					Promise.resolve(
						...
						.then(() => {
							...
							setTimeout(() => {
								...
								settled = true;
								cleanup();
								if (nonAnswerDecision)
									resolve({
										success: true,
										nonAnswer: true,
										decision: nonAnswerDecision.kind,
										answer: nonAnswerDecision.text,
		...
		safeHandle('__e2e__:context-os-prompt-audit', async () => {
			...
			const audit = Array.isArray(g.__contextOsPromptAudit)
				? g.__contextOsPromptAudit.slice()
				: [];
		...
		safeHandle(
			'__e2e__:manual-ask',
			async (event, params: { question: string; timeoutMs?: number }) => {
					...
					const sender: any = {
						...
						send: (channel: string, payload: any) => {
							if (
								...
								const finalText =
									payload && typeof payload.finalText === 'string'
										? payload.finalText
										: tokens;
								resolve({
									success: true,
					...
					Promise.resolve()
						.then(() =>
							handler
								...
								: Promise.reject(
										new Error('gemini-chat-stream handler not captured'),
									),
						)
						.catch((e: any) => {
							if (!done) {
		...
		safeHandle('__e2e__:last-provider-model', async () => {
			try {
				...
				return {
					success: true,
					model: llmHelper.getLastProviderModel?.() ?? null,
				};
			} catch (e: any) {
				return { success: false, error: e?.message || String(e) };
			}
