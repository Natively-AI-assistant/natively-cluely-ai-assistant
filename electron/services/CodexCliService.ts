...
export function isCodexAuthError(err: unknown): boolean {
	...
	return (
		message.includes(CODEX_NOT_SIGNED_IN_MESSAGE) ||
...
export const CODEX_SERVICE_TIERS: readonly CodexServiceTier[] = [
	'default',
	'fast',
	'flex',
] as const;
export const CODEX_MODEL_REASONING_EFFORTS: readonly CodexModelReasoningEffort[] =
...
const CODEX_MODEL_REASONING_SETS: ReadonlyArray<
	readonly [string, readonly CodexModelReasoningEffort[]]
> = [
...
/**
 * ChatGPT's Codex backend (`chatgpt.com/backend-api/codex/responses`) rejects
 * `service_tier: "fast"` with `Unsupported service_tier: fast`. `fast` is an
 * api.openai.com Responses-API value. This service always hits chatgpt.com, so
 * `fast` is dropped (same as `default`: omit the field). `flex` is still sent.
 */
export function resolveCodexServiceTierForChatGptBackend(
	tier?: CodexServiceTier | string | null,
): CodexServiceTier | undefined {
	if (!tier || tier === 'default' || tier === 'fast') return undefined;
	if ((CODEX_SERVICE_TIERS as readonly string[]).includes(tier)) {
		return tier as CodexServiceTier;
	}
	return undefined;
}

/**
 * gpt-5.3-codex-spark is API-key-only. ChatGPT-account OAuth 400s it. This
 * service only authenticates via ChatGPT OAuth, so Spark is rewritten to
 * gpt-5.3-codex (the fast model that works on both auth paths).
 */
export function resolveCodexModelForChatGptAccount(model: string): string {
	const id = (model || '').trim();
	if (id === 'gpt-5.3-codex-spark' || id.endsWith('-codex-spark')) {
		return 'gpt-5.3-codex';
	}
	return id || DEFAULT_CODEX_CLI_CONFIG.model;
}
...
export function resolveCodexReasoningEffort(
	...
	for (const entry of CODEX_MODEL_REASONING_SETS) {
		if (
			id.includes(entry[0]) &&
			(!bestEntry || entry[0].length > bestEntry[0].length)
		) {
			bestEntry = entry;
		}
	}
	...
	if ((valid as readonly string[]).includes(pick))
		return pick as CodexModelReasoningEffort;
	// Unsupported pick. Pick the lowest-latency REASONING effort (skip 'none').
	const reasoningOnly = (valid as readonly string[]).filter(
		(v) => v !== 'none',
	);
...
export class CodexCliService {
	 ...
	 */
	public static readonly SESSION_ID: string = `natively-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
	...
	public static normalizeConfig(
		...
		const sandboxMode =
			config.sandboxMode &&
			(CODEX_SANDBOX_MODES as readonly string[]).includes(config.sandboxMode)
				? config.sandboxMode
				: DEFAULT_CODEX_CLI_CONFIG.sandboxMode;
		const serviceTier =
			config.serviceTier &&
			(CODEX_SERVICE_TIERS as readonly string[]).includes(config.serviceTier)
				? config.serviceTier
		...
		if (
			...
			(CODEX_MODEL_REASONING_EFFORTS as readonly string[]).includes(
				config.modelReasoningEffort,
			)
		) {
		...
		const modelName =
			(config.model || DEFAULT_CODEX_CLI_CONFIG.model).trim() ||
			DEFAULT_CODEX_CLI_CONFIG.model;
		...
		return {
			...
			path:
				(config.path || DEFAULT_CODEX_CLI_CONFIG.path).trim() ||
				DEFAULT_CODEX_CLI_CONFIG.path,
			model: modelName,
			fastModel:
				(config.fastModel || DEFAULT_CODEX_CLI_CONFIG.fastModel).trim() ||
				DEFAULT_CODEX_CLI_CONFIG.fastModel,
			timeoutMs:
	...
	public static async run(
		...
		if (options.signal?.aborted)
			throw new Error('Codex request aborted before start.');
		let out = '';
	...
	public static async *stream(
		...
		let deadlineTimer: ReturnType<typeof setTimeout> = setTimeout(
	 ...
	 */
	private static async encodeImageForRequest(
		imagePath: string,
	): Promise<string | null> {
		try {
			...
			console.warn(
				`[CodexCliService] Image compression failed, trying raw: ${imagePath}`,
				compressErr?.message,
			);
		}
		...
		try {
			...
			if (raw.length > RAW_IMAGE_MAX_BYTES) {
				console.warn(
					`[CodexCliService] Raw image too large to send (${raw.length} bytes), skipping: ${imagePath}`,
				);
				return null;
			}
			...
			if (raw.length < RAW_IMAGE_MIN_BYTES) {
				console.warn(
					`[CodexCliService] Image is empty or truncated (${raw.length} bytes), skipping: ${imagePath}`,
				);
				return null;
			}
			const mimeType =
				IMAGE_MIME_BY_EXT[path.extname(imagePath).toLowerCase()] || 'image/png';
			...
			console.warn(
				`[CodexCliService] Failed to read image, skipping: ${imagePath}`,
				e?.message,
			);
			return null;
		}
	...
	private static async buildRequestBody(
		options: CodexCliRunOptions,
	): Promise<Record<string, unknown>> {
		const model = resolveCodexModelForChatGptAccount(options.model);
		const resolvedEffort = resolveCodexReasoningEffort(
			model,
		...
		const body: Record<string, unknown> = {
			...
			prompt_cache_key: `codex-${model}-${sessionId}`,
		};
		...
		const serviceTier = resolveCodexServiceTierForChatGptBackend(
			options.serviceTier,
		);
		if (serviceTier) {
		...
		while (true) {
			...
			try {
				...
				if (attempt >= TRANSIENT_RETRY_MAX) {
					throw new Error(
						`Codex request failed after ${TRANSIENT_RETRY_MAX} retries: ${e?.message || e}`,
					);
				}
			...
			if (
				...
				if (attempt >= TRANSIENT_RETRY_MAX) {
					...
					throw new Error(
						`Codex upstream ${response.status} after ${TRANSIENT_RETRY_MAX} retries: ${truncate(text, 500)}`,
					);
				}
				attempt++;
			...
			if (!response.ok) {
				...
				const text = await safeReadText(response);
				const message =
					extractResponsesErrorMessage(text) ||
					`Codex upstream ${response.status}`;
	 ...
	 */
	private static async *parseSseStream(
		response: Response,
		signal: AbortSignal,
	): AsyncGenerator<string, void, unknown> {
		if (!response.body) {
		...
		try {
			...
			if (isAbort && sawTerminalEvent) {
				...
				if (process.env.CODEX_DEBUG) {
					console.warn(
						'[Codex] parseSseStream: reader throw AFTER normal completion; swallowed.',
						{
		...
		for (const line of raw.split(/\r?\n/)) {
			...
			for (const c of candidates) {
				...
				if (inner.ok && inner.value?.error?.message)
					return inner.value.error.message;
				return c;
			}
		}
		return '';
...
function sleepWithJitter(
	...
	if (retryAfterMs && retryAfterMs > 0) {
		return new Promise((r) =>
			setTimeout(r, Math.min(retryAfterMs, TRANSIENT_RETRY_CAP_MS * 4)),
		);
	}
	const base = Math.min(
