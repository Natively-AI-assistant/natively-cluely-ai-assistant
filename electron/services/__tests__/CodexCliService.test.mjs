...
const {
	...
	resolveCodexServiceTierForChatGptBackend,
	resolveCodexModelForChatGptAccount,
} = mod;
...
test('ChatGPT backend omits service_tier fast (chatgpt.com 400s it)', () => {
	assert.equal(resolveCodexServiceTierForChatGptBackend('fast'), undefined);
	assert.equal(resolveCodexServiceTierForChatGptBackend('default'), undefined);
	assert.equal(resolveCodexServiceTierForChatGptBackend('flex'), 'flex');
	assert.equal(resolveCodexServiceTierForChatGptBackend(null), undefined);
});

test('ChatGPT account rewrites Codex Spark to gpt-5.3-codex', () => {
	assert.equal(
		resolveCodexModelForChatGptAccount('gpt-5.3-codex-spark'),
		'gpt-5.3-codex',
	);
	assert.equal(
		resolveCodexModelForChatGptAccount('gpt-5.3-codex'),
		'gpt-5.3-codex',
	);
	assert.equal(resolveCodexModelForChatGptAccount('gpt-5.4'), 'gpt-5.4');
});
...
test('resolveCodexReasoningEffort: unknown model id falls back to [low, medium, high]', () => {
	...
	assert.equal(
		resolveCodexReasoningEffort('some-future-model', 'xhigh'),
		'low',
	);
});
...
test('normalizeConfig: downgrades invalid effort for chosen model', () => {
	...
	const cfg = CodexCliService.normalizeConfig({
		model: 'gpt-5.3-codex',
...
test('normalizeConfig: empty input returns defaults', () => {
	assert.deepEqual(
		CodexCliService.normalizeConfig({}),
		DEFAULT_CODEX_CLI_CONFIG,
	);
});
...
test('normalizeConfig: invalid timeouts fall back to default', () => {
	assert.equal(
		CodexCliService.normalizeConfig({ timeoutMs: null }).timeoutMs,
		60_000,
	);
	assert.equal(
		CodexCliService.normalizeConfig({ timeoutMs: -1 }).timeoutMs,
		60_000,
	);
	assert.equal(
		CodexCliService.normalizeConfig({ timeoutMs: 0 }).timeoutMs,
		60_000,
	);
	assert.equal(
		CodexCliService.normalizeConfig({ timeoutMs: 'abc' }).timeoutMs,
		60_000,
	);
	assert.equal(
		CodexCliService.normalizeConfig({ timeoutMs: 30_000 }).timeoutMs,
		30_000,
	);
});
...
test('normalizeConfig: preserves the legacy `path` field for IPC backward-compat', () => {
	...
	assert.equal(
		CodexCliService.normalizeConfig({ path: '/usr/local/bin/codex' }).path,
		'/usr/local/bin/codex',
	);
});
...
test('normalizeConfig: enabled is coerced to boolean', () => {
	...
	assert.equal(
		CodexCliService.normalizeConfig({ enabled: 'yes' }).enabled,
		true,
	);
	assert.equal(
		CodexCliService.normalizeConfig({ enabled: undefined }).enabled,
		false,
	);
});
...
test('normalizeConfig: invalid sandboxMode falls back to read-only', () => {
	...
	assert.equal(
		CodexCliService.normalizeConfig({ sandboxMode: 'evil' }).sandboxMode,
		'read-only',
	);
	assert.equal(
		CodexCliService.normalizeConfig({ sandboxMode: undefined }).sandboxMode,
		'read-only',
	);
});
...
test('normalizeConfig: valid sandboxModes are preserved (legacy compat)', () => {
	assert.equal(
		CodexCliService.normalizeConfig({ sandboxMode: 'workspace-write' })
			.sandboxMode,
		'workspace-write',
	);
	assert.equal(
		CodexCliService.normalizeConfig({ sandboxMode: 'danger-full-access' })
			.sandboxMode,
		'danger-full-access',
	);
});
...
test('extractCodexError: handles plain string error message', () => {
	assert.equal(
		CodexCliService.extractCodexError(
...
test('run: throws when Codex OAuth is not signed in', async () => {
	...
	// the canned fallback.
	const oauthModulePath = path.resolve(
		__dirname,
		'../../../dist-electron/electron/services/CodexOAuthService.js',
	);
...
test('parseSseStream: post-response.completed AbortError does NOT throw "Codex request aborted."', async () => {
	...
	assert.equal(
		...
		'function',
		'parseSseStream must be reachable as a static method',
	);
	const gen = parseSseStream.call(
		mod.CodexCliService,
		ctrl.response,
...
test('parseSseStream: pre-completion AbortError surfaces as "Codex request aborted."', async () => {
	...
	assert.equal(
		final.ok,
		false,
		'pre-completion abort must surface as an error',
	);
	assert.match(
		final.error.message,
...
test('parseSseStream: source uses reader.cancel() (NOT releaseLock) in finally — fix pin', () => {
	...
	const parseSseIdx = source.indexOf(
		'parseSseStream(response: Response, signal: AbortSignal)',
	);
	assert.ok(
		parseSseIdx > 0,
...
test('parseSseStream: source-pin — done: true sets sawTerminalEvent=true (commit 05db8ca)', () => {
	...
	assert.ok(
		doneBranchIdx > 0,
		'`if (done)` branch must still exist in parseSseStream',
	);
	...
	const doneBranchSnippet = afterSignalAbort.slice(
		doneBranchIdx,
		doneBranchIdx + 800,
	);
	assert.match(
		doneBranchSnippet,
		/sawTerminalEvent\s*=\s*true/,
...
test('stream(): source uses resetDeadline() on each yielded delta — idle-timer fix pin', () => {
	...
	assert.ok(
		streamMethodIdx > 0,
		'stream() method must still exist in CodexCliService.ts',
	);
	...
	assert.ok(
		forAwaitIdx > 0,
		'stream() must still have a `for await` loop over deltas',
	);
