...
test('immediate screenshot capture stages pending ref without retaining ordinary attachments', () => {
  const source = read('src/components/NativelyInterface.tsx');
  ...
  assert.doesNotMatch(attachSource, /pendingCaptureRef\.current = data/);
  assert.match(attachSource, /appendScreenshotAttachment\(prev, data\)/);

  const captureStart = source.indexOf('const captureScreenshotForDynamicAction');
  const captureSource = source.slice(captureStart, source.indexOf('const handleDynamicActionAccept', captureStart));
  assert.match(captureSource, /pendingCaptureRef\.current = data/);
