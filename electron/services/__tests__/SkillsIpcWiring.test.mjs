...
test('skills:list and skills:open-folder handlers are registered in ipcHandlers.ts', () => {
	...
	assert.ok(
		findSafeHandle(source, 'skills:list') >= 0,
		'skills:list handler must be registered',
	);
	assert.ok(
		findSafeHandle(source, 'skills:open-folder') >= 0,
		'skills:open-folder handler must be registered',
	);

	// SkillsManager must be imported (handlers reference it).
	assert.match(
		source,
		/import\s*\{\s*SkillsManager\s*\}\s*from\s*['"]\.\/services\/SkillsManager['"]/,
	);
	...
	assert.match(
		openBlock,
		/SkillsManager\.getInstance\(\)\.openSkillsFolder\(\)/,
	);
	assert.match(
		openBlock,
		/catch[\s\S]{0,300}success:\s*false[\s\S]{0,120}path:\s*['"]['"]/,
	);
});
...
test('skills:upload and skills:reap-stages handlers are registered in ipcHandlers.ts', () => {
	...
	assert.ok(
		findSafeHandle(source, 'skills:upload') >= 0,
		'skills:upload handler must be registered (step 3 of the upload flow)',
	);
	assert.ok(
		findSafeHandle(source, 'skills:reap-stages') >= 0,
	...
	assert.match(
		source,
		/require\(['"]\.\/services\/skills\/SkillUploader['"]\)/,
	...
	assert.match(
		uploadBlock,
		/skillsRoot:\s*path\.join\(app\.getPath\(['"]userData['"]\),\s*['"]skills['"]\)/,
	);
	assert.match(uploadBlock, /stagingRoot:\s*os\.tmpdir\(\)/);
	assert.match(
		uploadBlock,
		/autoInstall:\s*opts\?\.autoInstall\s*\?\?\s*false/,
	);
...
test('electron.d.ts declares the skill upload types and bridge methods', () => {
	...
	assert.match(
		types,
		/skillsPreview:\s*\(payload:\s*SkillUploadPayload\)\s*=>\s*Promise<UploadSkillOutcome>/,
	);
});
...
test('SkillsSettings renderer guards upload bridge methods and exposes the upload UI', () => {
	...
	assert.match(
		view,
		/<input[\s\S]{0,200}type="file"[\s\S]{0,200}accept="\.md/,
...
test('preload exposes skillsRefresh / skillsOpenFolder on window.electronAPI', () => {
	...
	assert.match(
		preload,
		/skillsRefresh:\s*\(\)\s*=>\s*ipcRenderer\.invoke\(['"]skills:list['"]\)/,
	);
	assert.match(
		preload,
		/skillsOpenFolder:\s*\(\)\s*=>\s*ipcRenderer\.invoke\(['"]skills:open-folder['"]\)/,
	);
	...
	assert.ok(exposeIdx >= 0, 'electronAPI must be exposed via contextBridge');
	assert.ok(
		preload.indexOf('skillsRefresh:', exposeIdx) > exposeIdx,
...
test('electron.d.ts declares SkillSummary and the two skills methods', () => {
	...
	assert.match(
		types,
		/export interface SkillSummary\s*\{[\s\S]{0,200}id:\s*string;[\s\S]{0,200}source:\s*['"]builtin['"]\s*\|\s*['"]userData['"]/,
	);
	assert.match(types, /skillsRefresh:\s*\(\)\s*=>\s*Promise<SkillSummary\[\]>/);
	assert.match(
		types,
		/skillsOpenFolder:\s*\(\)\s*=>\s*Promise<\{\s*success:\s*boolean;\s*path:\s*string;\s*error\?:\s*string\s*\}>/,
	);
});
...
test('SkillsSettings renderer guards against a missing bridge instead of silent optional-chain', () => {
	...
	assert.match(
		view,
		/typeof window\.electronAPI\?\.skillsRefresh\s*!==\s*['"]function['"]/,
	);
	assert.match(
		view,
		/typeof window\.electronAPI\?\.skillsOpenFolder\s*!==\s*['"]function['"]/,
	);
	assert.match(view, /Skills IPC bridge not detected/);
...
test('every preload ipcRenderer.invoke channel has a matching ipcMain.handle registration', () => {
	...
	assert.ok(
		channels.size > 50,
		`expected many invoke channels, found ${channels.size}`,
	);
	assert.ok(
		channels.has('skills:list'),
		'sanity: skills:list should appear in preload',
	);
	assert.ok(
		channels.has('skills:open-folder'),
		'sanity: skills:open-folder should appear in preload',
	);
	...
	const handleRe =
		/(?:ipcMain\.handle|safeHandle|registerStealthHandler|registerHandler)\(\s*['"]([a-z0-9:_\-./]+)['"]/gi;
	...
	const missing = [...channels]
		.filter((ch) => !registered.has(ch) && !KNOWN_STALE.has(ch))
		.sort();
...
test('SkillsManager.listSkills() returns the builtin humanize-ai-text skill', () => {
	const tmpUserData = fs.mkdtempSync(
		path.join(os.tmpdir(), 'natively-skills-test-'),
	);
	...
	const distPath = path.join(
		root,
		'dist-electron/electron/services/SkillsManager.js',
	);
	assert.ok(
		fs.existsSync(distPath),
		'dist-electron must be built (npm test runs build:electron first)',
	);
	...
	assert.ok(
		humanize,
		`expected humanize-ai-text skill in: ${list.map((s) => s.id).join(', ')}`,
	);
	assert.equal(humanize.source, 'builtin');
	assert.equal(humanize.name, 'humanize-ai-text');
	assert.ok(
		humanize.description.length > 20,
		'description should be non-trivial',
	);

	const ponytail = list.find((s) => s.id === 'ponytail');
	assert.ok(
		ponytail,
		`expected ponytail skill in: ${list.map((s) => s.id).join(', ')}`,
	);
	assert.equal(ponytail.source, 'builtin');
	...
	const humanizeBody = fs.readFileSync(skillFile, 'utf8');
	assert.equal(
	...
	assert.ok(
		bytes > 1000 && bytes < 100 * 1024,
		`seeded SKILL.md (${bytes} bytes) must be under the 100KB cap so it is not skipped`,
	);

	const ponytailFile = path.join(tmpUserData, 'skills', 'ponytail', 'SKILL.md');
	assert.ok(
		fs.existsSync(ponytailFile),
		'ponytail SKILL.md must be seeded on disk',
	);
	const ponytailBody = fs.readFileSync(ponytailFile, 'utf8');
	assert.ok(
		ponytailBody.includes('Senior AI Engineering Partner'),
		'ponytail builtin must include the co-pilot role so on-disk copies rewrite',
	);
...
test('skills:delete handler is registered in ipcHandlers.ts', () => {
	...
	assert.ok(
		findSafeHandle(source, 'skills:delete') >= 0,
	...
	assert.match(
		deleteBlock,
		/catch[\s\S]{0,200}success:\s*false[\s\S]{0,80}error/,
	);
});
...
test('preload exposes skillsDelete on window.electronAPI', () => {
	...
	assert.ok(exposeIdx >= 0, 'electronAPI must be exposed via contextBridge');
	assert.ok(
		preload.indexOf('skillsDelete:', exposeIdx) > exposeIdx,
...
test('SkillsSettings renderer guards the skillsDelete bridge and renders delete UI', () => {
	...
	assert.match(
		view,
		/Trash2/,
		'must import Trash2 from lucide-react for the delete affordance',
	);
...
function freshManager() {
	const tmpUserData = fs.mkdtempSync(
		path.join(os.tmpdir(), 'natively-skills-func-'),
	);
...
test('SkillsManager.deleteSkill() removes a custom skill but refuses builtins', () => {
	...
	assert.ok(
		...
		'builtin SKILL.md must survive a delete attempt',
	);
	...
	assert.equal(
		...
		true,
		`expected delete success, got: ${JSON.stringify(customResult)}`,
	);
	assert.equal(
		fs.existsSync(customDir),
		false,
		'custom skill folder must be removed from disk',
	);
...
test('SkillsManager.setSkillEnabled() persists by folder name across reloads', () => {
	...
	const beforeDisable = manager
		.listSkills()
		.find((s) => s.id === 'humanize-ai-text');
	...
	const afterDisable = manager
		.listSkills()
		.find((s) => s.id === 'humanize-ai-text');
	assert.equal(
		afterDisable.enabled,
		false,
		'listSkills must reflect disabled state',
	);
	...
	assert.ok(
		Array.isArray(raw.disabledFolders),
		'sidecar must have a disabledFolders array',
	);
	assert.ok(
		raw.disabledFolders.includes('humanize-text'),
		`sidecar must contain the folder name 'humanize-text' (got: ${JSON.stringify(raw.disabledFolders)})`,
	);
	assert.equal(
		raw.disabledFolders.includes('humanize-ai-text'),
		false,
...
test('corrupt .skills-state.json defaults to all-enabled without throwing', () => {
	...
	assert.ok(
		list.length > 0,
		'builtin must still seed even with a corrupt sidecar',
	);
	const humanize = list.find((s) => s.id === 'humanize-ai-text');
	assert.ok(humanize);
	assert.equal(
		humanize.enabled,
		true,
		'corrupt sidecar must default to enabled',
	);
});
...
test('SkillsManager.deleteSkill() recursively removes all files in a skill folder', () => {
	...
	fs.writeFileSync(
		path.join(customDir, 'references', 'extra.md'),
		'extra content',
		'utf8',
	);
	...
	assert.equal(
		fs.existsSync(customDir),
		false,
		'skill folder itself must be gone',
	);
	assert.equal(
		fs.existsSync(path.join(customDir, 'references', 'extra.md')),
		false,
		'nested reference file must also be deleted',
	);
	assert.equal(
		fs.existsSync(path.join(customDir, 'notes.txt')),
		false,
...
test('SkillsManager classifies humanize-ai-text as builtin even when folder is renamed', () => {
	...
	const baselineHumanize = baselineList.find(
		(s) => s.id === 'humanize-ai-text',
	);
	assert.ok(
		baselineHumanize,
		'baseline: humanize-ai-text must be discoverable',
	);
	assert.equal(
		baselineHumanize.source,
		'builtin',
	...
	assert.equal(
		baselineDelete.success,
		false,
		'baseline: humanize-ai-text must not be deletable',
	);
	assert.match(
		baselineDelete.error || '',
		/built-in/i,
