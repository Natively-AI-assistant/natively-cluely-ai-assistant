...
describe('Issue #303: /skill and $skill prefix handling in gemini-chat-stream', () => {
	test('gemini-chat-stream handler contains skill prefix parsing', () => {
		...
		assert.match(
			source,
			/skillPrefixMatch/,
			'skillPrefixMatch variable must exist',
		);
		assert.match(
			source,
			/SkillsManager\.getInstance\(\)\.getSkill\(/,
			'getSkill must be called in the handler',
		);
		assert.match(
			source,
			/SkillsManager\.getInstance\(\)\.buildPromptBlock\(/,
			'buildPromptBlock must be called',
		);
		assert.match(
			source,
			/SkillsManager\.getInstance\(\)\.buildDefaultChatSkillPromptBlock\(/,
			'Meeting Copilot must be injected when no /skill prefix is present',
		);
		...
		assert.match(
			source,
			/\[\/\$\]/,
			'prefix regex must accept both / and $ characters',
		);
	});
	...
	test('skill prompt block is prepended to context before streamChat', () => {
		...
		assert.match(
			source,
			/skillPromptBlock[\s\S]{0,200}context = context[\s\S]{0,120}skillPromptBlock/,
		...
		const streamChatIdx = source.indexOf(
			'const stream = llmHelper.streamChat(',
		);
		assert.ok(
			skillInjectionIdx < streamChatIdx,
...
describe('SkillsManager invocation API', () => {
	test('SkillsManager has getSkill() and buildPromptBlock() methods', () => {
		...
		assert.match(
			...
			'getSkill must be a public method',
		);
		assert.match(
			source,
			/public buildPromptBlock\(skill: SkillDetails\)/,
			'buildPromptBlock must be a public method',
		);
		assert.match(
			source,
			/public buildDefaultChatSkillPromptBlock\(\)/,
			'buildDefaultChatSkillPromptBlock must be a public method',
		);
	});
	...
	test('buildPromptBlock wraps instructions in <active_skill> tag', () => {
		...
		assert.match(
			source,
			/<active_skill/,
			'prompt block must use <active_skill> tag',
		);
		assert.match(
			source,
			/\/active_skill>/,
			'prompt block must close <active_skill> tag',
		);
		assert.match(
			source,
			/instruction-only guidance/,
			'prompt block must include security instruction',
		);
	});
	...
	test('getSkill() uses slugify so /humanize-ai-text matches id humanize-ai-text', () => {
		...
		assert.match(
			...
			'getSkill must slugify the input id',
		);
	});
	...
	test('default overlay-chat skill is ponytail (JS Meeting Copilot), separate from humanize', () => {
		...
		assert.match(
			...
			'default skill must lock JavaScript, not Python',
		);
		assert.match(
			source,
			/## 1\. The Core Answer/,
			'default skill must use the numbered Core Answer heading',
		);
		assert.match(
			source,
			/## 2\. Brute Force \(/,
...
describe('Issue #303: skill picker dropdown in overlay chat input', () => {
	...
	test('SkillPicker component is defined in NativelyInterface.tsx', () => {
		...
		assert.match(
			source,
			/function SkillPicker\s*\(/,
			'SkillPicker must be defined',
		);
		assert.match(
			source,
			/position:\s*['"]fixed['"]/,
			'SkillPicker must use fixed positioning to escape overflow-hidden shell',
		);
		assert.match(
			source,
			/onMouseDown.*e\.preventDefault/,
			'SkillPicker must use onMouseDown+preventDefault to keep input focus',
		);
	});
	...
	test('skill list is fetched on mount via skillsRefresh in NativelyInterface.tsx', () => {
		...
		assert.match(
			source,
			/skillsRefresh\?\.\(\)/,
			'skillsRefresh must be called in NativelyInterface',
		);
		assert.match(
			source,
			/setAvailableSkills/,
			'availableSkills state must be set from skillsRefresh result',
		);
	});
	...
	test('picker derives from inputValue — opens on / or $ prefix, closes on space', () => {
		...
		assert.match(
			source,
			/\^[\/\$\[]/,
			'picker regex must anchor to start and match / or $',
		);
		assert.match(
			source,
			/filteredSkills/,
			'filtered skills derived list must exist',
		);
		assert.match(
			source,
			/skillPickerQuery/,
			'skillPickerQuery derived value must exist',
		);
	});
	...
	test('ArrowUp/ArrowDown/Escape/Tab/Enter handled in onKeyDown for picker navigation', () => {
		...
		assert.match(
			source,
			/Escape.*setInputValue\(['"]['"]\)|setInputValue\(['"]['"]\).*Escape/s,
			'Escape must clear inputValue to close picker',
		);
		assert.match(
			source,
			/Tab.*selectSkill|selectSkill.*Tab/s,
			'Tab must select the highlighted skill',
		);
	});
