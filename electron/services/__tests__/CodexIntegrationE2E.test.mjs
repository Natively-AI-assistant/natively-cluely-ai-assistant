...
test('A.1: gpt-5.4 + xhigh → request body carries reasoning.effort "xhigh"', async () => {
	assert.equal(
		resolveCodexReasoningEffort('gpt-5.4', 'xhigh'),
		'xhigh',
...
test('A.2: gpt-5.3-codex + xhigh → resolver downgrades to "low" (smoking gun)', async () => {
	...
	assert.equal(
		...
		'low',
		'the downgraded value, not the requested one, reaches the wire',
	);
});
...
test('A.4b: ChatGPT backend omits service_tier fast and rewrites Spark', async () => {
	const fastBody = await CodexCliService.buildRequestBody({
		prompt: 'hi',
		model: 'gpt-5.3-codex-spark',
		timeoutMs: 60_000,
		serviceTier: 'fast',
	});
	assert.equal(fastBody.model, 'gpt-5.3-codex');
	assert.equal(fastBody.service_tier, undefined);
});
...
test('A.7: an already-aborted signal is refused before any auth or network work', async () => {
	...
	await assert.rejects(
		() =>
			CodexCliService.run('ignored-path', {
				prompt: 'hi',
				model: 'gpt-5.4',
				signal: ac.signal,
			}),
	...
	await assert.rejects(async () => {
		for await (const _ of CodexCliService.stream('ignored-path', {
			prompt: 'hi',
			model: 'gpt-5.4',
			signal: ac.signal,
		})) {
...
test('B.1: source-level — fastModeApplies + fastModeAppliesNS both contain !isCodexCliModel(currentModelId) (Issue #315 fix)', () => {
	...
	const nsMatch = llmHelperSrc.match(
		/const fastModeAppliesNS\s*=[\s\S]*?(?=\s*;)/,
	);
	const sMatch = llmHelperSrc.match(
		/const fastModeApplies\s*=[\s\S]*?(?=\s*;)/,
	);
	assert.ok(
		nsMatch,
		'fastModeAppliesNS declaration must exist in LLMHelper.ts',
	);
