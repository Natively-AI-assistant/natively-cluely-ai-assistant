...
// probe additionally gates on !skillWasExplicit, making an immunity that was
...
describe('composeWtaSystemPrompt (WTA surface)', () => {
	const skill = {
		...
		promptBlock: '<active_skill>rewrite naturally</active_skill>',
	};

	test('V3 system + active skill → skill block is appended, V3 system preserved', () => {
		const out = composeWtaSystemPrompt(
			'V3 SYSTEM PROMPT',
			'LEGACY OVERRIDE',
			skill,
		);
		...
		assert.ok(
			out.includes(skill.promptBlock),
			'skill promptBlock must be present',
		);
	});
	...
	test('V3 system + no skill → exactly the V3 system prompt (no behavior change)', () => {
		...
		assert.equal(
			composeWtaSystemPrompt('V3 SYSTEM PROMPT', 'LEGACY OVERRIDE', null),
			'V3 SYSTEM PROMPT',
		);
	});
	...
	test('no V3 prompt → legacy override verbatim, with or without skill', () => {
		assert.equal(
			composeWtaSystemPrompt(undefined, 'LEGACY OVERRIDE', skill),
			'LEGACY OVERRIDE',
		);
		assert.equal(
			composeWtaSystemPrompt(null, 'LEGACY OVERRIDE', undefined),
			'LEGACY OVERRIDE',
		);
	});
	...
	test('WhatToAnswerLLM wires the composition helper at the _wtaSystemPrompt site', () => {
		...
		assert.match(
			source,
			/_wtaSystemPrompt = composeWtaSystemPrompt\(/,
			'_wtaSystemPrompt must be built via composeWtaSystemPrompt so activeSkill survives V3',
		);
		assert.doesNotMatch(
			source,
			/_wtaSystemPrompt = _v3p\?\.system \?\? finalPromptOverride/,
...
describe('manual-chat V3 short-circuit honors /skill prefixes', () => {
	test('skill prefix parsing occurs BEFORE the V3 short-circuit', () => {
		...
		assert.ok(
			skillParseIdx !== -1 && v3EntryIdx !== -1,
			'both sites must exist',
		);
		assert.ok(
			skillParseIdx < v3EntryIdx,
	...
	test('V3 stream sends a system prompt that includes skillPromptBlock when set', () => {
		...
		assert.match(
			source,
			/skillPromptBlock\s*\?\s*`\$\{composed\.system\}[\s\S]{0,40}\$\{skillPromptBlock\}`/,
	...
	test('legacy context injection site still exists (non-V3 path unchanged)', () => {
		...
		assert.match(
			source,
			/context = context[\s\S]{0,80}\$\{skillPromptBlock\}[\s\S]{0,40}\$\{context\}[\s\S]{0,40}skillPromptBlock/,
	...
	test('legacy identity probe is skipped when a skill is active', () => {
		...
		assert.match(
			source,
			/if \(\s*!skillWasExplicit &&\s*!imagePaths\?\.length &&\s*typeof message === 'string'\s*\) \{/,
			'the identity-probe gate must include !skillWasExplicit so explicit /skill turns bypass the probe',
		);
	});
});
