...
test('a finalized automatic question pushes hint then answer and invokes the LLM once',async t=>{
  ...
  await new Promise((resolve,reject)=>{const deadline=setTimeout(()=>reject(Error('answer timeout')),3000);const timer=setInterval(()=>{if(messages.some(item=>item.type==='full_answer')){clearTimeout(deadline);clearInterval(timer);resolve()}},20)});
  assert.equal(llmCalls,1);assert.equal(messages.filter(item=>item.type==='fast_hint').length,1);assert.equal(messages.filter(item=>item.type==='full_answer').length,1);assert.ok(messages.findIndex(item=>item.type==='fast_hint')<messages.findIndex(item=>item.type==='full_answer'));live.socket.close();
});
