...
test('a finalized automatic question pushes hint then answer and invokes the LLM once',async t=>{
  ...
  const server=new DefenceServer(config);const info=await server.listen();const base=info.urls[0];
  t.after(async()=>{await server.close();await new Promise(resolve=>provider.close(resolve));await fs.rm(root,{recursive:true,force:true})});
  await fetch(base+'/api/project/index',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
  const pairing=await fetch(base+'/api/pairing/create',{method:'POST'}).then(r=>r.json());const auth=await fetch(base+'/api/pairing/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:pairing.id,secret:pairing.secret})}).then(r=>r.json());
  const live=await wsSession(base,auth.token);const messages=[];live.socket.on('message',raw=>messages.push(JSON.parse(raw.toString())));
  const transcript={type:'transcript',text:'How does the project ground its answers?',final:true,silenceMs:700};live.socket.send(JSON.stringify(transcript));live.socket.send(JSON.stringify(transcript));
  await new Promise((resolve,reject)=>{const deadline=setTimeout(()=>reject(Error('answer timeout')),3000);const timer=setInterval(()=>{if(messages.some(item=>item.type==='evidence')){clearTimeout(deadline);clearInterval(timer);resolve()}},20)});
  const transcriptIndex=messages.findIndex(item=>item.type==='transcript'&&item.final);const finalizedIndex=messages.findIndex(item=>item.type==='question_finalized');const hintIndex=messages.findIndex(item=>item.type==='fast_hint');const answerIndex=messages.findIndex(item=>item.type==='full_answer');const evidenceIndex=messages.findIndex(item=>item.type==='evidence');
  assert.equal(llmCalls,1);assert.equal(messages.filter(item=>item.type==='question_finalized').length,1);assert.equal(messages.filter(item=>item.type==='fast_hint').length,1);assert.equal(messages.filter(item=>item.type==='full_answer').length,1);assert.equal(messages.filter(item=>item.type==='evidence').length,1);assert.ok(transcriptIndex<finalizedIndex&&finalizedIndex<hintIndex&&hintIndex<answerIndex&&answerIndex<evidenceIndex);live.socket.close();
});
