...
async function main(): Promise<void> {
  ...
  if (!configured(config.stt.apiKey) || config.stt.provider === 'none' || !configured(config.llm.apiKey) || config.llm.provider === 'none') {
    report.status = 'BLOCKED_MISSING_LIVE_PROVIDER_CONFIG'; report.configuration = { sttConfigured: configured(config.stt.apiKey) && config.stt.provider !== 'none', llmConfigured: configured(config.llm.apiKey) && config.llm.provider !== 'none' }; console.log(JSON.stringify(report, null, 2)); return;
  }
  if (config.projectId !== PROJECT_ID || !fs.existsSync(path.join(dataRoot, 'manifest.json'))) {
    report.status = 'BLOCKED_CBA_PROJECT_NOT_ACTIVE'; report.activeProjectId = config.projectId; console.log(JSON.stringify(report, null, 2)); return;
  }
  ...
  if (Object.values(fixtures).some(value => !value)) { report.status = 'BLOCKED_MISSING_LIVE_AUDIO_FIXTURES'; report.missingFixtures = Object.entries(fixtures).filter(([, value]) => !value).map(([language]) => language); console.log(JSON.stringify(report, null, 2)); return; }
