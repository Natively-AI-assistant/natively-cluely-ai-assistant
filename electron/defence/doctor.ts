...
export function runDoctor(env: NodeJS.ProcessEnv = process.env) {
  ...
  const result = {
    ...
    dualSource: { enabled: config.input.dualSource.enabled, overlapWindowMs: config.input.dualSource.overlapWindowMs, transcriptSimilarity: config.input.dualSource.transcriptSimilarity },
