...
export class AppState {
  ...
  public setUndetectable(state: boolean): void {
    ...
    if (process.platform === 'win32') {
      ...
      // whose tooltip still read the real app name for the whole session, and a
      // session STARTED undetectable never got a tray menu (show window / quit)
      // back after toggling off. hideTray() is null-guarded and showTray() is
      // idempotent, so this is safe on every real state change.
      ...
      // the equivalent by dropping the Dock tile. Goes through WindowHelper so
      // the same call also runs at launcher creation, keeping a cold start with
      // undetectable already ON consistent with a mid-session toggle.
