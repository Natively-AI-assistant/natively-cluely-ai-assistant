...
export class AppState {
  ...
  private setupAutoUpdater(): void {
    ...
    // Use saved channel setting, fallback to version-based detection
    const settingsManager = SettingsManager.getInstance()
    ...
    console.log(`[AutoUpdater] Channel: ${autoUpdater.channel} (saved: ${savedChannel || 'none'})`)
  ...
  // For manual check in development mode (dev mode skip electron-updater)
  private async checkForUpdatesManual(): Promise<void> {
    try {
      console.log('[AutoUpdater] Manual check via GitHub API...');
      const releaseManager = ReleaseNotesManager.getInstance();
      const notes = await releaseManager.fetchReleaseNotes('latest');

      if (notes) {
        const currentVersion = app.getVersion();
        const latestVersion = notes.version.replace(/^v/, '');

        console.log(`[AutoUpdater] Manual: Current=${currentVersion}, Latest=${latestVersion}`);
  ...
  public setUpdateChannel(channel: 'stable' | 'beta'): void {
    console.log(`[AutoUpdater] setUpdateChannel(${channel}) called`);
    autoUpdater.channel = channel === 'beta' ? 'beta' : 'latest';
    console.log(`[AutoUpdater] Channel updated to: ${autoUpdater.channel}`);
