...
import {
	MeetingLifecycleQueue,
	type MeetingLifecycleState,
} from './audio/meetingLifecycleQueue';
...
process.on('uncaughtException', (err) => {
	...
	if (
		...
		/^\[nativeArch(?::packaged)?\]/.test(err.message)
	) {
...
process.on('unhandledRejection', (reason, promise) => {
	...
	const now = Date.now();
	while (
		unhandledRejectionHistory.length > 0 &&
		now - unhandledRejectionHistory[0] >= UNHANDLED_REJECTION_WINDOW_MS
	) {
...
function logCrashEvent(label: string, payload: Record<string, unknown>): void {
	try {
		logToFile(
			`[CRASH:${label}] ${safeJsonForLog(getCrashDiagnostics(payload))}`,
		);
	} catch (e: any) {
		...
		try {
			logToFile(`[CRASH:${label}] logging-failed: ${String(e?.message || e)}`);
		} catch {
			/* ignored */
		}
	}
...
function emergencyCloseDatabase(reason: string): void {
	...
	try {
		...
		try {
			...
			logToFile(
				`[DB-EMERGENCY] close failed during ${reason}: ${e?.message || e}`,
			);
		...
		try {
			logToFile(`[DB-EMERGENCY] failed during ${reason}: ${e?.message || e}`);
		} catch {
			/* ignored */
		}
	}
...
function isNativeArchGateCrash(err: unknown): boolean {
	return (
		err instanceof Error && /^\[nativeArch(?::packaged)?\]/.test(err.message)
	);
}
...
function writeProcessReport(label: string): string | null {
	try {
		...
		const dir = path.dirname(
			getLogFile() || path.join(os.tmpdir(), 'natively_debug.log'),
		);
		try {
		...
		try {
			...
			if (
				...
				'environmentVariables' in parsed
			) {
				delete parsed.environmentVariables;
				parsed.environmentVariables =
					'[REDACTED: see natively_debug.log for env-free diagnostic context]';
			...
			logToFile(
				`[CRASH:${label}] process report redacted-failed; deleted: ${e?.message || e}`,
			);
			return null;
		}
		logToFile(
			`[CRASH:${label}] process report written (env redacted): ${file}`,
		);
		return file;
	} catch (e: any) {
...
function resetStartupLog(): void {
	...
	try {
		...
		try {
			fs.appendFileSync(logFile, `[reset-failed] ${e?.message || e}\n`);
		} catch {
			/* ignored */
		}
	}
...
function collectWalSnapshot(): Array<{ file: string; mb: number }> {
	try {
		const userData = app.isReady()
			...
			: path.dirname(getLogFile() || os.tmpdir());
		return fs
...
function getMacScreenCaptureStatus(): MacScreenCaptureStatus {
	...
	if (isDevTccBypassEnabled()) {
		console.log(
			'[Main] Dev TCC bypass enabled (NATIVELY_DEV_BYPASS_SCREEN_TCC=1) — reporting screen capture as granted',
		);
		return 'granted';
	}
	...
	try {
		...
		console.log(
			`[Main] Screen recording permission status: ${status} (packaged=${app.isPackaged})`,
		);
		return status;
	} catch (error) {
...
async function resolveMacScreenCaptureCapability(
	...
	if (!options?.bypassCache && screenCapabilityCache) {
		...
		if (age < SCREEN_CAPABILITY_CACHE_TTL_MS) {
			console.log(
				`[Main] Using cached screen capability result (age=${age}ms) for context: ${context}`,
			);
	...
	try {
		...
		if (capturable) {
			clearSystemAudioPermissionWarning();
			console.warn(
				`[Main] Screen Recording status is denied during ${context}, but capture probe succeeded (sourceCount=${sourceCount}); continuing without permission banner.`,
			);
		} else {
			...
			console.warn(
				`[Main] Screen Recording capture probe returned 0 screens during ${context} (status=denied, packaged=${app.isPackaged}) — showing permission banner.`,
			);
		}
		...
		const result = {
			...
			...(capturable
				? {}
				: { titleKey: permissionTitleKey('screen-recording-denied') }),
		};
		screenCapabilityCache = { result, timestamp: Date.now() };
		return result;
	} catch (error: any) {
...
export type OnnxRecoveryFamily =
	| 'whisper'
	| 'intent'
	| 'embeddings'
	| 'reranker';
...
try {
	...
	textHasCompEvidence =
		require('../premium/electron/knowledge/NegotiationConversationTracker').textHasCompEvidence;
} catch {
	console.log(
...
} from './services/PhoneMirrorService';
import {
	describePageCaptureFallback,
	describeDoubleCaptureFailure,
	PAGE_CAPTURE_FALLBACK_CHANNEL,
	PAGE_CAPTURE_STARTED_CHANNEL,
} from './services/pageCaptureFallback';
...
const VALID_DISGUISE_MODES: readonly DisguiseMode[] = [
	'terminal',
	'settings',
	'activity',
	'none',
...
export class AppState {
	...
	public recordNativeOomOutboundIpc(
	...
	private readonly _meetingLifecycle = new MeetingLifecycleQueue((state) =>
		this.logMeetingLifecycleState(state),
	);
	...
	private _transcriptPartialThrottle = new Map<
		string,
		{
	...
	private onnxRecoveryNotices: Partial<
		Record<OnnxRecoveryFamily, OnnxRecoveryNotice>
	> = {};
	...
	constructor() {
		...
		this.disguiseMode = normalizeDisguiseMode(
			settingsManager.get('disguiseMode'),
		);
		this._verboseLogging = settingsManager.get('verboseLogging') ?? true;
		setVerboseLoggingFlag(this._verboseLogging);
		this._ambientChatEnabled =
			settingsManager.get('ambientChatEnabled') ?? false;
		console.log(
			`[AppState] Initialized with isUndetectable=${this.isUndetectable}, disguiseMode=${this.disguiseMode}, verboseLogging=${this._verboseLogging}, ambientChatEnabled=${this._ambientChatEnabled}`,
		);
		...
		try {
			...
			const {
				bindContextDebugLogDirectory,
			} = require('./context-intelligence/debug/jsonl-writer');
			bindContextDebugConfig({
				readStoredLevel: () => {
					try {
						return SettingsManager.getInstance().get('contextDebugLevel');
					} catch {
						return undefined;
					}
				},
			...
			bindContextDebugLogDirectory(
				path.join(app.getPath('logs'), 'context-debug'),
			);
			const dbg = describeContextDebugConfig();
			if (dbg.level !== 'off') {
				console.log(
					`[CONTEXT_DEBUG] level=${dbg.level} (source: ${dbg.levelSource})`,
				);
			}
			if (dbg.contentInclusion) {
				console.warn(
					'[CONTEXT_DEBUG_WARNING] Full local evidence logging is enabled. Logs may contain sensitive personal data.',
				);
			}
		} catch (e) {
			console.warn(
				'[AppState] context-debug binding failed (logging disabled):',
				(e as Error)?.message,
			);
		}
		...
		setStealthHookAvailabilityProvider(() => {
			...
			try {
				...
				} = require('./services/StealthKeyboardManager');
				...
				if (
					...
					if (poisoned?.modelId) {
						...
						if (resetAny) {
							...
							console.warn(
								`[AppState] Previous local Whisper load for ${poisoned.modelId} did not finish cleanly; current settings no longer reference it.`,
							);
						}
					}
					const rawModelId =
						settingsManager.get('localWhisperModel') ?? FALLBACK;
					const modelId = MODEL_CATALOG_IDS.has(rawModelId)
						? rawModelId
						: FALLBACK;
					if (modelId !== rawModelId) {
						console.warn(
							`[AppState] Persisted localWhisperModel "${rawModelId}" not in catalog — resetting to ${modelId}`,
						);
					...
					if (settingsManager.get('localWhisperPerChannelEnabled')) {
						for (const key of [
							...
							if (raw && !MODEL_CATALOG_IDS.has(raw)) {
								console.warn(
									`[AppState] Persisted ${key} "${raw}" not in catalog — resetting to ${FALLBACK}`,
								);
								settingsManager.set(key, FALLBACK);
							}
						}
					...
					const micOverride = settingsManager.get(
						'localWhisperPerChannelEnabled',
					)
						? (settingsManager.get('localWhisperModelMic') ?? '')
						: '';
					const preloadPriority = [
						micOverride && MODEL_CATALOG_IDS.has(micOverride)
							? micOverride
							: '',
						modelId,
					].filter(Boolean);
					const primaryPreloadId =
						preloadPriority.find((id) => isModelCached(id, dtype)) ?? '';
					...
					for (const id of modelIds) {
						if (isModelCached(id, dtype)) {
							...
							console.log(
								`[AppState] Local Whisper model "${id}" not cached — starting background download`,
							);
							try {
								const result = LocalModelDownloadService.getInstance().start(
									resolveLocalModelProviderName(id),
									id,
								);
								if (!result.success && !result.alreadyDownloading) {
									console.warn(
										`[AppState] Auto-download for "${id}" rejected:`,
										result.error,
									);
								}
							} catch (dlErr: any) {
								console.warn(
									`[AppState] Auto-download for "${id}" failed to start:`,
									dlErr?.message,
								);
							}
						}
		...
		setImmediate(() => {
			try {
				const {
					consumeIntentClassifierSentinel,
				} = require('./llm/IntentClassifier');
				const {
					consumeLocalEmbeddingSentinel,
				...
				console.warn(
					'[AppState] ONNX sentinel consume skipped (non-fatal):',
					e?.message || e,
				);
			}
		});
		...
		registerStealthHandler(
			'get-system-audio-permission-warning',
			() => latestSystemAudioPermissionWarning,
		);
		...
		if (process.platform === 'darwin' || process.platform === 'win32') {
			...
			registerStealthHandler('stealth-tap:available', () =>
				stealth.isAvailable(),
			);
		...
		keybindManager.onShortcutTriggered(async (actionId) => {
			...
			try {
				if (actionId === 'general:toggle-visibility') {
					...
					this.sendToWindow(mainWindow, 'global-shortcut', {
					...
					try {
						...
						console.warn(
							'[Main] DOM capture error — falling back to screenshot:',
							e?.message || e,
						);
					}
					if (!captured) {
						...
						const fallbackNotice =
							describePageCaptureFallback(domFailureReason);
					...
					if (overlay && !overlay.isDestroyed()) {
						this.sendToWindow(overlay, 'global-shortcut', {
							action: 'focusInput',
						});
				...
				if (
					...
					console.error(
						`[Main] Error handling global shortcut ${actionId}:`,
						e,
					);
				}
			}
		...
		// block which silently skips when premium modules are absent.
		{
			...
			llmHelper.setCodexCliConfig({
				...
				fastModel: settingsManager.get('codexCliFastModel') || 'gpt-5.3-codex',
				...
				modelReasoningEffort: settingsManager.get(
					'codexCliModelReasoningEffort',
				),
			});
		}
		...
		const emit = () => {
			try {
				...
				const flags = {
					...
					ragSpeculativeRerank: isIntelligenceFlagEnabled(
					...
					jitFinalAnswerEnforced: isIntelligenceFlagEnabled(
				...
				try {
					...
					try {
						for (const wc of webContents?.getAllWebContents?.() || []) {
							try {
								...
								pidToWin[ospid] = pidToWin[ospid]
									? `${pidToWin[ospid]}+${label}`
									: label;
							} catch {
								/* per-wc best effort */
							}
						...
						/* webContents enumeration best effort */
					}
					procMem = (eApp.getAppMetrics?.() || [])
						.map((m: any) => ({
							type: m.type,
							rssMB: m.memory?.workingSetSize
					...
					/* getAppMetrics unavailable pre-ready — skip */
				}
	...
	private sampleNativeOomTrace(memory = process.memoryUsage()): void {
		...
		nativeOomTrace.sample(
			...
			(() => {
				try {
					return (app.getAppMetrics?.() || []) as unknown as Array<
						Record<string, unknown>
					>;
				} catch {
					return [];
			...
			(() => {
				...
				return pid > 0
					? { webContentsId: launcher.webContents.id, pid }
					: undefined;
			})(),
	...
	private startNativeOomTraceSampling(): void {
		...
		this.nativeOomTraceTimer = setInterval(
			() => this.sampleNativeOomTrace(),
			1000,
		);
	...
	private emitTranscriptToSurfaces(payload: {
		speaker: string;
		text: string;
		timestamp: number;
		final: boolean;
	 ...
	 */
	private sendThrottledTranscript(payload: {
		speaker: string;
		text: string;
		timestamp: number;
		final: boolean;
	...
	public sendSystemAudioPermissionDenied(
		message: string,
		titleKey?: string,
	): void {
		this.sendToMeetingSurfaces(
	...
	public scheduleModeReferenceIndexRetry(): void {
		...
		this.modeReferenceRetryPromise = pipeline
			.waitForReady(15000)
			.then(async () => {
				const { ModesManager } = require('./services/ModesManager');
				const modesManager = ModesManager.getInstance();
				await modesManager.retryAllLexicalOnlyFiles().catch(() => {
					/* logged inside */
				});
			})
			.catch(() => {
				/* provider unavailable — lexical fallback remains valid */
			})
			.finally(() => {
				this.modeReferenceRetryPromise = null;
			});
	}
			...
			try {
				...
				try {
					...
					if (hasCloudEmbeddingKey) {
						console.log(
							'[AppState] Skipping Ollama embeddings bootstrap — a cloud embedding provider is configured.',
						);
						return;
					}
				} catch (guardErr: any) {
					...
					console.warn(
						'[AppState] Ollama bootstrap cloud-key guard failed (non-fatal):',
						guardErr?.message,
					);
				}
				...
				try {
					...
					if (explicitNoOllama) {
						console.log(
							'[AppState] Skipping Ollama embeddings bootstrap — user has opted out (disableOllamaBootstrap/localProvider).',
						);
						return;
					}
					...
					console.warn(
						'[AppState] Ollama bootstrap opt-out check failed (non-fatal):',
						settingsErr?.message,
					);
				}
	...
	private initializeRAGManager(): void {
		try {
			...
			if (sqliteDb) {
				...
				ModesManager.getInstance().setSharedEmbeddingPipeline(
					modeEmbeddingPipeline,
				);
				this.scheduleModeReferenceIndexRetry();
				...
				try {
					...
					console.warn('[AppState] V3 meeting retriever wiring skipped:', e);
				}
		...
		try {
			...
			if (
				...
				this.knowledgeOrchestrator = new KnowledgeOrchestratorClass(
					knowledgeDb,
				);
				...
				if (
					typeof this.knowledgeOrchestrator.setLiveCoachingContentFn ===
					'function'
				) {
					this.knowledgeOrchestrator.setLiveCoachingContentFn(
						async (contents: any[]) => {
							return await llmHelper.generateContentStructured(
								joinContents(contents),
								{ preferFast: true },
				...
				if (
					typeof this.knowledgeOrchestrator.setSearchProviderResolver ===
					'function'
				) {
					const {
						resolveCompanySearchProvider,
					} = require('./services/resolveCompanySearchProvider');
					this.knowledgeOrchestrator.setSearchProviderResolver(
						resolveCompanySearchProvider,
					);
				}
				...
				if (
					typeof this.knowledgeOrchestrator.setEmbedWithMetadataFn ===
					'function'
				) {
					this.knowledgeOrchestrator.setEmbedWithMetadataFn(
						embedWithProducerMetadata,
					);
				}
				...
				if (
					typeof this.knowledgeOrchestrator.setFastQueryEmbedFn === 'function'
				) {
				...
				if (
					typeof this.knowledgeOrchestrator.ensureEmbeddingSpace === 'function'
				) {
					...
					(async () => {
						try {
							...
							console.warn(
								'[main] Knowledge ensureEmbeddingSpace kick failed (non-fatal):',
								e?.message || e,
							);
						}
					})();
				...
				if (
					typeof this.knowledgeOrchestrator.setConversationContextProvider ===
					'function'
				) {
					this.knowledgeOrchestrator.setConversationContextProvider(() => {
						...
						try {
							...
							const interviewerTurns = items.filter(
								(i: any) => i.role === 'interviewer',
							);
							const lastTwo = interviewerTurns.slice(-2);
							const lastInterviewerTurn = lastTwo.length
								? lastTwo[lastTwo.length - 1].text
								: undefined;
							const recentInterviewerComp = lastTwo.some((i: any) =>
								textHasCompEvidence!(i.text),
							);
	...
	private setupAutoUpdater(): void {
		...
		try {
			...
			if (Array.isArray(publish) && publish.length > 0) {
				...
				feedLabel = publish
					.map(
						(p: any) =>
							`${p.provider || 'unknown'}:${p.owner ? `${p.owner}/${p.repo ?? ''}` : p.url || '(default)'}`,
					)
					.join(', ');
		...
		autoUpdater.on('update-downloaded', (info) => {
			...
			this.downloadedUpdateInfo = {
				...info,
				updateFile: (info as any).filePath,
			};
	...
	public async quitAndInstallUpdate(): Promise<void> {
		...
		const currentVersion = app.getVersion();
		const downloadedVersion = (this.downloadedUpdateInfo?.version ?? '')
			.toString()
			.replace(/^v/, '');
		if (!downloadedVersion) {
			console.error(
				'[AutoUpdater] quitAndInstall called but no downloaded update info',
			);
			return;
		}
		if (!AppState.isRealUpgrade(currentVersion, downloadedVersion)) {
			...
			this.broadcast(
				'update-error',
				`Refusing non-upgrade update (${currentVersion} → ${downloadedVersion})`,
			);
			return;
		}
		...
		if (canAutoInstall()) {
			console.log(
				'[AutoUpdater] Performing real quitAndInstall (signed/auto-installable build)',
			);
	...
	public async downloadUpdate(): Promise<void> {
		if (
			...
			console.log(
				'[AutoUpdater] Download already completed — re-broadcasting downloaded update',
			);
			this.broadcast('update-downloaded', this.downloadedUpdateInfo);
			return;
		}
		...
		if (this.updateDownloadState === 'downloading') {
			console.log(
				'[AutoUpdater] Download already in progress — ignoring duplicate request',
			);
	...
	private createSTTProvider(
		...
		if (sttProvider === 'natively') {
			...
			if (apiKey) {
				...
				try {
					if (
						speaker === 'interviewer' &&
						isIntelligenceFlagEnabled('speakerDiarizationV1')
					) {
			...
			if (apiKey) {
				console.log(
					`[Main] Using OpenAIStreamingSTT for ${speaker}${baseUrl ? ` (custom endpoint: ${baseUrl})` : ' (WebSocket+REST fallback)'}`,
				);
			...
			const sm = SettingsManager.getInstance();
			const globalModel =
				sm.get('localWhisperModel') ?? 'Xenova/whisper-tiny.en';
		...
		const effectiveSttId: string =
			stt instanceof DeepgramStreamingSTT
				? 'deepgram'
				: stt instanceof SonioxStreamingSTT
					? 'soniox'
					: stt instanceof OpenAIStreamingSTT
						? 'openai'
						: stt instanceof ElevenLabsStreamingSTT
							? 'elevenlabs'
							: stt instanceof NativelyProSTT
								? 'natively'
								: stt instanceof GoogleSTT
									? 'google'
									: sttProvider;
		...
		stt.on(
			...
			(segment: {
				...
				this.intelligenceManager.handleTranscript({
					...
					punctuationSource: punctuationSourceFor(
						effectiveSttId,
						segment.isFinal,
					),
				});
				...
				if (segment.isFinal && speaker === 'interviewer') {
					...
					if (trackerFeedAllowed) {
						this.knowledgeOrchestrator?.feedInterviewerUtterance?.(
							segment.text,
						);
					}
				}
		...
		stt.on('error', (err: Error) => {
			...
			if (grpcCode === 11 || /Audio Timeout Error/i.test(err.message || '')) {
				console.warn(
					`[Main] STT (${speaker}) idle-timed-out (provider's no-audio limit), reconnecting on next chunk.`,
				);
				return;
			}
		...
		stt.on(
			...
			(w: { code?: string; message?: string; droppedBytes?: number }) => {
				console.warn(
					`[Main] STT (${speaker}) warning: ${w?.code ?? 'unknown'}`,
					{
						provider: sttProvider,
						message: w?.message,
		...
		if (stt instanceof NativelyProSTT) {
			...
			stt.on('persistent-reconnect', (info: { attempts: number }) => {
				console.warn(
					`[Main] STT persistent reconnect (${speaker}): ${info.attempts} consecutive attempts.`,
				);
				this.sendSttStatus({
					state: 'reconnecting',
					provider: sttProvider,
	...
	private wireSystemCapture(
		...
		const systemAudioHealth = new SystemAudioHealthClassifier({
			watchdogMs: STUCK_WATCHDOG_MS,
		});
		const handleSystemAudioHealthDecision = (decision: any) => {
			...
			if (decision.type === 'log') {
				...
				if (
					decision.reason === 'sustained-zero-valued-silence' &&
					process.platform === 'darwin'
				) {
					void resolveMacScreenCaptureCapability(
						'sustained zero-fill diagnosis',
						{ bypassCache: true },
					)
						.then((cap) => {
							...
							const msg = formatPermissionMessage(
								'mac-screen-recording-revoked-rebuild',
							);
						...
						.catch(() => {
							/* probe failed — keep log-only behavior */
						});
				}
				return;
			}
		...
		const armStuckWatchdog = () => {
			handleSystemAudioHealthDecision(
				systemAudioHealth.handle({
		...
		capture.on('data', (chunk: Buffer) => {
			const now = Date.now();
			handleSystemAudioHealthDecision(
				systemAudioHealth.handle({ kind: 'chunk', nowMs: now, chunk }),
			);
			chunkCount++;
			...
			if (chunkCount <= 3 || chunkCount % 500 === 0) {
				console.log(
					`${prefix}SystemAudio->STT: chunk #${chunkCount}, ${chunk.length}B, googleSTT=${this.googleSTT ? 'active' : 'NULL'}`,
				);
			}
		...
		capture.on('data', (chunk: Buffer) => {
			...
			if (lastChunkAt > 0) {
				...
				if (gap > 2000 && gap < 8000) {
					console.warn(
						`${prefix}Mic chunk gap ${gap}ms — likely transient device change (USB hot-plug, BT reconnect). Resuming.`,
					);
				}
			}
			lastChunkAt = now;
						...
						if (builtIn && !alreadyBuiltIn) {
							...
							setImmediate(() => {
								if (
									...
									void this.reconfigureAudio(builtIn.id, outputId).catch(
										(err) =>
											console.warn(
												`${prefix}HFP auto-switch reconfigure failed:`,
												err,
											),
									);
								}
			...
			if (!zerofillLatched && !zerofillTriggered) {
				...
				if (peakToPeak > 100) {
					...
					console.warn(
						`${prefix}Mic chunks all zero-filled (peak-to-peak < 100) for ${ZEROFILL_OBSERVATION_MS / 1000}s — TCC denial or device-mute suspected.`,
					);
					this.sendAudioCaptureFailed({
						channel: 'mic',
		...
		try {
			...
			if (screenCapability.effectiveDenied) {
				...
				this.sendSystemAudioPermissionDenied(
					message,
					screenCapability.titleKey ??
						permissionTitleKey('screen-recording-denied'),
				);
				...
				try {
					...
					console.error(
						'[Main] SystemAudioCapture construction failed:',
						capErr,
					);
					this.systemAudioCapture = null;
					this.sendAudioCaptureFailed({
						channel: 'system',
						message:
							'System audio capture failed to initialize. The native audio module could not allocate the capture device. Restarting Natively may help; if the problem persists, file a bug.',
						attempt: 0,
						maxAttempts: 0,
						terminal: true,
						stuck: false,
			...
			if (!this.microphoneCapture) {
				...
				try {
					...
					console.error(
						'[Main] MicrophoneCapture construction failed:',
						capErr,
					);
					...
					this.sendAudioCaptureFailed({
						channel: 'mic',
						message:
							'Microphone capture failed to initialize. The native audio module could not open the default input device. Check that the device is connected and not in exclusive use by another app, then restart Natively.',
						attempt: 0,
						maxAttempts: 0,
						terminal: true,
						stuck: false,
	...
	public async restartCapturesAfterResume(): Promise<void> {
		...
		if (this.googleSTT) {
			try {
				...
				console.warn('[Main] Resume: googleSTT teardown threw:', e);
			}
			this.googleSTT = null;
		}
		if (this.googleSTT_User) {
			try {
				...
				console.warn('[Main] Resume: googleSTT_User teardown threw:', e);
			}
			this.googleSTT_User = null;
		}
		...
		try {
			...
			if (screenCapability.effectiveDenied) {
				...
				console.warn(
					'[Main] Resume: capture rebuilt by another flow mid-await — keeping theirs.',
				);
			} else {
				this.systemAudioCapture = new SystemAudioCapture(
		...
		if (!this.googleSTT) {
			try {
				...
				console.error(
					'[Main] Resume: interviewer STT recreate failed:',
					sttErr,
				);
				this.googleSTT = null;
			}
	 ...
	 */
	private checkSameInputOutputDevice(
		inputId?: string,
		outputId?: string,
	): string | undefined {
	 ...
	 */
	private pickFallbackInputDevice(
	...
	private findBuiltInInputDevice(): { id: string; name: string } | undefined {
		try {
			const builtIn = AudioDevices.getInputDevices().find((d) =>
				/macbook|built[- ]?in|imac|mac\s+studio|mac\s+mini|internal/i.test(
					d.name,
				),
			);
	...
	private async reconfigureAudio(
		...
		if (wantedInput && wantedOutput) {
			const conflict = this.checkSameInputOutputDevice(
				wantedInput,
				wantedOutput,
			);
			if (conflict) {
		...
		if (!micAutoSwitched) {
			try {
				...
				const inputIsExplicitBt =
					!!explicitName && this.isBluetoothInputName(explicitName);

				const outputName = this.getEffectiveOutputDeviceName(wantedOutput);
				const outputIsBt =
					!!outputName && this.isBluetoothInputName(outputName);
				...
				if (willBeHfp) {
					...
					if (
						builtIn &&
						this.normalizeDeviceName(builtIn.name) !==
							this.normalizeDeviceName(fromLabel)
					) {
						console.warn(
							`[Main] Bluetooth mic ("${fromLabel}") would force HFP (low quality). Auto-switching mic to "${builtIn.name}" to keep it in A2DP.`,
						);
	...
	private setupAudioRecoveryHandler(): void {
		...
		this.systemAudioCapture.on('error', async (err: Error) => {
			...
			const isRecoveryCurrentMeeting = () =>
				this.isMeetingActive &&
				this._meetingGeneration === recoveryMeetingGeneration;
			...
			try {
				...
				if (this.systemAudioCapture) {
					console.warn(
						'[AudioRecovery] Capture rebuilt by another flow mid-await — keeping theirs.',
					);
					return;
				}
	...
	private async handleDefaultOutputChanged(currentId?: string): Promise<void> {
		...
		const isCurrentMeeting = () =>
			this.isMeetingActive && this._meetingGeneration === meetingGeneration;
		if (this._isQuitting) return;
		...
		try {
			...
			if (this.systemAudioCapture) {
				console.warn(
					'[DefaultOutputWatcher] Capture rebuilt by another flow mid-await — keeping theirs.',
				);
			...
			if (this._routeChangeRebuildFailures <= 3) {
				this._lastObservedDefaultOutputId = previousObservedOutputId;
				console.error(
					`[DefaultOutputWatcher] Route-change rebuild failed (attempt ${this._routeChangeRebuildFailures}/3) — will retry on next tick:`,
					err,
				);
			} else {
				...
				console.error(
					'[DefaultOutputWatcher] Route-change rebuild failed after 3 attempts — giving up until the next route change:',
					err,
				);
				this.broadcastDeviceSelection({
					kind: 'output',
	...
	private setupMicRecoveryHandler(): void {
		...
		this.microphoneCapture.on('error', async (err: Error) => {
			...
			const isMicRecoveryCurrentMeeting = () =>
				this.isMeetingActive &&
				this._meetingGeneration === micRecoveryMeetingGeneration;
				...
				try {
					...
					if (
						...
						try {
							...
							console.error(
								'[MicRecovery] Failed to restart system audio after mic recovery pause:',
								restartErr,
							);
							this.sendAudioCaptureFailed({
								channel: 'system',
	...
	private async _startAudioTestImpl(deviceId?: string): Promise<void> {
		...
		if (!(await ensureMacMicrophoneAccess('audio test'))) {
			...
			// what failed.
			throw new Error(
				`${permissionTitleKey('mic-denied')}: ${formatPermissionMessage('mic-denied')}`,
			);
		}
	 ...
	 */
	private startCaptureChannels(context: string): {
		mic: boolean;
		system: boolean;
	} {
	...
	public startMeeting(metadata?: any): Promise<void> {
		return this._meetingLifecycle.start(() =>
			this.startMeetingTransition(metadata),
		);
	}
	...
	private async startMeetingTransition(metadata?: any): Promise<void> {
		...
		try {
			...
			console.warn(
				'[Main] failed to bind dynamic action context at meeting start:',
		...
		try {
			...
			telemetryService.track({
				...
				modeId: am?.id,
				properties: {
					modeTemplateType: am?.templateType,
		...
		try {
			...
			/* non-fatal — warmup must never block meeting start */
		}
		...
		this._audioInitPromise = (async () => {
			...
			const abortStaleAudioInit = () => {
				...
				if (userSttStartedByInit) {
					if (this.googleSTT_User === userSttOwnedByInit)
						this.googleSTT_User?.stop();
				}
				if (liveIndexingStartedByInit) {
					if (this.ragManager === ragManagerOwnedByInit)
						this.ragManager?.stopLiveIndexing?.();
				}
			};
			...
			try {
				...
				if (this._ambientChatEnabled) {
					...
					console.warn(
						'[Main] Meeting starting WITHOUT audio capture — Ambient AI Chat is ON (Settings > General). Transcript, summary and usage will be empty for this meeting.',
					);
				}
				if (!this._ambientChatEnabled) {
					...
					console.log(
						'[Main] Ambient AI Chat enabled — skipping mic/system audio capture and STT for this session.',
					);
				}
				...
				const isAbort =
					(err as Error)?.message === 'audio_init_aborted' ||
					!isCurrentMeeting();
				if (!isAbort) {
			...
			try {
				...
				if (meetingId) {
					...
					if ((stopResult?.memoryEligibleCount ?? 1) > 0) {
						...
						console.log(
							'[Main] No memory-eligible transcript — skipping meeting RAG processing.',
						);
					}
	...
	private async processCompletedMeetingForRAG(
		...
		if (this._ragProcessingInFlight.has(meetingId)) {
			console.log(
				`[AppState] RAG processing for ${meetingId} already in flight — skipping duplicate.`,
			);
			return;
		}
	...
	private setupIntelligenceEvents(): void {
		...
		this.intelligenceManager.on(
			'suggested_answer_discard',
			(reason: string) => {
				flushBatchesBeforeFinal();
				const win = mainWindow();
				this.sendToWindow(win, 'intelligence-suggested-answer-discard', {
					reason,
				});
			},
		);
		...
		this.intelligenceManager.on(
			...
			(answer: string, intent: string) => {
				flushBatchesBeforeFinal();
				const win = mainWindow();
				this.sendToWindow(win, 'intelligence-refined-answer', {
					answer,
					intent,
				});
			},
		...
		this.intelligenceManager.on(
			...
			(questions: string) => {
				flushBatchesBeforeFinal();
				const win = mainWindow();
				this.sendToWindow(win, 'intelligence-follow-up-questions-update', {
					questions,
				});
			},
		);
		...
		this.intelligenceManager.on(
			...
			(answer: string, question: string) => {
				const win = mainWindow();
				this.sendToWindow(win, 'intelligence-manual-result', {
					answer,
					question,
				});
			},
		...
		this.intelligenceManager.on('error', (error: Error, mode: string) => {
			...
			const win = mainWindow();
			this.sendToWindow(win, 'intelligence-error', {
				error: error.message,
				mode,
			});
		});
	...
	public setLocalWhisperRecoveryNotice(
		notice: LocalWhisperRecoveryNotice,
	): void {
	...
	public takeOnnxRecoveryNotice(
		family: OnnxRecoveryFamily,
	): OnnxRecoveryNotice | null {
		...
		return this.withScreenshotCaptureSession(
			...
			async () => {
				...
				if (process.platform === 'win32' || process.platform === 'darwin') {
					...
					captureArea =
						(await this.cropperWindowHelper.showCropper()) ?? undefined;
	...
	private _enforceDockState(
		...
		if (attempt < maxAttempts) {
			const t = setTimeout(() => {
				this._dockReassertTimers = this._dockReassertTimers.filter(
					(x) => x !== t,
				);
				this._enforceDockState(
					wantUndetectable,
					targetFocusWindow,
					attempt + 1,
					maxAttempts,
...
async function initializeApp() {
	...
	try {
		const { LifecycleTracker } = require('./utils/lifecycleTracker');
		const consoleLog: (msg: string, meta?: Record<string, unknown>) => void = (
			msg,
			meta,
		) => {
	...
	try {
		...
		const telemetryEnabledSetting =
			SettingsManager.getInstance().get('telemetryEnabled');
		...
		try {
			...
			/* settings unavailable — distinctId stays undefined */
		}

		const sinks: Array<Record<string, unknown>> = [
			{ name: 'local-jsonl', enabled: true },
		];
		if (process.env.POSTHOG_API_KEY) {
			sinks.push({
				...
				endpoint: process.env.POSTHOG_HOST || 'https://app.posthog.com',
				distinctId,
			});
		}
		...
		const remote = sinks
			.filter((s) => s.name !== 'local-jsonl')
			.map((s) => s.name);
		console.log(
			`[Telemetry] sinks: local-jsonl${remote.length ? ' + ' + remote.join(' + ') : ' (remote unconfigured)'} release=${release}`,
		);
		telemetryService.track({
			name: 'app_start',
			properties: { platform: process.platform, release },
		});
		logStartupPhase('telemetry-configure:complete', {
			release,
			remoteSinks: remote,
		});
	} catch (err) {
	...
	try {
		...
		getReviewHardwareId()
			.then((hwid: string | null) =>
				reviewService.syncWithBackend(apiKey, hwid),
			)
			.catch(() => {});
	} catch (err) {
		console.warn(
			'[Init] ReviewService recordSessionStart failed (non-fatal):',
			err,
		);
	}
	...
	try {
		const {
			LocalModelDownloadService,
			createWhisperDownloadProvider,
			createNemotronDownloadProvider,
		} = require('./services/LocalModelDownloadService');
		...
		try {
			const {
				createRerankerDownloadProvider,
			} = require('./rag/rerankerDownloadProvider');
			downloadService.registerProvider(createRerankerDownloadProvider());
		} catch (e: any) {
			console.warn(
				'[main] Reranker download provider registration failed (non-fatal):',
				e?.message,
			);
		}
	} catch (e: any) {
		console.warn(
			'[main] LocalModelDownloadService init failed (non-fatal):',
			e?.message,
		);
	}
	...
	try {
		...
		if (shouldStartOllama) {
			OllamaManager.getInstance()
				...
				.catch((err: any) =>
					console.warn(
						'[OllamaManager] Startup ensureRunning failed (non-fatal):',
						err?.message || err,
					),
				);
		} else {
			OllamaManager.getInstance().skipStartup(
				'Ollama not selected; startup skipped',
			);
			console.log(
				'[OllamaManager] Skipping Ollama startup; Ollama provider not selected',
			);
		}
	} catch (err: any) {
		console.warn(
			'[OllamaManager] Startup selection check failed (non-fatal):',
			err?.message || err,
		);
		OllamaManager.getInstance().skipStartup(
			'Ollama startup skipped after selection check failure',
		);
	}
	...
	try {
		...
		setTimeout(() => {
			void usageOutbox.dispatchOnce();
		}, 5000);
	} catch (err: any) {
		console.warn(
			'[UsageOutbox] startup failed (non-fatal):',
			err?.message || err,
		);
	}
	...
	// See googleServiceAccount.ts.
	{
		...
		for (const candidate of candidates) {
			...
			if (verdict.usable) {
				console.log(
					`[Init] Loaded Google Service Account path from the ${candidate.source}`,
				);
	...
	if (process.env.THINKING_BENCH === '1') {
		(async () => {
			try {
				...
				if (!llmHelper) {
					console.error('[ThinkingBudgetBench] LLMHelper unavailable');
					app.quit();
					return;
				}
				const {
				...
				const budgets = (
					process.env.THINKING_BENCH_BUDGETS || '0,128,512,1024,-1'
				)
					.split(',')
					.map((s: string) => Number(s.trim()));
				const repeats = Number(process.env.THINKING_BENCH_REPEATS || '1');
				const model =
					process.env.THINKING_BENCH_MODEL || 'gemini-3.1-flash-lite';
				// Give the embedding/provider init a moment to settle.
				await new Promise((r) => setTimeout(r, 2000));
				await runThinkingBudgetBench(llmHelper, {
					budgets,
					repeats,
					model,
	...
	if (process.env.THINKING_MATRIX === '1') {
		(async () => {
			try {
				const llmHelper = appState.processingHelper?.getLLMHelper?.();
				if (!llmHelper) {
					console.error('[ThinkingMatrix] LLMHelper unavailable');
					app.quit();
					return;
				}
				const {
				...
				await new Promise((r) => setTimeout(r, 2000));
				await runThinkingMatrix(llmHelper, {
					model,
					delayMs,
					configs,
					log: (s: string) => console.log(s),
	...
	if (process.platform === 'darwin') {
		try {
			const mc = (appState as unknown as { microphoneCapture: unknown })
				.microphoneCapture;
			const at = (appState as unknown as { audioTestCapture: unknown })
				.audioTestCapture;
		...
		() => {
			...
			try {
				...
				runLocalFallbackPreflight({
					...
					console.warn(
						'[LocalFallbackPreflight] failed to run (non-fatal):',
						err?.message || err,
					),
				);
			} catch (err: any) {
				console.warn(
					'[LocalFallbackPreflight] scheduling failed (non-fatal):',
					err?.message || err,
				);
			}
		},
	...
	if (preflightTimer && typeof preflightTimer.unref === 'function')
	...
	setTimeout(
		() => {
			try {
				...
				console.warn(
					'[Init] Intent classifier warmup scheduling failed (non-fatal):',
					err,
				);
			}
		},
	...
	try {
		...
		powerMonitor.on('suspend', () => {
			console.log(
				'[Main] powerMonitor: system suspending. Captures will be recreated on resume if a meeting is active.',
			);
		});
	} catch (err) {
		console.warn(
			'[Main] powerMonitor unavailable — sleep/wake recovery disabled:',
			err,
		);
	}
	...
	const disablePhoneMirrorOnBoot =
		process.env.NATIVELY_DISABLE_PHONE_MIRROR === '1';
	if (
		shouldStartPhoneMirrorOnBoot({
			disablePhoneMirror: disablePhoneMirrorOnBoot,
			phoneMirrorEnabled:
				!!SettingsManager.getInstance().get('phoneMirrorEnabled'),
		})
	) {
		PhoneMirrorService.getInstance()
			.start({
				exposeOnLan: !!SettingsManager.getInstance().get(
					'phoneMirrorExposeOnLan',
				),
				persist: false,
			})
			.catch((err) =>
				console.error('[Init] PhoneMirror auto-start failed:', err),
			);
	} else if (disablePhoneMirrorOnBoot) {
		console.warn(
			'[LeakTest] NATIVELY_DISABLE_PHONE_MIRROR=1 → PhoneMirror WS server NOT started this run',
		);
	}
		...
		setTimeout(async () => {
			try {
				...
				if (isDevTccBypassEnabled()) {
					...
					console.log(
						'[Init] Dev TCC bypass enabled — skipping startup screen-recording check',
					);
					return;
				}
				...
				if (screenStatus === 'not-determined') {
					...
					screenStatus === 'denied' &&
					!appState.getAmbientChatEnabled()
				) {
				...
				try {
					...
					console.log(
						`[Init] Microphone permission status at startup: ${micStatus}`,
					);
					...
					if (appState.getAmbientChatEnabled()) {
						...
						console.warn(
							'[Init] Microphone was previously denied — notifying UI banner.',
						);
						...
						console.warn(
							'[Init] Microphone is restricted by device policy at startup.',
						);
						appState.sendAudioCaptureFailed({
							channel: 'mic',
							message:
								'Microphone is restricted by device policy. Contact your administrator to enable microphone access for Natively.',
							attempt: 0,
							maxAttempts: 0,
							terminal: true,
							stuck: false,
					...
					console.warn(
						'[Init] Startup microphone permission check failed:',
						micErr,
					);
				}
			} catch (e) {
	...
	function checkpointDatabase(reason: string): void {
		try {
			...
			console.warn(
				`[main] DatabaseManager.checkpoint failed during ${reason} (non-fatal):`,
				e,
			);
		}
	}
	...
	app.on('render-process-gone', (_event, webContents, details) => {
		...
		if (!isCrash || appState.isQuitting?.()) {
			...
			if (appState.isQuitting?.()) {
				...
				logToFile(
					`[main] render-process-gone: non-crash reason=${reason} — not reloading, DB left open`,
				);
			}
			return;
		}
		...
		if (!isRecoverableWindow) {
			logToFile(
				`[main] render-process-gone: not auto-reloading transient window (${urlNow})`,
			);
			return;
		}
		...
		if (typeof id !== 'number' || !webContents || webContents.isDestroyed?.()) {
			...
			try {
				appState.createWindow();
				logToFile(
					'[main] render-process-gone: webContents destroyed, recreated launcher window',
				);
			} catch (e: any) {
				logToFile(
					`[main] render-process-gone: window recreate failed: ${e?.message || e}`,
				);
			}
			return;
		}
		...
		try {
			...
			logToFile(
				`[main] render-process-gone: reload failed: ${e?.message || e}`,
			);
		}
	});
	...
	app.on('before-quit', (event) => {
		...
		try {
			...
			console.error(
				'[main] Failed to stop DefaultOutputWatcher during shutdown:',
				e,
			);
		}
		...
		try {
			...
			if (totals.counted) {
				...
				getReviewHardwareId()
					.then((hwid: string | null) =>
						reviewService.reportUsage(apiKey, hwid, totals.usage_ms),
					)
					.catch(() => {});
			}
		} catch {
		...
		if (process.platform === 'darwin' || process.platform === 'win32') {
			...
			try {
				...
				console.error(
					'[main] Failed to stop StealthKeyboardManager during shutdown:',
					e,
				);
			}
		}
		...
		PhoneMirrorService.getInstance()
			.dispose()
			.catch((err) => console.error('[Main] PhoneMirror dispose failed:', err));
		...
		try {
			...
			console.error(
				'[Main] Failed to checkpoint/close database during shutdown:',
				e,
			);
		}
...
initializeApp().catch((err) => {
	...
	try {
		emergencyCloseDatabase('initializeApp-failed');
	} catch {
		/* best-effort */
	}
	const reportPath = isNativeArchGateCrash(err)
