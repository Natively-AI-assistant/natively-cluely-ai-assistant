...
- **Audio you capture, screen content, transcripts, notes, and meeting history are stored locally on your device**. Meeting metadata, transcripts, notes, and summaries are stored in a local SQLite database; saved meeting audio recordings are stored as local WAV files in the app data `recordings` folder. They are **not** uploaded to a Natively-operated server.
...
- **Audio captures** (microphone and system audio), including saved mixed meeting recordings in local WAV format.
...
- **Local SQLite database** containing meeting history, plus local recording files stored alongside the app's user data.
...
- **At rest (on your device):** local data (the SQLite meeting database, transcripts, saved meeting audio recordings, settings via `electron-store`) is **not** encrypted by Natively at the application layer. Please use full-disk encryption (FileVault on macOS, BitLocker on Windows, LUKS on Linux) to protect your local data.
