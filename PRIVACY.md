...
- **Screen content, transcripts, notes, and meeting history are stored locally on your device** in a SQLite database. Live audio is processed without being retained by default; if you explicitly enable local meeting recordings, the mixed WAV files are stored separately on this device. This content is **not** uploaded to a Natively-operated server.
...
- **Audio captures** (microphone and system audio). Live audio is not retained as a recording by default. If you explicitly enable **Save local meeting recordings**, Natively stores a mixed WAV on this device and deletes it when you delete the associated meeting history.
