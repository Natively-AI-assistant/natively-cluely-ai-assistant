...
describe('KeybindManager', () => {
    describe('DEFAULT_KEYBINDS', () => {
        ...
        it('all chat shortcuts use CommandOrControl modifier', () => {
            const chatKeybinds = DEFAULT_KEYBINDS.filter(kb => kb.id.startsWith('chat:'))
        ...
        it('includes toggle-visibility shortcut', () => {
            const toggle = DEFAULT_KEYBINDS.find(kb => kb.id === 'general:toggle-visibility')
            expect(toggle).toBeDefined()
            expect(toggle!.accelerator).toBe('CommandOrControl+B')
        })

        it('includes whatToAnswer shortcut', () => {
            const whatToAnswer = DEFAULT_KEYBINDS.find(kb => kb.id === 'chat:whatToAnswer')
        ...
        it('includes all chat mode shortcuts (1-7)', () => {
            for (let i = 1; i <= 7; i++) {
                const kb = DEFAULT_KEYBINDS.find(k => k.accelerator === `CommandOrControl+${i}`)
