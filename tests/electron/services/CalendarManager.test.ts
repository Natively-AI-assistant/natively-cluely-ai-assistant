...
describe('CalendarManager', () => {
    ...
    describe('init', () => {
        ...
        it('should load tokens from storage when available', () => {
            ...
            vi.mocked(safeStorage.decryptString).mockReturnValueOnce(JSON.stringify({
    ...
    describe('OAuth flow', () => {
        it('should reject when http server emits error', async () => {
            await expect(calendarManager.startAuthFlow()).rejects.toThrow('Server error')
        })
    })
    ...
    describe('event fetching', () => {
        it('should fetch events from Google Calendar API when connected', async () => {
            ...
            vi.mocked(axios.get).mockResolvedValueOnce({ data: { items: mockEvents } })
