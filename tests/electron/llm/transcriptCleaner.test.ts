...
const turn = (
  ...
  text: string,
  ts: number,
): TranscriptTurn => ({ role, text, timestamp: ts })
...
describe('transcriptCleaner', () => {
  describe('cleanTranscript', () => {
    ...
    it('removes repeated words', () => {
      ...
      const turns = [
        turn(
          'user',
          'yeah yeah the answer is definitely forty two and correct',
          1000,
        ),
      ]
      const result = cleanTranscript(turns)
    ...
    it('converts text to lowercase', () => {
      const turns = [
        turn(
          'user',
          'React is POWERFUL for building modern frontend applications',
          1000,
        ),
      ]
      const result = cleanTranscript(turns)
  ...
  describe('sparsifyTranscript', () => {
    ...
    it('prioritizes recent interviewer turns (last 6)', () => {
      ...
      const interviewerCount = result.filter(
    ...
    it('respects maxTurns parameter', () => {
      ...
      for (let i = 0; i < 20; i++) {
        turns.push(
          turn(
            'interviewer',
            `question about topic number ${i} that has enough content`,
            i * 1000,
          ),
        )
      }
