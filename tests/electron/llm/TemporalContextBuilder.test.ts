...
const ctxItem = (role: ContextItem['role'], text: string, ts: number): ContextItem => ({ role, text, timestamp: ts })
const response = (text: string, ts: number, question: string = 'q'): AssistantResponse => ({ text, timestamp: ts, questionContext: question })
...
describe('TemporalContextBuilder', () => {
    ...
    describe('buildTemporalContext', () => {
        ...
        it('detects "responding_to_user" role context', () => {
            const items = [
                ctxItem('user', 'What should I say', now - 5000),
            ]
            const result = buildTemporalContext(items, [])
        ...
        it('extracts technical tone signals', () => {
            const history = [
                response('We should implement the algorithm using a binary search tree for optimal performance', now - 5000),
                response('The function uses async await with a promise callback for the database query', now - 3000),
            ]
            const result = buildTemporalContext([], history)
            const technicalSignal = result.toneSignals.find(s => s.type === 'technical')
        ...
        it('extracts formal tone signals', () => {
            const history = [
                response('Therefore, I would recommend using the established pattern. Furthermore, it is important to note that the architecture supports this.', now - 5000),
                response('Consequently, the implementation should follow the guidelines. Moreover, this approach has been validated previously.', now - 3000),
            ]
            const result = buildTemporalContext([], history)
        ...
        it('extracts casual tone signals', () => {
            const history = [
                response('So basically you can just use the hook and it pretty much handles everything', now - 5000),
                response('Honestly, kind of easy actually. You literally just call the function.', now - 3000),
            ]
            const result = buildTemporalContext([], history)
        ...
        it('respects custom windowSeconds', () => {
            const items = [
                ctxItem('interviewer', 'question', now - 60_000),
            ]
    ...
    describe('formatTemporalContextForPrompt', () => {
        it('includes previous_responses section when responses exist', () => {
            const ctx = buildTemporalContext([], [response('first response', now - 5000)])
        ...
        it('includes tone_guidance when signals exist', () => {
            const history = [
                response('We should implement the algorithm using function and async await with component and module and database and architecture', now - 5000),
                response('The API uses async await promise callback for implementing the function and algorithm with database component', now - 3000),
            ]
