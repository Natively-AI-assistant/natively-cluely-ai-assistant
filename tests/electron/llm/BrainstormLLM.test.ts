...
describe('BrainstormLLM', () => {
    ...
    beforeEach(() => {
        const mock = createMockLlmHelperForTests({
            streamChunks: ['First approach', ': use brute force', '. Second approach', ': use DP.']
        })
    ...
    describe('generateStream', () => {
        it('yields chunks from the stream', async () => {
            const chunks: string[] = []
            for await (const chunk of brainstormLLM.generateStream('Solve two sum problem')) {
                chunks.push(chunk)
            }
            expect(chunks).toEqual(['First approach', ': use brute force', '. Second approach', ': use DP.'])
            expect(chunks.join('')).toBe('First approach: use brute force. Second approach: use DP.')
        })
        ...
        it('passes image paths to streamChat', async () => {
            const chunks: string[] = []
            for await (const chunk of brainstormLLM.generateStream('context', ['/img1.png', '/img2.png'])) {
                chunks.push(chunk)
            }
            expect(mockStreamChat).toHaveBeenCalledWith(
                'context',
        ...
        it('proceeds when context is empty but images are provided', async () => {
            const chunks: string[] = []
            for await (const chunk of brainstormLLM.generateStream('', ['/screenshot.png'])) {
                chunks.push(chunk)
            }
