...
describe('credentialLoader', () => {
  ...
  beforeEach(() => {
    ...
    mockLlmHelper.initModelVersionManager
      .mockClear()
      .mockResolvedValue(undefined)

    // Reset CredentialsManager mock to default values
    mockCredentialsManagerInstance.getGeminiApiKey.mockReturnValue(
      'test-gemini-key',
    )
    mockCredentialsManagerInstance.getGroqApiKey.mockReturnValue(
      'test-groq-key',
    )
    mockCredentialsManagerInstance.getOpenaiApiKey.mockReturnValue(
      'test-openai-key',
    )
    mockCredentialsManagerInstance.getClaudeApiKey.mockReturnValue(
      'test-claude-key',
    )
    mockCredentialsManagerInstance.getDefaultModel.mockReturnValue(
      'gemini-3.1-flash',
    )
  ...
  describe('loadStoredCredentials', () => {
    ...
    describe('API key loading', () => {
      ...
      it('loads Groq API key when available', () => {
        const appState = createAppState()
        loadStoredCredentials(mockLlmHelper as any, appState)
        expect(mockLlmHelper.setGroqApiKey).toHaveBeenCalledWith(
          'test-groq-key',
        )
      })
      ...
      it('loads OpenAI API key when available', () => {
        const appState = createAppState()
        loadStoredCredentials(mockLlmHelper as any, appState)
        expect(mockLlmHelper.setOpenaiApiKey).toHaveBeenCalledWith(
          'test-openai-key',
        )
      })
      ...
      it('loads Claude API key when available', () => {
        const appState = createAppState()
        loadStoredCredentials(mockLlmHelper as any, appState)
        expect(mockLlmHelper.setClaudeApiKey).toHaveBeenCalledWith(
          'test-claude-key',
        )
      })
    ...
    describe('language loading', () => {
      ...
      it('loads AI response language when available', () => {
        mockCredentialsManagerInstance.getAiResponseLanguage.mockReturnValue(
          'de',
        )
        const appState = createAppState()
        loadStoredCredentials(mockLlmHelper as any, appState)
    ...
    describe('model version manager initialization', () => {
      ...
      it('handles model version manager errors gracefully', async () => {
        mockLlmHelper.initModelVersionManager.mockRejectedValueOnce(
          new Error('Non-critical error'),
        )
        const appState = createAppState()
        // Should not throw
        expect(() =>
