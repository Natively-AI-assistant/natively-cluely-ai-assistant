...
describe('Config Snapshots', () => {
  ...
  it('AI response languages config matches snapshot', async () => {
    const { AI_RESPONSE_LANGUAGES } = await import(
      '../../../electron/config/languages'
    )
  ...
  it('default keybinds config matches snapshot', async () => {
    const { DEFAULT_KEYBINDS } = await import(
      '../../../electron/services/KeybindManager'
    )
