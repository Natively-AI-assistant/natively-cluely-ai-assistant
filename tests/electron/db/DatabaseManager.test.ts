...
import {
  DatabaseManager,
  type Meeting,
} from '../../../electron/db/DatabaseManager'
...
describe('DatabaseManager', () => {
  ...
  describe('saveMeeting', () => {
    ...
    it('should save a meeting with usage interactions', () => {
      ...
      const interactions = db
        .prepare('SELECT * FROM ai_interactions WHERE meeting_id = ?')
        .all('meeting-with-usage')
  ...
  describe('updateMeetingTitle', () => {
    it('should update meeting title', () => {
      ...
      const result = manager.updateMeetingTitle(
        'test-meeting-1',
        'Updated Title',
      )
  ...
  describe('updateMeetingSummary', () => {
    ...
    it('should return false for non-existent meeting', () => {
      const result = manager.updateMeetingSummary('non-existent', {
        overview: 'x',
      })
      expect(result).toBe(false)
    })
  ...
  describe('deleteMeeting', () => {
    it('should delete meeting and related data via cascade', () => {
      const meeting: Meeting = {
        ...
        duration: '1:00',
        summary: '',
        transcript: [{ speaker: 'Speaker', text: 'Hello', timestamp: 0 }],
      }
  ...
  describe('clearAllData', () => {
    it('should clear all data from tables', () => {
      const meeting: Meeting = {
        ...
        summary: '',
        transcript: [{ speaker: 'Speaker', text: 'Hello', timestamp: 0 }],
        usage: [{ type: 'assist', timestamp: Date.now() }],
      }
  ...
  describe('indexes', () => {
    it('should create indexes on foreign keys', () => {
      ...
      const idxChunksMeeting = db
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_chunks_meeting'",
        )
        .get()
      expect(idxChunksMeeting).toBeDefined()

      const idxTranscriptsMeeting = db
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_transcripts_meeting'",
        )
        .get()
      expect(idxTranscriptsMeeting).toBeDefined()

      const idxInteractionsMeeting = db
        .prepare(
          "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_ai_interactions_meeting'",
        )
        .get()
