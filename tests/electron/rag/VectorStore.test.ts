...
function insertTestMeeting(db: Database.Database, id: string, provider?: string) {
    db.prepare(
...
describe('VectorStore', () => {
    ...
    describe('saveChunks', () => {
        ...
        it('persists chunk data correctly', () => {
            ...
            const rows = db.prepare('SELECT * FROM chunks ORDER BY chunk_index').all() as any[]
            expect(rows).toHaveLength(2)
            expect(rows[0].speaker).toBe('Alice')
            expect(rows[1].speaker).toBe('Bob')
            expect(rows[0].cleaned_text).toBe('This is chunk number 0 with some content for testing.')
        })
    ...
    describe('getChunksForMeeting', () => {
        it('returns all chunks for a meeting ordered by chunk_index', () => {
            ...
            expect(result[0].text).toBe('This is chunk number 0 with some content for testing.')
        })
    ...
    describe('storeEmbedding', () => {
        it('stores embedding as BLOB in chunks table', () => {
            ...
            const row = db.prepare('SELECT embedding FROM chunks WHERE id = 1').get() as any
    ...
    describe('saveSummary / storeSummaryEmbedding', () => {
        it('saves a summary for a meeting', () => {
            ...
            const row = db.prepare('SELECT * FROM chunk_summaries WHERE meeting_id = ?').get('meeting-1') as any
        ...
        it('overwrites existing summary (INSERT OR REPLACE)', () => {
            ...
            const rows = db.prepare('SELECT * FROM chunk_summaries WHERE meeting_id = ?').all('meeting-1') as any[]
        ...
        it('stores summary embedding as BLOB', () => {
            ...
            const row = db.prepare('SELECT embedding FROM chunk_summaries WHERE meeting_id = ?').get('meeting-1') as any
            expect(row.embedding).toBeInstanceOf(Buffer)
            expect(row.embedding.byteLength).toBe(12) // 3 floats * 4 bytes
        })
    })
    ...
    describe('deleteChunksForMeeting', () => {
        it('deletes all chunks for a meeting', () => {
            ...
            const rows = db.prepare('SELECT * FROM chunks WHERE meeting_id = ?').all('meeting-1')
    ...
    describe('clearEmbeddingsForMeeting', () => {
        it('nullifies embeddings but keeps chunks', () => {
            ...
            const rows = db.prepare('SELECT embedding FROM chunks WHERE meeting_id = ?').all('meeting-1') as any[]
        ...
        it('clears meeting provider metadata', () => {
            ...
            store.clearEmbeddingsForMeeting('meeting-1')
            const meeting = db.prepare('SELECT embedding_provider, embedding_dimensions FROM meetings WHERE id = ?').get('meeting-1') as any
    ...
    describe('getIncompatibleMeetingsCount', () => {
        ...
        it('excludes unprocessed meetings', () => {
            db.prepare('INSERT INTO meetings (id, title, is_processed, embedding_provider) VALUES (?, ?, 0, ?)').run('m1', 'M1', 'openai')
            insertTestMeeting(db, 'm2', 'openai')
    ...
    describe('deleteEmbeddingsForMeetings', () => {
        ...
        it('nullifies embeddings for incompatible meetings', () => {
            ...
            const rows = db.prepare('SELECT embedding FROM chunks WHERE meeting_id = ?').all('m1') as any[]
            expect(rows[0].embedding).toBeNull()
            expect(rows[1].embedding).toBeNull()
        })
        ...
        it('clears provider metadata from meetings', () => {
            insertTestMeeting(db, 'm1', 'openai')
            store.deleteEmbeddingsForMeetings('gemini')
            const meeting = db.prepare('SELECT embedding_provider FROM meetings WHERE id = ?').get('m1') as any
            expect(meeting.embedding_provider).toBeNull()
        })
        ...
        it('nullifies summary embeddings for incompatible meetings', () => {
            ...
            store.deleteEmbeddingsForMeetings('gemini')
            const row = db.prepare('SELECT embedding FROM chunk_summaries WHERE meeting_id = ?').get('m1') as any
            expect(row.embedding).toBeNull()
        })
    })
    ...
    describe('embeddingToBlob (via storeEmbedding)', () => {
        it('correctly encodes Float32 values', () => {
            insertTestMeeting(db, 'meeting-1')
            store.saveChunks(makeChunks(1))
            const embedding = [1.0, -0.5, 0.0, 3.14159]
            store.storeEmbedding(1, embedding)
            const row = db.prepare('SELECT embedding FROM chunks WHERE id = 1').get() as any
            ...
            expect(buf.readFloatLE(12)).toBeCloseTo(3.14159, 3)
        })
