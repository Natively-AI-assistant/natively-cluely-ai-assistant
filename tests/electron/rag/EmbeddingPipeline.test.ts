...
describe('EmbeddingPipeline', () => {
    ...
    describe('isReady', () => {
        ...
        it('returns true after successful initialization', async () => {
            ...
            vi.mocked(EmbeddingProviderResolver.resolve).mockResolvedValue(mockProvider as any)
    ...
    describe('initialize', () => {
        it('initializes with a resolved provider', async () => {
            ...
            const row = db.prepare("SELECT value FROM app_state WHERE key = 'last_embedding_provider'").get() as any
        ...
        it('re-initializes if ollamaUrl is newly provided', async () => {
            ...
            await pipeline.initialize({ openaiKey: 'key1' })
            vi.mocked(EmbeddingProviderResolver.resolve).mockClear()
            await pipeline.initialize({ openaiKey: 'key1', ollamaUrl: 'http://localhost:11434' })
            expect(EmbeddingProviderResolver.resolve).toHaveBeenCalledTimes(1)
        })
    })
    ...
    describe('waitForReady', () => {
        ...
        it('throws if not initialized and no initPromise', async () => {
            await expect(pipeline.waitForReady(100)).rejects.toThrow('Embedding pipeline has not been initialized')
        })
        ...
        it('waits for in-progress initialization', async () => {
            ...
            vi.mocked(EmbeddingProviderResolver.resolve).mockReturnValue(providerPromise as any)
        ...
        it('times out if initialization takes too long', async () => {
            vi.mocked(EmbeddingProviderResolver.resolve).mockReturnValue(new Promise(() => {}) as any) // never resolves
    ...
    describe('queueMeeting', () => {
        ...
        it('queues chunks and summary when chunks exist', async () => {
            ...
            const queueItems = db.prepare('SELECT * FROM embedding_queue ORDER BY id').all() as any[]
        ...
        it('uses INSERT OR IGNORE for duplicate chunk entries', async () => {
            ...
            const chunkItems = db.prepare('SELECT * FROM embedding_queue WHERE chunk_id IS NOT NULL').all()
    ...
    describe('getQueueStatus', () => {
        it('returns zero counts for empty queue', async () => {
            ...
            expect(status).toEqual({ pending: 0, processing: 0, completed: 0, failed: 0 })
        })
        ...
        it('counts items by status', async () => {
            ...
            await pipeline.initialize({ openaiKey: 'test-key' })
            insertTestMeeting(db, 'meeting-1')
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status) VALUES ('meeting-1', 1, 'pending')").run()
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status) VALUES ('meeting-1', 2, 'completed')").run()
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status) VALUES ('meeting-1', 3, 'failed')").run()
            const status = pipeline.getQueueStatus()
        ...
        it('counts stalled items (retry_count >= MAX_RETRIES) as failed', async () => {
            ...
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status, retry_count) VALUES ('meeting-1', 1, 'pending', 3)").run()
            const status = pipeline.getQueueStatus()
        ...
        it('does not count fallback sentinel items (retry_count=-1) as stalled', async () => {
            ...
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status, retry_count) VALUES ('meeting-1', 1, 'pending', -1)").run()
            const status = pipeline.getQueueStatus()
            expect(status.pending).toBe(1)
    ...
    describe('cleanupQueue', () => {
        it('removes completed items older than specified days', async () => {
            ...
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status, processed_at) VALUES ('meeting-1', 1, 'completed', ?)").run(oldDate)
            // Recent completed item
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status, processed_at) VALUES ('meeting-1', 2, 'completed', ?)").run(new Date().toISOString())
            // Pending item (should not be deleted regardless of age)
            db.prepare("INSERT INTO embedding_queue (meeting_id, chunk_id, status, processed_at) VALUES ('meeting-1', 3, 'pending', ?)").run(oldDate)

            pipeline.cleanupQueue(7) // Remove items older than 7 days

            const remaining = db.prepare('SELECT * FROM embedding_queue ORDER BY id').all() as any[]
    ...
    describe('getEmbedding', () => {
        ...
        it('throws when not initialized', async () => {
            await expect(pipeline.getEmbedding('test')).rejects.toThrow('Embedding provider not initialized')
        })
    })
    ...
    describe('getEmbeddingForQuery', () => {
        ...
        it('throws when not initialized', async () => {
            await expect(pipeline.getEmbeddingForQuery('test')).rejects.toThrow('Embedding provider not initialized')
        })
    })
})
