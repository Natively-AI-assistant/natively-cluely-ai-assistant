...
import { preprocessTranscript, estimateTokens, type RawSegment } from '../../../electron/rag/TranscriptPreprocessor'

const raw = (speaker: string, text: string, timestamp: number): RawSegment => ({ speaker, text, timestamp })
...
describe('TranscriptPreprocessor', () => {
    ...
    describe('preprocessTranscript', () => {
        ...
        it('does NOT merge segments with >5s gap', () => {
            const segments = [
                raw('Alice', 'First statement about the project architecture and design patterns.', 1000),
                raw('Alice', 'Second statement about different topic that came much later in conversation.', 8000), // 7s gap
            ]
            const result = preprocessTranscript(segments)
        ...
        it('does NOT merge segments from different speakers', () => {
            const segments = [
                ...
                raw('Bob', 'Bob responds with his own perspective on the same topic.', 2000),
            ]
            const result = preprocessTranscript(segments)
        ...
        it('removes filler words', () => {
            const segments = [raw('user', 'um yeah so like the answer is forty two and that is correct', 1000)]
            const result = preprocessTranscript(segments)
        ...
        it('detects questions (ends with ?)', () => {
            const segments = [raw('interviewer', 'Can you explain how react hooks work in detail?', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isQuestion).toBe(true)
        })

        it('detects questions (starts with question word)', () => {
            const segments = [raw('interviewer', 'What is your experience with typescript and its advanced features', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isQuestion).toBe(true)
        })

        it('does not flag non-questions', () => {
            const segments = [raw('interviewer', 'Tell me about your experience with react and frontend development', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isQuestion).toBe(false)
        })

        it('detects decisions', () => {
            const segments = [raw('interviewer', 'We decided to go with react for this project', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isDecision).toBe(true)
        })

        it('detects "agreed" as decision', () => {
            const segments = [raw('interviewer', 'Agreed that typescript is the best choice here', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isDecision).toBe(true)
        })

        it('detects action items', () => {
            const segments = [raw('interviewer', 'You will need to implement the auth module by next week', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isActionItem).toBe(true)
        })

        it('detects deadline patterns as action items', () => {
            const segments = [raw('interviewer', 'Please finish the api integration by end of day', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].isActionItem).toBe(true)
        })
        ...
        it('normalizes speaker labels', () => {
            const segments = [
                raw('user', 'This is my response about the topic and I think we should proceed', 1000),
                raw('interviewer', 'This is the interviewer speaking about something important here', 2000),
                raw('assistant', 'This is the assistant response with helpful information included', 3000),
            ]
            const result = preprocessTranscript(segments)
        ...
        it('preserves custom speaker names', () => {
            const segments = [raw('Alice', 'Alice has a very detailed opinion about the architecture and design choices', 1000)]
            const result = preprocessTranscript(segments)
            expect(result[0].speaker).toBe('Alice')
        })
