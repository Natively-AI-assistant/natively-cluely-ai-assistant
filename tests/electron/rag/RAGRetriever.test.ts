...
import { RAGRetriever, type QueryIntent } from '../../../electron/rag/RAGRetriever'
...
describe('RAGRetriever', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks()
        retriever = new RAGRetriever(mockVectorStore as any, mockEmbeddingPipeline as any)
    })

    describe('detectIntent', () => {
        it('detects "What did we decide?" as decision_recall', () => {
            expect(retriever.detectIntent('What did we decide?')).toBe('decision_recall')
        })

        it('detects "What did we agree on?" as decision_recall', () => {
            expect(retriever.detectIntent('What did we agree on?')).toBe('decision_recall')
        })

        it('detects "Did we agree on a timeline?" as decision_recall', () => {
            expect(retriever.detectIntent('Did we agree on a timeline?')).toBe('decision_recall')
        })

        it('detects "What did he say about React?" as speaker_lookup', () => {
            expect(retriever.detectIntent('What did he say about React?')).toBe('speaker_lookup')
        })

        it('detects "Who said we should use TypeScript?" as speaker_lookup', () => {
            expect(retriever.detectIntent('Who said we should use TypeScript?')).toBe('speaker_lookup')
        })

        it('detects "What are my action items?" as action_items', () => {
            expect(retriever.detectIntent('What are my action items?')).toBe('action_items')
        })

        it('detects "What should I do next?" as action_items', () => {
            expect(retriever.detectIntent('What should I do next?')).toBe('action_items')
        })
        ...
        it('returns open_question for generic questions', () => {
            expect(retriever.detectIntent('How does React work?')).toBe('open_question')
        })
    ...
    describe('detectScope', () => {
        it('detects "this meeting" as meeting scope', () => {
            expect(retriever.detectScope('what happened in this meeting')).toBe('meeting')
        })
        ...
        it('detects "all meetings" as global scope', () => {
            expect(retriever.detectScope('search all meetings for React')).toBe('global')
        })

        it('detects "when did we" as global scope', () => {
            expect(retriever.detectScope('when did we discuss TypeScript')).toBe('global')
        })

        it('detects "find" as global scope', () => {
            expect(retriever.detectScope('find all discussions about API')).toBe('global')
        })

        it('defaults to meeting when currentMeetingId provided', () => {
            expect(retriever.detectScope('tell me about components', 'meeting-123')).toBe('meeting')
        })
        ...
        it('meeting patterns take precedence over global', () => {
            ...
            expect(retriever.detectScope('what did they say in the last meeting')).toBe('meeting')
        })
    })
})
