...
const mockLlmInstance = {
  ...
  generateRollingScript: vi.fn(() => Promise.resolve({
  ...
  debugSolutionWithImages: vi.fn(() => Promise.resolve({
...
describe('ProcessingHelper', () => {
  ...
  describe('processScreenshots - queue view with screenshots', () => {
    beforeEach(() => {
      mockAppState.getView.mockReturnValue('queue');
      mockAppState.getScreenshotHelper.mockReturnValue({
        getScreenshotQueue: vi.fn(() => ['/tmp/screenshot1.png', '/tmp/screenshot2.png']),
        getExtraScreenshotQueue: vi.fn(() => []),
      });
    });
    ...
    it('sends INITIAL_START event when processing begins', async () => {
      ...
      expect(mockMainWindow.webContents.send).toHaveBeenCalledWith('initial-start');
    });
    ...
    it('handles LLM errors gracefully', async () => {
      const llmHelper = processingHelper.getLLMHelper();
      llmHelper.generateRollingScript.mockRejectedValueOnce(new Error('API Error'));
  ...
  describe('processScreenshots - solutions view (debug mode)', () => {
    ...
    it('sends DEBUG_START event when processing extra screenshots', async () => {
      ...
      expect(mockMainWindow.webContents.send).toHaveBeenCalledWith('debug-start');
    });
    ...
    it('sends DEBUG_ERROR event when debug fails', async () => {
      const llmHelper = processingHelper.getLLMHelper();
      llmHelper.debugSolutionWithImages.mockRejectedValueOnce(new Error('Vision API Error'));
    ...
    it('sends NO_SCREENSHOTS when extra queue is empty in solutions view', async () => {
      ...
      expect(mockMainWindow.webContents.send).toHaveBeenCalledWith('processing-no-screenshots');
    });
  ...
  describe('cancelOngoingRequests', () => {
    ...
    it('cancels extra processing abort controller', async () => {
      ...
      llmHelper.generateSolution.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve({ solution: { code: 'test' } }), 100))
      );
  ...
  describe('loadStoredCredentials', () => {
    ...
    it('loads credentials from CredentialsManager into llmHelper', () => {
      ...
      expect(mockLlmInstance.setGroqApiKey).toHaveBeenCalledWith('fake-groq-key');
      expect(mockLlmInstance.setOpenaiApiKey).toHaveBeenCalledWith('fake-openai-key');
      expect(mockLlmInstance.setClaudeApiKey).toHaveBeenCalledWith('fake-claude-key');
  ...
  describe('state management via appState', () => {
    ...
    it('appState PROCESSING_EVENTS are accessible', () => {
      expect(mockAppState.PROCESSING_EVENTS.NO_SCREENSHOTS).toBe('processing-no-screenshots');
      expect(mockAppState.PROCESSING_EVENTS.INITIAL_START).toBe('initial-start');
      expect(mockAppState.PROCESSING_EVENTS.INITIAL_SOLUTION_ERROR).toBe('solution-error');
      ...
      expect(mockAppState.PROCESSING_EVENTS.PROBLEM_EXTRACTED).toBe('problem-extracted');
      expect(mockAppState.PROCESSING_EVENTS.SOLUTION_SUCCESS).toBe('solution-success');
    });
  ...
  describe('processScreenshots - return value and promise behavior', () => {
    it('processScreenshots returns a promise that resolves', async () => {
      ...
      await expect(processingHelper.processScreenshots()).resolves.toBeUndefined();
    });
  ...
  describe('error handling edge cases', () => {
    ...
    it('handles generateSolution error in debug mode', async () => {
      ...
      const llmHelper = processingHelper.getLLMHelper();
      llmHelper.generateSolution.mockRejectedValueOnce(new Error('Solution generation failed'));
...
describe('ProcessingHelper - multiple screenshots edge cases', () => {
  ...
  it('processes multiple screenshots in order', async () => {
    mockAppState.getView.mockReturnValue('queue');
    const screenshots = ['/tmp/screenshot1.png', '/tmp/screenshot2.png', '/tmp/screenshot3.png'];
    mockAppState.getScreenshotHelper.mockReturnValue({
  ...
  it('handles extra queue screenshots in debug mode', async () => {
    ...
    mockAppState.getScreenshotHelper.mockReturnValue({
      getScreenshotQueue: vi.fn(() => []),
      getExtraScreenshotQueue: vi.fn(() => ['/tmp/extra1.png', '/tmp/extra2.png']),
    });
...
describe('ProcessingHelper - event emission verification', () => {
  ...
  it('emits INITIAL_START before setting view', async () => {
    ...
    const initialStartIndex = calls.findIndex(call => call[0] === 'initial-start');
  ...
  it('emits PROBLEM_EXTRACTED after setProblemInfo', async () => {
    ...
    const calls = mockMainWindow.webContents.send.mock.calls;
    const problemExtractedCall = calls.find(call => call[0] === 'problem-extracted');
