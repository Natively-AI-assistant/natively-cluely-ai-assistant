...
import { SessionTracker, type TranscriptSegment } from '../../electron/SessionTracker'
...
describe('SessionTracker', () => {
    ...
    describe('mapSpeakerToRole', () => {
        ...
        it('maps anything else to "interviewer"', () => {
            expect((tracker as any).mapSpeakerToRole('interviewer')).toBe('interviewer')
    ...
    describe('looksLikeCodingQuestion (via handleTranscript)', () => {
        ...
        it('detects coding questions with algorithm patterns', () => {
            const seg = makeSegment('Implement an algorithm to find the maximum element in an array using binary search approach')
        ...
        it('rejects casual conversation', () => {
            const seg = makeSegment('That is a great point and I agree with your assessment of the situation completely today')
            tracker.handleTranscript(seg)
            const result = tracker.getDetectedCodingQuestion()
            expect(result.question).toBeNull()
        })
        ...
        it('detects function + complexity patterns', () => {
            const seg = makeSegment('Write a function that processes the array with O(n) time complexity and returns the sorted result')
            tracker.handleTranscript(seg)
            const result = tracker.getDetectedCodingQuestion()
            expect(result.question).toBeTruthy()
        })
        ...
        it('only processes final segments', () => {
            const seg = makeSegment('Implement an algorithm to find the maximum in the given array of integers')
            seg.final = false
            tracker.handleTranscript(seg)
            const result = tracker.getDetectedCodingQuestion()
            expect(result.question).toBeNull()
    ...
    describe('addAssistantMessage', () => {
        it('stores last assistant message', () => {
            tracker.addAssistantMessage('Here is a detailed and helpful response about the topic you asked about')
            expect(tracker.getLastAssistantMessage()).toContain('detailed and helpful response')
        })
        ...
        it('ignores "I\'m not sure" fallback messages', () => {
            tracker.addAssistantMessage("I'm not sure about that answer but I can try to help")
            expect(tracker.getLastAssistantMessage()).toBeNull()
        })
        ...
        it('tracks response history', () => {
            tracker.addAssistantMessage('First detailed and meaningful response about the topic')
            tracker.addAssistantMessage('Second detailed and meaningful response about the topic')
        ...
        it('caps response history at 10', () => {
            for (let i = 0; i < 15; i++) {
                tracker.addAssistantMessage(`Response number ${i} with enough content to pass filters`)
            }
            const history = tracker.getAssistantResponseHistory()
    ...
    describe('context management', () => {
        ...
        it('addTranscript deduplicates identical segments', () => {
            const now = Date.now()
            tracker.addTranscript({ speaker: 'interviewer', text: 'Hello there this is a test message', timestamp: now, final: true })
            const result = tracker.addTranscript({ speaker: 'interviewer', text: 'Hello there this is a test message', timestamp: now, final: true })
            expect(result).toBeNull()
        })
        ...
        it('getContext returns items within time window', () => {
            vi.useFakeTimers()
            const now = Date.now()
            tracker.addTranscript({ speaker: 'interviewer', text: 'Recent question about the topic', timestamp: now, final: true })
            vi.advanceTimersByTime(5000)
            tracker.addTranscript({ speaker: 'user', text: 'Recent answer to the question', timestamp: now + 5000, final: true })
        ...
        it('getFormattedContext formats with labels', () => {
            tracker.addTranscript({ speaker: 'interviewer', text: 'Question about React', timestamp: Date.now(), final: true })
            tracker.addTranscript({ speaker: 'user', text: 'Answer about React', timestamp: Date.now(), final: true })
        ...
        it('getLastInterviewerTurn returns last interviewer text', () => {
            const now = Date.now()
            tracker.addTranscript({ speaker: 'user', text: 'some user text here that is long enough', timestamp: now, final: true })
            tracker.addTranscript({ speaker: 'interviewer', text: 'the last interviewer question about the topic', timestamp: now + 1000, final: true })
            expect(tracker.getLastInterviewerTurn()).toBe('the last interviewer question about the topic')
        })

        it('getLastInterviewerTurn returns null when no interviewer turns', () => {
            tracker.addTranscript({ speaker: 'user', text: 'only user text here that is long enough', timestamp: Date.now(), final: true })
    ...
    describe('reset', () => {
        it('clears all state', () => {
            tracker.addTranscript({ speaker: 'interviewer', text: 'question here', timestamp: 1000, final: true })
