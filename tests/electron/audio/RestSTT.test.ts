...
describe('RestSTT', () => {
  ...
  describe('lifecycle', () => {
    ...
    it('stop flushes remaining audio', async () => {
      ...
      expect(spy).toHaveBeenCalledWith({ text: 'flushed', isFinal: true, confidence: 1.0 });
    });
  ...
  describe('notifySpeechEnded()', () => {
    it('triggers upload when buffer is large enough', async () => {
      ...
      expect(spy).toHaveBeenCalledWith({ text: 'Hello', isFinal: true, confidence: 1.0 });
      stt.stop();
    });
  ...
  describe('provider configs', () => {
    ...
    it.each([
      ['azure',     'https://eastus.stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1?language=en-US'],
      ['ibmwatson', 'https://api.us-south.speech-to-text.watson.cloud.ibm.com/v1/recognize?language=en-US'],
    ...
    it('azure uses binary upload with Ocp-Apim-Subscription-Key header', async () => {
      ...
      expect(mockAxiosPost).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ 'Ocp-Apim-Subscription-Key': 'my-key' })
        })
      );
      stt.stop();
    });
  ...
  describe('safety-net timer', () => {
    it('auto-flushes after 10s of continuous speech', async () => {
      ...
      expect(spy).toHaveBeenCalledWith({ text: 'auto-flushed', isFinal: true, confidence: 1.0 });
      stt.stop();
    });
  });
});
