...
vi.mock('ws', () => {
    ...
    class MockWebSocket {
        ...
        _triggerOpen() { this._trigger('open'); }
        ...
        static _triggerOpen() { /* no-op for static — use instance */ }
        static _triggerMessage(_data: string) { /* no-op — use instance */ }
        static _triggerError(_err: Error) { /* no-op — use instance */ }
        static _triggerClose(_code: number) { /* no-op — use instance */ }
    }

    return { default: MockWebSocket, mockSend, mockClose, getLastInstance: () => lastInstance };
});
...
import { mockSend, mockClose, getLastInstance } from 'ws';
...
describe('NativelyProSTT', () => {
    ...
    describe('server handshake', () => {
        it('sends auth frame on WebSocket open', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            expect(mockSend).toHaveBeenCalledWith(expect.stringContaining('"key":"my-key"'));
            stt.stop();
        });
        ...
        it('sends sample_rate in auth frame', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            expect(mockSend).toHaveBeenCalledWith(expect.stringContaining('"sample_rate":44100'));
            stt.stop();
        });
        ...
        it('sends audio_channels in auth frame', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            expect(mockSend).toHaveBeenCalledWith(expect.stringContaining('"audio_channels":2'));
            stt.stop();
        });
        ...
        it('sends language in auth frame', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            expect(mockSend).toHaveBeenCalledWith(expect.stringContaining('"language":"en-US"'));
            stt.stop();
        });
        ...
        it('sends auto language when set to auto mode', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            expect(mockSend).toHaveBeenCalledWith(expect.stringContaining('"language":"auto"'));
            stt.stop();
        });
        ...
        it('sends trial_token when key is __trial__ sentinel', () => {
            ...
            expect(mockSend).toHaveBeenCalled();
            expect(mockSend).toHaveBeenCalledWith(expect.not.stringContaining('"key":"__trial__"'));
            stt.stop();
        });
    });
    ...
    describe('setRecognitionLanguage()', () => {
        ...
        it('warns on unknown language key', () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Unknown language key'));
            warnSpy.mockRestore();
        });
    });
    ...
    describe('events', () => {
        it('emits transcript event on server text message', () => {
            ...
            inst._triggerMessage(JSON.stringify({ status: 'connected', provider: 'deepgram' }));
            expect(spy).not.toHaveBeenCalled();

            inst._triggerMessage(JSON.stringify({ text: 'Hello world', is_final: true }));
            expect(spy).toHaveBeenCalledWith({ text: 'Hello world', isFinal: true, confidence: 1.0 });
        ...
        it('emits error event on server error message', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            getLastInstance()!._triggerMessage(JSON.stringify({ error: 'auth_timeout' }));
        ...
        it('emits languageDetected on auto-detect message', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            getLastInstance()!._triggerMessage(JSON.stringify({ language_detected: 'fr-FR' }));
        ...
        it('emits transcript with confidence from server message', () => {
            ...
            vi.advanceTimersByTime(3000);
            getLastInstance()!._triggerOpen();
            getLastInstance()!._triggerMessage(JSON.stringify({ text: 'Hello', is_final: true, confidence: 0.85 }));

            expect(spy).toHaveBeenCalledWith({ text: 'Hello', isFinal: true, confidence: 0.85 });
    ...
    describe('reconnect', () => {
        ...
        it('emits error after close when reconnect limit exhausted', () => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                const reconnectDelay = 1500 * Math.pow(2, Math.min(i, 4));
