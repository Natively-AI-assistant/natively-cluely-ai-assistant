...
vi.mock('ws', () => {
  ...
  class FakeWs extends EventEmitter {
    ...
    readyState = 0; url: string; headers: Record<string, any>;
    send = vi.fn(); close = vi.fn(); ping = vi.fn(); sentMessages: any[] = [];
    constructor(url: string, opts?: any) {
      super();
      this.url = url; this.headers = opts?.headers || {};
      this.send = vi.fn((data: any) => this.sentMessages.push(data));
      wsInstances.push(this);
      setTimeout(() => { if (this.readyState === 0) { this.readyState = 1; this.emit('open'); } }, 5);
    }
  }
...
vi.mock('fs', () => ({
  ...
  default: { createWriteStream: vi.fn(() => ({ write: vi.fn(), end: vi.fn() })) },
}));

import { ElevenLabsStreamingSTT } from '../../../electron/audio/ElevenLabsStreamingSTT';

function getWs() { return wsInstances[wsInstances.length - 1]; }
...
describe('ElevenLabsStreamingSTT', () => {
  ...
  it('emits partial_transcript as interim', async () => {
    ...
    expect(spy).toHaveBeenCalledWith({ text: 'Hello', isFinal: false, confidence: 1.0 });
  });
  ...
  it('emits committed_transcript as final', async () => {
    ...
    getWs().emit('message', JSON.stringify({
      type: 'committed_transcript', text: 'Hello world',
    }));
