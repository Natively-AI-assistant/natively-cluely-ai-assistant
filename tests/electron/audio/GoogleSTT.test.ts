...
vi.mock('@google-cloud/speech', () => {
  ...
  SpeechClientMock.prototype.streamingRecognize = vi.fn(() =>
    createMockStream(),
  )
...
describe('GoogleSTT', () => {
  ...
  beforeEach(() => {
    ...
    proto.streamingRecognize.mockImplementation(() => createMockStream())
    stt = new GoogleSTT()
  })
  ...
  describe('start()', () => {
    it('creates a streaming recognize connection', () => {
      stt.start()
      expect(
        (SpeechClient.prototype as any).streamingRecognize,
      ).toHaveBeenCalledTimes(1)
    })

    it('passes correct audio configuration', () => {
      stt.start()
      const config = (SpeechClient.prototype as any).streamingRecognize.mock
        .calls[0][0]
  ...
  describe('transcript parsing', () => {
    ...
    it('emits transcript event for interim results', () => {
      ...
      currentStream.emit('data', {
        results: [
          {
            alternatives: [{ transcript: 'Hel', confidence: 0.4 }],
            isFinal: false,
          },
        ],
      })
    ...
    it('ignores empty transcripts', () => {
      ...
      currentStream.emit('data', {
        results: [
          { alternatives: [{ transcript: '', confidence: 0 }], isFinal: true },
        ],
      })
  ...
  describe('configuration', () => {
    it('setSampleRate restarts stream when active', () => {
      stt.start()
      const count = (SpeechClient.prototype as any).streamingRecognize.mock
        .calls.length
      stt.setSampleRate(44100)

      expect(
        (SpeechClient.prototype as any).streamingRecognize.mock.calls.length,
      ).toBeGreaterThan(count)
      const newConfig = (SpeechClient.prototype as any).streamingRecognize.mock
        .calls[count][0]
