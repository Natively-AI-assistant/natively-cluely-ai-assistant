...
//   1. undetectable on            → dashed 2px ring at offset -5px, in every
...
//
// The offset assertion is load-bearing too: this window is exactly the shell
// card (p-0, h-fit, flush to the window's top and bottom edges), so a POSITIVE
// outline-offset would be clipped away by the native window bounds and the
// ring would silently lose its top and bottom edges.
  ...
  try {
    ...
    for (const colorTheme of COLOR_THEMES) {
      for (const it of INTERFACE_THEMES) {
        ...
        // Negative, because the card is flush to the native window's top and
        // bottom edges — a positive offset is clipped away by the window.
        check(
          on.offset === '-5px',
          `${where}: outline-offset resolved "${on.offset}", expected "-5px" — a ` +
            `non-negative offset is clipped by the overlay window bounds and the ` +
            `ring loses its top and bottom edges`,
        );
    ...
    console.log(
      `✓ undetectable-ring check passed (dashed 2px ring at offset -5px in ` +
