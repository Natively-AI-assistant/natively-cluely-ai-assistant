...
import { chunkTranscript } from '../../electron/rag/SemanticChunker'
...
describe('RAG Pipeline Integration', () => {
  ...
  describe('SemanticChunker', () => {
    it('chunks transcript text into meaningful segments', () => {
      const segments = [
        createSegment(
          'Alice',
          'We need to discuss the new architecture for the React components and how they will interact with the backend API services.',
          0,
          5000,
        ),
        createSegment(
          'Alice',
          'I think we should use a micro-frontend approach where each team owns their own bounded context and deploys independently.',
          5000,
          10000,
        ),
        createSegment(
          'Bob',
          'That makes sense. What about shared state management? We need a consistent approach across all the micro-frontends.',
          10000,
          15000,
        ),
        createSegment(
          'Bob',
          'I suggest we use a shared event bus pattern for cross-context communication while keeping local state within each context.',
          15000,
          20000,
        ),
        createSegment(
          'Alice',
          'Agreed. Lets draft a proposal and review it in the next sprint planning meeting.',
          20000,
          25000,
        ),
      ]
    ...
    it('respects max chunk size', () => {
      ...
      for (let i = 0; i < 20; i++) {
        segments.push(
          createSegment(
            'Alice',
            `This is segment number ${i} with enough content to be meaningful. We discuss various topics including architecture, design patterns, and implementation details for our project.`,
            time,
            time + 3000,
          ),
        )
  ...
  describe('VectorStore', () => {
    it('stores and retrieves vectors', () => {
      ...
      const segments = [
        createSegment(
          'Alice',
          'We decided to use PostgreSQL for the new analytics pipeline because it handles time-series data well.',
          0,
          5000,
        ),
        createSegment(
          'Bob',
          'I agree. We should also set up automated backups and monitoring for the database cluster.',
          5000,
          10000,
        ),
        createSegment(
          'Alice',
          'Lets assign the database setup task to the infrastructure team by end of week.',
          10000,
          15000,
        ),
      ]
  ...
  describe('Full pipeline: chunk → embed → store → retrieve', () => {
    it('processes text through the full pipeline', () => {
      ...
      const churnRelated = retrieved.filter(
        (c) =>
      ...
      const revenueRelated = retrieved.filter(
        (c) =>
      ...
      for (let i = 1; i < retrieved.length; i++) {
        expect(retrieved[i].chunkIndex).toBeGreaterThan(
          retrieved[i - 1].chunkIndex,
        )
      }

      for (let i = 1; i < retrieved.length; i++) {
        expect(retrieved[i].startMs).toBeGreaterThanOrEqual(
          retrieved[i - 1].startMs,
        )
      }
    })
    ...
    it('handles pipeline with single-segment transcript', () => {
      ...
      const transcript = [
        createSegment('Alice', 'Single statement meeting.', 0, 5000),
      ]
      const chunks = chunkTranscript(meetingId, transcript)
