...
vi.mock('../../../src/hooks/useShortcuts', () => ({
  useShortcuts: vi.fn(() => ({ keybinds: {}, isShortcutPressed: vi.fn(() => false) })),
}))
...
vi.mock('../../../src/UI_comp/mainui.png', () => ({ default: '/mock-mainui.png' }))
vi.mock('../../../src/UI_comp/calender.png', () => ({ default: '/mock-calendar.png' }))
...
vi.mock('../../../src/components/FeatureSpotlight', () => ({
  __esModule: true,
  FeatureSpotlight: () => <div data-testid="feature-spotlight">FeatureSpotlight</div>,
}))

vi.mock('../../../src/components/ui/ConnectCalendarButton', () => ({
  __esModule: true,
  default: () => <div data-testid="connect-calendar">ConnectCalendarButton</div>,
}))
...
describe('Launcher Component', () => {
  ...
  beforeEach(() => {
    ...
    ;(window.electronAPI.getRecentMeetings as any).mockResolvedValue([
      { id: '1', title: 'Test Meeting 1', date: '2026-03-28', duration: '3600', summary: 'Summary 1' },
      { id: '2', title: 'Test Meeting 2', date: '2026-03-27', duration: '1800', summary: 'Summary 2' },
    ])
  })
  ...
  it('renders launcher component without crashing', () => {
    ...
    const { container } = render(<Launcher onStartMeeting={mockOnStartMeeting} onOpenSettings={mockOnOpenSettings} />)
    expect(container).toBeInTheDocument()
  })

  it('renders meeting list when getRecentMeetings returns data', async () => {
    render(<Launcher onStartMeeting={mockOnStartMeeting} onOpenSettings={mockOnOpenSettings} />)
  ...
  it('calls onStartMeeting when start meeting is triggered', () => {
    ...
    const startMeetingBtn = buttons.find(btn => btn.textContent?.includes('Start'))
    if (startMeetingBtn) {
  ...
  it('handles onMeetingsUpdated event without crashing', async () => {
    ...
    await waitFor(() => {
      expect((window.electronAPI.getRecentMeetings as any).mock.calls.length).toBeGreaterThanOrEqual(1)
    })
  })
  ...
  it('calls calendarRefresh on refresh button click', async () => {
    ...
    const refreshBtn = buttons.find(btn => {
      ...
      return svg !== null && btn.getAttribute('class')?.includes('refresh')
  ...
  it('renders meeting groups sorted by date (Today first)', async () => {
    ;(window.electronAPI.getRecentMeetings as any).mockResolvedValue([
      { id: '1', title: 'Old Meeting', date: '2026-01-01', duration: '3600', summary: 'Old' },
      { id: '2', title: 'Today Meeting', date: new Date().toISOString(), duration: '1800', summary: 'Today' },
    ])
  ...
  it('unmounts cleanly without errors', () => {
    const { unmount } = render(<Launcher onStartMeeting={mockOnStartMeeting} onOpenSettings={mockOnOpenSettings} />)
  ...
  it('handles getRecentMeetings rejection gracefully', async () => {
    ;(window.electronAPI.getRecentMeetings as any).mockRejectedValue(new Error('DB error'))
  ...
  it('calls onOpenSettings when settings button is clicked', () => {
    ...
    const settingsBtn = buttons.find(btn => {
      ...
      return svg !== null && btn.querySelector('[class*="lucide-settings"]') !== null
    })
