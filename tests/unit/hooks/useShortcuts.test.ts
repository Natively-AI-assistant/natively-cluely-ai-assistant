...
describe('useShortcuts', () => {
  ...
  beforeEach(() => {
    ...
    onKeybindsUpdateMock = vi.fn((_cb: Function) => {
  ...
  it('should call setKeybind when updateShortcut is called', async () => {
    ...
    // Update a shortcut
    await act(async () => {
      await result.current.updateShortcut('whatToAnswer', [
        'Ctrl',
        'Shift',
        '1',
      ])
    ...
    expect(setKeybindMock).toHaveBeenCalledWith(
      'chat:whatToAnswer',
      'Ctrl+Shift+1',
    )
  })
