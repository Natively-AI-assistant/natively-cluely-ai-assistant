...
class FakeLive extends EventEmitter {
  readyState = 1; // OPEN
  requestClose = vi.fn(() => { this.readyState = 2; this.emit('close', { code: 1000, reason: 'Closed' }); });
  send = vi.fn((data: any) => {});
  keepAlive = vi.fn();
  ...
  constructor() {
    super();
    // Simulate async open
    setTimeout(() => { this.readyState = 1; this.emit('open'); }, 5);
  }
}
  ...
  return {
    listen: {
      live: (opts: any) => {
        ...
        (live as any).url = `wss://api.deepgram.com/v1/listen?${new URLSearchParams(opts as any).toString()}`;
