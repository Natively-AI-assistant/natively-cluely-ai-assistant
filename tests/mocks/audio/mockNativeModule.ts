 ...
 */
export function createMockCaptureInstance(
  options: MockNativeCaptureOptions = {},
): MockCaptureInstance {
  const sampleRate = options.sampleRate ?? 48000

  let dataCallback: ((err: Error | null, chunk: Buffer) => void) | null = null
  let speechEndedCallback:
    | ((err: Error | null, ended: boolean) => void)
    | null = null
  ...
  const instance: MockCaptureInstance = {
    ...
    get _speechEndedCallback() {
      return speechEndedCallback
    },
    set _dataCallback(cb: any) {
      dataCallback = cb
    },
    set _speechEndedCallback(cb: any) {
      speechEndedCallback = cb
    },
  }
...
export function createMockNativeModule(options: MockNativeModuleOptions = {}) {
  ...
  const module_ = {
    ...
    MicrophoneCapture: vi.fn(function MicrophoneCaptureMock(
    ...
    SystemAudioCapture: vi.fn(function SystemAudioCaptureMock(
      _deviceId?: string | null,
    ) {
...
export function mockMicrophoneCapture(options: MockNativeCaptureOptions = {}) {
  ...
  const Constructor = vi.fn((_deviceId?: string | null) => {
