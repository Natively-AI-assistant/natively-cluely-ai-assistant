...
export class MockWebSocket {
  ...
  constructor(url: string, protocols?: string | string[] | Record<string, any>, options?: MockWebSocketOptions) {
    this.url = url;
    ...
    this.close = vi.fn((code?: number, reason?: string) => {
      ...
      queueMicrotask(() => {
        ...
        this._emit('close', code ?? 1000, reason ? Buffer.from(reason) : Buffer.from(''));
      });
    });
    ...
    if (delay >= 0 && !this.options.connectError) {
      ...
      this.openTimer = setTimeout(() => this._simulateError(this.options.connectError!), delay);
    }
  }
...
export function createMockWebSocket(defaults?: MockWebSocketOptions) {
  ...
  const MockWsConstructor = vi.fn((url: string, protocols?: any, options?: any) => {
    const instance = new MockWebSocket(url, protocols, { ...defaults, ...options });
