...
export function createMockGoogleGenAI(config: { response?: string; streamChunks?: string[] } = {}) {
  const mockResponse = {
  ...
  const mockStreamChunks = config.streamChunks || ['Mocked ', 'chunk ', 'response'];
...
export function createMockGroqClient(config: MockStreamConfig = {}) {
  ...
  const mockCreate = vi.fn((params: any) => {
    return {
      choices: [{
        message: {
          content: chunks.join('')
...
export function createMockStreamResponse(chunks: string[], config: { delay?: number } = {}) {
...
export function createMockApiResponse(text: string, config: { delay?: number } = {}) {
  const { delay = 0 } = config;
...
export function createLLMHelperMock(overrides: {
  mockClients?: {
  ...
  const {
    mockClients = {},
    streamChunks = ['Test ', 'response ', 'chunk']
  } = overrides;

  const google = mockClients.google || createMockGoogleGenAI({ streamChunks });
  const groq = mockClients.groq || createMockGroqClient({ chunks: streamChunks });
  const openai = mockClients.openai || createMockOpenAIClient({ chunks: streamChunks });
  const anthropic = mockClients.anthropic || createMockAnthropicClient({ chunks: streamChunks });
 ...
 */
export function createMockLlmHelperForTests(config: {
  chatResponse?: string;
  streamChunks?: string[];
  chatError?: Error;
  streamError?: Error;
  ...
  const mockHelper = {
    ...
    setChatResponse: (resp: string) => { mockChat.mockResolvedValue(resp); },
    setStreamChunks: (chunks: string[]) => {
      mockStreamChat.mockImplementation(async function* () {
        for (const chunk of chunks) { yield chunk; }
      });
    },
    setChatError: (err: Error) => { mockChat.mockRejectedValue(err); },
    setStreamError: (err: Error) => {
      mockStreamChat.mockImplementation(async function* () { throw err; });
    },
  };
