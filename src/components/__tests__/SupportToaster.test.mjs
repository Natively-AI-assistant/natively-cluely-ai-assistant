...
test('returning after 20s from the support page is treated as a donation', () => {
  ...
  assert.ok(focus.includes('onDismiss()'));
...
test('Escape, backdrop, close and "Maybe later" all dismiss', () => {
  assert.ok(rendered.includes("if (e.key === 'Escape') onDismiss();"));
  assert.ok(rendered.includes('if (e.target === e.currentTarget) onDismiss();'));
  assert.equal((rendered.match(/onClick=\{onDismiss\}/g) || []).length, 2,
...
test('split layout with the support art, one tree for both themes', () => {
  assert.ok(source.includes("import supportArt from '../../assets/support.png'"));
