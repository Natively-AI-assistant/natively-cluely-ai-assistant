...
test('start trial: hides on success, stays open with an error on failure', () => {
  ...
  assert.ok(start.indexOf('setVisible(false)') > start.indexOf('await onStartTrial();'),
...
test('manual setup and every dismissal still work, but not mid-start', () => {
  assert.ok(between('const handleManual', 'const item').includes('onManualSetup()'));
  assert.ok(rendered.includes("if (e.key === 'Escape' && !starting) handleDismiss();"));
  assert.ok(rendered.includes('if (e.target === e.currentTarget && !starting) handleDismiss();'));
  assert.ok(rendered.includes('aria-label="Close"'));
});

test('the exit animation can actually run', () => {
  // An early `return null` above AnimatePresence unmounts the presence
  // boundary with the card, which makes every `exit` dead code.
  assert.ok(!/if \(!visible\) return null/.test(rendered));
  assert.match(rendered, /<AnimatePresence>\s*\{visible && \(/);
});
...
test('split layout with the flower art, one tree for both themes', () => {
  assert.ok(source.includes("import flowerArt from '../../../assets/flower.png'"));
