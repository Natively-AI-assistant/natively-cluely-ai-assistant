...
//
// Same two-pane family as the notification cards (see
...
// The family's measured ink sets (spec §3.2).
const INK_DARK = {
    strong: '#F2F2F4',
...
// ─── Corner toast ──────────────────────────────────────────────
//
// What the card shrinks to when the download is hidden. No scrim, so the app
// stays usable; a thumbnail of the version panel keeps the wave going. The
// body brings the full card back; × closes it for good.
...
export const UpdateCornerToast: React.FC<UpdateCornerToastProps> = ({
    isOpen, updateInfo, downloadProgress, downloadDetail, status, onExpand, onClose,
}) => {
    const t = useT();
    const reduced = useReducedMotion() ?? false;
    const isLight = useResolvedTheme() === 'light';
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    exit={   reduced ? { opacity: 0 } : { opacity: 0, y: 8, scale: 0.98, transition: { duration: 0.16 } }}
                    ...
                    style={{
                        ...
                        background: isLight ? '#F7F8FC' : '#1C1C1E',
                        boxShadow: isLight
                            ? 'inset 0 0 0 1px rgba(11,16,32,0.10), inset 0 1px 0 rgba(255,255,255,0.80), 0 20px 50px -24px rgba(16,24,40,0.45)'
                            : 'inset 0 0 0 1px rgba(255,255,255,0.08), inset 0 1px 0 rgba(255,255,255,0.06), 0 24px 60px -24px rgba(0,0,0,0.85)',
                        fontFamily: FONT,
                        WebkitFontSmoothing: 'antialiased',
                    } as React.CSSProperties}
                >
