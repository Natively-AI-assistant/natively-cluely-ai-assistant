 ...
 * status colour.
 */
export const AipBadge: React.FC<AipBadgeProps> = ({
	tone,
	label,
	busy = false,
	title,
...
export const AIP_PROVIDER_BRANDS: Record<
	string,
	{ mono: string; brand: string }
> = {
...
export const AipMonogram: React.FC<AipMonogramProps> = ({
	mono,
	brand,
	className = '',
}) => (
	<span
		...
		style={
			{
				['--aip-brand' as string]: brand ?? 'var(--aip-accent)',
 ...
 */
export const AipProviderMark: React.FC<AipProviderMarkProps> = ({
	provider,
	name,
	className = '',
}) => {
	...
	if (!markup && imageSrc) {
		return (
			<span
				...
				style={
					{
						['--aip-brand' as string]: brand?.brand ?? 'var(--aip-accent)',
					} as React.CSSProperties
				}>
				<img
					src={imageSrc}
	...
	if (!markup) {
		return (
			<AipMonogram
				mono={brand?.mono ?? name ?? provider ?? 'AI'}
				brand={brand?.brand}
				className={className}
			/>
		);
...
const AIP_PREVIEW_RE =
	/preview|exp(erimental)?\b|-latest$|-\d{4}-\d{2}-\d{2}$|-\d{2}-\d{2}$/i;
...
export const AipModelList: React.FC<AipModelListProps> = ({
	...
	const isOn = (id: string) =>
		optIn ? enabled.includes(id) : enabled.length === 0 || enabled.includes(id);
	const enabledCount =
		!optIn && enabled.length === 0 ? models.length : enabled.length;
		...
		<>
			<button
				...
				<span className="aip-meta truncate min-w-0 flex-1 text-right">
					{defaultId
						? `${models.find((m) => m.id === defaultId)?.label ?? defaultId} · ${t('default')}`
						: ''}
				</span>
				{error ? (
			...
			<div
				className="aip-reveal aip-reveal--models w-full basis-full order-4"
				data-open={open ? 'true' : 'false'}>
				<div>
					<div
						id={panelId}
						role="group"
						aria-label={t('Models shown in the picker')}
						className="pt-2"
							...
							{onRefresh && (
								<button
									...
									<RefreshCw
										size={11}
										strokeWidth={1.75}
										className={refreshing ? 'aip-spinner' : ''}
									/>
									{refreshing
										? t('Fetching...')
										: showFilterBar
						...
						<div
							ref={listRef}
							className="aip-well aip-scroll-y custom-scrollbar aip-models-well">
							{visible.length === 0 ? (
								<p className="aip-meta px-2 py-3 text-center">
									...
									return (
										<div key={m.id} className="aip-model-row aip-row">
											<button
												...
												onClick={() => {
													if (!inert) onToggle(m.id);
												}}
												onFocus={() => setActiveIndex(i)}
												title={
													inert
														? t(
																'At least one model must stay on. Turn the provider off to hide it entirely.',
															)
														: m.id
												}
												...
												<Check
													size={11}
													strokeWidth={2.5}
													className="aip-model-check"
													aria-hidden="true"
												/>
												<span className="aip-model-name truncate">
													{m.label}
												</span>
												{m.label !== m.id && (
													<span className="aip-model-id aip-mono truncate">
														{m.id}
													</span>
												)}
											</button>
											{stale && (
												<AipBadge tone="warn" label={t('Not offered')} />
											)}
													...
													onSetDefault && (
														...
														<div className="aip-row-actions">
															<button
																...
																title={t(
																	'Use this model by default for this provider',
																)}>
																{t('Set default')}
															</button>
														</div>
						...
						{models.length <= AIP_MODEL_FILTER_THRESHOLD && (
							<p className="aip-meta mt-2">
								{t('Showing built-in models only.')}
							</p>
						)}
					</div>
				</div>
...
export interface AipSelectOption {
	id: string;
	name: string;
}
		...
		<div className={`aip-select ${className}`}>
			<button
				...
				<span className="truncate">
					{selected ? selected.name : value || placeholder || label}
				</span>
				<ChevronDown
					size={14}
					strokeWidth={1.75}
			...
			<div className="aip-reveal" data-open={open ? 'true' : 'false'}>
				<div>
					<div
							...
							<div
								...
								{value === option.id && (
									<Check
										size={13}
										strokeWidth={1.75}
										className="aip-accent-fg shrink-0"
										aria-hidden="true"
									/>
								)}
							</div>
						...
						{options.length === 0 && (
							<div className="aip-select-empty">
			...
			{disabled && disabledHint && (
				<p className="aip-meta mt-1.5">{disabledHint}</p>
			)}
		</div>
	);
};
...
const SCOPE_ROWS = [
	...
	{
		key: 'reference_files' as const,
		labelKey: 'Reference files',
		Icon: FileText,
	},
	...
	{
		key: 'post_call_summary' as const,
		labelKey: 'Post-call summaries',
		Icon: ClipboardList,
	},
];
...
const CODEX_MODEL_REASONING_SETS: ReadonlyArray<
...
function getValidCodexReasoningEfforts(modelId: string): readonly string[] {
	...
	for (const entry of CODEX_MODEL_REASONING_SETS) {
		if (id.includes(entry[0]) && (!best || entry[0].length > best[0].length))
			best = entry;
	}
						...
						<button
							...
							{value === option.id && (
								<Check
									size={13}
									strokeWidth={1.75}
									className="aip-accent-fg shrink-0 ml-2"
									aria-hidden="true"
								/>
							)}
						</button>
...
export const AIProvidersSettings: React.FC<AIProvidersSettingsProps> = ({
	...
	const [cloudEnabledModels, setCloudEnabledModelsState] = useState<
	...
	const [cloudFetchedModels, setCloudFetchedModels] = useState<
		Record<string, AipModelEntry[]>
	>({});
	...
	const [customVision, setCustomVision] = useState<'auto' | 'on' | 'off'>(
		'auto',
	);
	...
	const activeTabIndex = Math.max(
		0,
		PROVIDER_TABS.findIndex((tab) => tab.id === activeTab),
	);
	...
	const tabRefs = useRef<
		Partial<Record<ProviderTabId, HTMLButtonElement | null>>
	>({});
	...
	const handleTabKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>) => {
		...
		if (event.key === 'Home') nextIndex = 0;
			...
			nextIndex =
				(current + delta + PROVIDER_TABS.length) % PROVIDER_TABS.length;
		}
	...
	const [codexCliConfig, setCodexCliConfig] = useState({
		...
		timeoutMs: 60000,
		sandboxMode: 'read-only' as string,
		serviceTier: 'default',
		modelReasoningEffort: undefined as string | undefined,
	});
	const [codexCliStatus, setCodexCliStatus] = useState<
		'idle' | 'testing' | 'success' | 'error'
	>('idle');
	const [codexCliError, setCodexCliError] = useState('');
	const [codexAuthAction, setCodexAuthAction] = useState<
		'idle' | 'status' | 'logout' | 'login' | 'doctor'
	>('idle');
	...
	const visionRequired =
		screenUnderstandingMode === 'vision_only' || visionLocalOnly;
	...
	const applyVisionMode = (localOnly: boolean, required: boolean) => {
		...
		if (localOnly && !visionLocalOnly) {
			...
			requiredBeforeLocalOnly.current =
	...
	const disabledScopeCount = SCOPE_ROWS.filter(
		(r) => providerDataScopes[r.key] === false,
	).length;
	...
	useEffect(() => {
		const loadCredentials = async () => {
			try {
				...
				if (creds) {
					...
					setLitellmMaxTokens(
						creds.litellmMaxTokens ? String(creds.litellmMaxTokens) : '',
					);
					// Load preferred models
					const pm: Record<string, string> = {};
					if (creds.geminiPreferredModel)
						pm.gemini = creds.geminiPreferredModel;
					if (creds.groqPreferredModel) pm.groq = creds.groqPreferredModel;
					if (creds.openaiPreferredModel)
						pm.openai = creds.openaiPreferredModel;
					if (creds.claudePreferredModel)
						pm.claude = creds.claudePreferredModel;
					if (creds.deepseekPreferredModel)
						pm.deepseek = creds.deepseekPreferredModel;
					...
					// on no row at all.
					if (creds.litellmPreferredModel)
						pm.litellm = creds.litellmPreferredModel;
					setDisabledProviders(
						Array.isArray(creds.disabledProviders)
							? creds.disabledProviders
							: [],
					);
					setCloudEnabledModelsState(creds.cloudEnabledModels || {});
					window.electronAPI
						?.getCloudFetchedModels?.()
						.then((res: { models?: Record<string, AipModelEntry[]> }) => {
		...
		if (window.electronAPI?.onGroqFastTextChanged) {
			// @ts-ignore
			unsubs.push(
				window.electronAPI.onGroqFastTextChanged((enabled: boolean) => {
		...
		if (window.electronAPI?.onCredentialsChanged) {
			// @ts-ignore
			unsubs.push(
				window.electronAPI.onCredentialsChanged(() => {
					loadCredentials();
				}),
			);
		}
	 ...
	 * wipe them.
	 */
	const effectiveModels = useCallback(
		(provider: string): AipModelEntry[] => {
			...
			if (provider === 'litellm')
				litellmModels.forEach((m) =>
					push(`litellm/${m}`, litellmModelLabel(m)),
				);
			(cloudFetchedModels[provider] || []).forEach((m) =>
				push(m.id, m.label || m.id),
			);
	...
	const handleToggleModel = async (provider: string, modelId: string) => {
		...
		const normalised = optIn
			? nextList
			: nextList.length === 0 || nextList.length === universe.length
				? []
				: nextList;
		...
		const currentDefault =
			preferredModels[provider as keyof typeof preferredModels];
		if (currentDefault === modelId && normalised.length > 0) {
			const moved = normalised[0];
			setPreferredModels((p) => ({ ...p, [provider]: moved }));
			window.electronAPI
				?.setProviderPreferredModel?.(provider as any, moved)
				.catch((e: unknown) =>
					console.error('Failed to move default model:', e),
				);
		}
		try {
			const res = await window.electronAPI?.setCloudEnabledModels?.(
				provider,
				normalised,
			);
			...
			setTimeout(
				() => setModelSaveError((p) => ({ ...p, [provider]: false })),
				4000,
			);
		}
	};
	 ...
	 */
	const handleBulkToggleModels = async (
		provider: string,
		ids: string[],
		enable: boolean,
	) => {
		...
		if (
			currentDefault &&
			!isModelAllowed(provider, currentDefault, normalised) &&
			normalised.length > 0
		) {
			const moved = normalised[0];
			setPreferredModels((p) => ({ ...p, [provider]: moved }));
	...
	const handleSetDefaultModel = async (provider: string, modelId: string) => {
		...
		if (needsAllow)
			setCloudEnabledModelsState((p) => ({ ...p, [provider]: nextList }));
		setPreferredModels((p) => ({ ...p, [provider]: modelId }));

		try {
			if (needsAllow) {
				const r = await window.electronAPI?.setCloudEnabledModels?.(
					provider,
					nextList,
				);
				if (r && r.success === false)
					throw new Error(r.error || 'allow-list write failed');
			}
	...
	useEffect(() => {
		...
		try {
			if (api?.onCodexLoginComplete) {
				unsubs.push(
					api.onCodexLoginComplete((info: any) => {
						setCodexOauthInProgress(false);
						setCodexOauthStatus((prev) => ({
							...prev,
							signedIn: true,
						...
						setCodexAuthMessage(
							`${t('Signed in to ChatGPT')}${info?.email ? ` ${t('as')} ${info.email}` : ''}.`,
						);
			...
			if (api?.onCodexLoginFailed) {
				unsubs.push(
					api.onCodexLoginFailed((info: any) => {
						setCodexOauthInProgress(false);
			...
			if (api?.onCodexTokensRefreshed) {
				unsubs.push(
					api.onCodexTokensRefreshed((info: any) => {
						setCodexOauthStatus((prev) => ({
							...prev,
	...
	const checkOllamaInner = async () => {
		...
		try {
			...
			setLocalFallback({
				text: Boolean(st?.text),
				vision: Boolean(st?.vision),
			});
		} catch {
			setLocalFallback({ text: false, vision: false });
		}
	...
	const saveCodexCliConfig = async (next = codexCliConfig) => {
		...
		if (result?.config)
			setCodexCliConfig(result.config as typeof codexCliConfig);
		return result;
	};
	...
	const handleCodexAuthAction = async (
		...
		try {
			...
			if (action === 'login' && api?.codexStartLogin) {
				...
				setCodexAuthMessage(
					t('Opening browser — complete sign-in there, then return here.'),
				);
				...
				if (result?.success) {
					setCodexAuthStatus('success');
					setCodexAuthMessage(
						`${t('Signed in to ChatGPT')}${result.email ? ` ${t('as')} ${result.email}` : ''}.`,
					);
					setCodexOauthStatus({
						signedIn: true,
						email: result.email,
			...
			if (result?.success) {
				...
				setCodexAuthStatus('error');
				const msg =
					result?.error || result?.output || `Codex ${action} failed.`;
	...
	const handleSaveKey = async (
		...
		try {
			...
			if (provider === 'deepseek')
				result = await window.electronAPI.setDeepseekApiKey(key);
	...
	const handleSaveLitellm = async () => {
		...
		try {
			...
			if (result && result.success) {
				...
				setTimeout(
					() => setSavedStatus((prev) => ({ ...prev, litellm: false })),
					2000,
				);
			}
		} catch (e) {
	...
	const performRemoveKey = async (
		...
		try {
			...
			// @ts-ignore
			if (provider === 'deepseek')
				result = await window.electronAPI.setDeepseekApiKey('');
	...
	const handleTestConnection = async (provider: string, key: string) => {
		...
		try {
			...
			if (result.success) {
				...
				setTestError((prev) => ({
					...prev,
					[provider]: result.error || t('Connection failed'),
				}));
			}
		} catch (e: any) {
			setTestStatus((prev) => ({ ...prev, [provider]: 'error' }));
			setTestError((prev) => ({
				...prev,
				[provider]: e.message || t('Connection failed'),
			}));
		}
	};
		...
		switch (pendingConfirm.kind) {
			case 'litellm':
				return {
					...
					description: t(
						'The proxy URL, virtual key, and token limit will be cleared. Discovered models will no longer appear in the model picker.',
					),
			...
			case 'providerKey':
				return {
					...
					description: t(
						'The stored key is deleted. You will need to paste it again to re-enable this provider.',
					),
			...
			case 'customProvider':
				return {
					...
					description: t(
						'Its endpoint, cURL template, and response path are deleted. This cannot be undone.',
					),
		...
		try {
			switch (pendingConfirm.kind) {
				case 'litellm':
					...
					break;
				case 'providerKey':
					await performRemoveKey(
						pendingConfirm.provider,
						pendingConfirm.setter,
					);
					break;
				case 'customProvider':
	...
	return (
		...
		<div
			className="aip-root space-y-5 pb-10"
			data-theme={theme}
			data-settings-stagger>
			{confirmCopy && (
				<ConfirmDialog
					open
					onOpenChange={(next) => {
			...
			<div className="aip-card p-5 flex items-center justify-between gap-4">
				<div className="min-w-0">
					...
					<p className="text-[10px] aip-muted mt-0.5">
						{t('Applies to new chats instantly.')}
					</p>
				</div>
				<ModelSelect
					value={defaultModel}
			...
			<div
				...
				title={
					!canUseFastMode
						? t('Requires Groq, Natively API, or Codex CLI to be configured')
						: ''
				}>
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						<label className="block text-xs font-medium uppercase tracking-wide mb-0 aip-hero">
							{t('Fast Response Mode')}
						</label>
						<AipBadge tone="info" label={t('New')} />
						{!canUseFastMode && (
							<AipBadge tone="warn" label={t('Needs Groq')} />
						)}
					</div>
					<p className="text-[10px] aip-muted mt-0.5">
						{t(
							'Uses the fastest available provider instead of your selected model.',
						)}
					</p>
					{!canUseFastMode && (
			...
			{activeTab === 'cloud' && (
				<div
					...
					<div className="space-y-5">
						<div>
							<h3 className="text-sm font-bold aip-hero mb-1">
								{t('Cloud Providers')}
							</h3>
							<p className="text-xs aip-muted mb-2">
								{t('Add API keys to unlock cloud AI models.')}
							</p>
						</div>
								...
								return (
									<ProviderCard
										...
										onToggleDisabled={(enabled) =>
											handleToggleProvider(id, enabled)
										}
										...
										onSetDefaultModel={(modelId) =>
											handleSetDefaultModel(id, modelId)
										}
										hasCatalog={(cloudFetchedModels[id]?.length ?? 0) > 0}
										modelSaveError={!!modelSaveError[id]}
										onSaveKey={async () => {
											await handleSaveKey(id, keyValue, setKeyValue);
										}}
										...
										onPreferredModelChange={(model) =>
											setPreferredModels((prev) => ({ ...prev, [id]: model }))
										}
									/>
								);
							})}
					...
					<div className="space-y-5">
						...
						<div className="aip-card p-5 space-y-4">
							...
							{codexOauthStatus.signedIn ? (
								<div className="flex gap-2 mb-3">
									<div className="aip-well flex-1 min-w-0 px-3 py-2.5 text-xs aip-text flex items-center gap-2">
										<span
											className="aip-badge-dot aip-ok-fg"
											aria-hidden="true"
										/>
										<span className="truncate">
											{codexOauthStatus.email || t('ChatGPT account connected')}
										</span>
									</div>
								</div>
							) : (
								<div className="flex gap-2 mb-3">
									<button
										...
										disabled={
											codexOauthInProgress || codexAuthAction !== 'idle'
										}
							...
							{codexAuthMessage && (
								<p
									className={`text-[10px] mt-1.5 mb-2 ${codexAuthStatus === 'error' ? 'aip-danger-fg' : 'aip-ok-fg'}`}>
									{codexAuthMessage}
								</p>
							)}
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
										...
										<div className="min-w-0">
											...
											<p className="aip-meta mt-1">
												{t(
													'Spark is API-key only. ChatGPT login uses Codex 5.3 instead.',
												)}
											</p>
										</div>
										<label className="space-y-1 block min-w-0">
											...
											<ModelSelect
												value={(() => {
													const valid = getValidCodexReasoningEfforts(
														codexCliConfig.model,
													);
												...
												options={(() => {
													...
													return [
														...
														...CODEX_MODEL_REASONING_EFFORTS.filter(
															(e) => e !== 'none' && valid.includes(e),
														).map((e) => ({
															id: e,
												...
												onChange={(effort) =>
													saveCodexCliConfig({
														...codexCliConfig,
														modelReasoningEffort: effort || undefined,
													})
												}
												...
												if (saved && !valid.includes(saved)) {
													return (
														<p className="aip-meta aip-warn-fg flex items-center gap-1.5">
															...
															{saved}'{' '}
															{t(
																"unsupported by this model — will default to 'low'.",
															)}
														</p>
													);
												}
										...
										<label className="space-y-1 block min-w-0">
											...
											<ModelSelect
												value={
													codexCliConfig.serviceTier === 'fast'
														? 'default'
														: (codexCliConfig.serviceTier ?? 'default')
												}
												options={CODEX_SERVICE_TIERS.filter(
													(t) => t !== 'fast',
												).map((t) => ({
													id: t,
												...
												onChange={(serviceTier) =>
													saveCodexCliConfig({
														...codexCliConfig,
														serviceTier:
															serviceTier as (typeof CODEX_SERVICE_TIERS)[number],
													})
												}
											...
											<p className="aip-meta">
												{t(
													'ChatGPT accounts do not support the Fast service tier. Use Fast Response Mode + Codex 5.3 instead.',
												)}
											</p>
										</label>
									</div>
									<div className="flex items-end justify-between gap-4 mt-1">
										<label className="space-y-1 block min-w-0">
											...
											{codexCliStatus === 'error' && codexCliError && (
												<p className="text-[10px] aip-danger-fg mt-1">
													{codexCliError}
												</p>
											)}
										</label>
							...
							<div className="aip-card p-5 space-y-4">
								<div className="flex items-center justify-between">
									<div className="flex items-start gap-2.5 min-w-0">
										...
										/>
										<div className="min-w-0">
											<label className="block text-xs font-bold aip-hero mb-0">
												LiteLLM Proxy
											</label>
                                        ...
                                        two controls for one action. */}
											...
											<AipSwitch
												...
												title={
													disabledProviders.includes('litellm')
														? t('Enable provider')
														: t('Disable provider (keeps your configuration)')
												}
											/>
										</div>
									)}
										...
										<input
											...
											placeholder={
												hasStoredKey.litellm
													...
													: t('sk-... (only if proxy requires auth)')
											}
										/>
									</label>
								</div>
								...
								<div className="space-y-1">
									<span className="block aip-label">
										{t('Max Output Tokens')}
									</span>
									<ModelSelect
									...
									<p className="text-[10px] aip-muted">
										{t(
											"Auto reads each model's real output budget from the proxy's",
										)}{' '}
										<span className="aip-code-inline">/model/info</span>{' '}
										{t(
											'(falls back to 8,192 if unavailable). Pick a fixed value to override.',
										)}
									</p>
								</div>
                                ...
                                when the active model becomes unavailable. */}
									{hasStoredKey.litellm && (
										<AipModelList
											...
											optIn
											onBulkToggle={(ids, enable) =>
												handleBulkToggleModels('litellm', ids, enable)
											}
					...
					<div className="space-y-5">
						<div className="flex items-center justify-between mb-2">
							<div className="flex items-start gap-2.5 min-w-0">
								...
								<div className="min-w-0">
									<h3 className="text-sm font-bold aip-hero mb-1">
										{t('Local Models (Ollama)')}
									</h3>
									<p className="text-xs aip-muted">
										{t('Run open-source models locally.')}
									</p>
								</div>
							</div>
							<div className="flex items-center gap-2 shrink-0">
								<button
									...
									<RefreshCw
										size={16}
										strokeWidth={1.75}
										className={isRefreshingOllama ? 'aip-spinner' : ''}
									/>
								</button>
								<AipSwitch
							...
							{ollamaStatus === 'not-found' && (
								<div className="flex flex-col gap-2">
									<div className="flex items-center gap-2">
										...
										<span className="text-xs aip-danger-fg">
											{t('Ollama not detected')}
										</span>
									</div>
							...
							{ollamaStatus === 'detected' && ollamaModels.length > 0 && (
								<div className="space-y-3">
									<div className="flex items-center gap-2 mb-3">
										...
										<span className="text-xs aip-muted">
											{t('Ollama connected')}
										</span>
									</div>
									...
									<div className="grid grid-cols-1 gap-2">
										{ollamaModels.map((model) => (
											<div
												key={model}
												className="aip-well flex items-center justify-between gap-2 p-2">
					...
					<div className="space-y-5">
						<div className="flex items-center justify-between mb-2">
							<div>
								<div className="flex items-center gap-2 mb-1">
									<h3 className="text-sm font-bold aip-hero">
										{t('Custom Providers')}
									</h3>
									<AipBadge tone="warn" label={t('Experimental')} />
								</div>
								<p className="text-xs aip-muted">
									{t('Add your own AI endpoints via cURL.')}
								</p>
							</div>
							<div className="flex items-center gap-2 shrink-0">
								{!isEditingCustom && (
									<button onClick={handleNewProvider} className="aip-btn">
									...
									<AipSwitch
										...
										title={
											disabledProviders.includes('custom')
												...
												: t('Disable custom providers (keeps them saved)')
										}
									/>
								)}
							</div>
						...
						{isEditingCustom ? (
							<div className="aip-card p-5 aip-panel-fade">
								<h4 className="text-sm font-bold aip-hero mb-4">
									{editingProvider ? t('Edit Provider') : t('New Provider')}
								</h4>
								...
								<div className="space-y-4">
									...
									<div>
										<label className="block aip-label mb-1">
											{t('Response JSON Path')}{' '}
											<span className="aip-faint normal-case font-normal">
												{t('(Optional)')}
											</span>
										</label>
										<input
                                    ...
                                    are translated keys that need re-plumbing). */}
										<select
											value={customVision}
											onChange={(e) =>
												setCustomVision(e.target.value as 'auto' | 'on' | 'off')
											}
											className="aip-input">
											<option value="auto">
												{t('Auto-detect (recommended)')}
											</option>
											<option value="on">{t('Always send screenshots')}</option>
											<option value="off">
												{t('Never send screenshots (text only)')}
											</option>
										</select>
										<p className="text-[10px] aip-muted mt-1">
											{t('Auto-detect enables vision when your cURL uses')}{' '}
											<code className="aip-code-inline">
												{'{{IMAGE_BASE64}}'}
											</code>{' '}
											{t('or an OpenAI-style')}{' '}
											<code className="aip-code-inline">messages</code>{' '}
											{t(
												'body. Choose “Always” only if your endpoint accepts images another way; “Never” keeps this provider out of screenshot analysis.',
											)}
										</p>
									</div>
									...
									<div className="aip-well mt-4">
										<div
											...
											style={{ borderColor: 'var(--aip-divider)' }}>
										...
										<div className="p-4 space-y-4 min-w-0">
											<div>
												<p className="text-xs aip-muted mb-2 font-medium">
													{t('Available Variables')}
												</p>
												<div className="grid grid-cols-1 gap-2">
													<div className="flex items-center gap-2 text-xs">
														<code className="aip-code-inline shrink-0">
															{'{{TEXT}}'}
														</code>
													...
													<div className="flex items-center gap-2 text-xs">
														...
														<span className="aip-muted">
															{t('Screenshot data (if available)')}
														</span>
													</div>
												</div>
											</div>
											...
											<div className="min-w-0">
												...
												<div className="space-y-3 min-w-0">
													{/* Ollama Example */}
													<div className="min-w-0">
														<div className="aip-label mb-1.5">
															{t('Local (Ollama)')}
														</div>
									...
									{curlError && (
										<div
											...
											style={{
												...
												border: '1px solid var(--aip-danger-border)',
											}}>
											<AlertCircle
												size={14}
												strokeWidth={1.75}
							...
							<div className="space-y-3">
								{customProviders.length === 0 ? (
									<div className="aip-card aip-card-dashed text-center py-8">
										<p className="text-xs aip-muted">
											{t('No custom providers added yet.')}
										</p>
									</div>
								) : (
                ...
                Now two switches over the same enum; see applyVisionMode(). */}
					<div className="space-y-5">
						<div>
							...
							<p className="text-xs aip-muted mb-2">
								{t('Controls where screenshots of your screen are processed.')}
							</p>
						</div>
						<div className="aip-card p-5 flex flex-col gap-3">
							<div className="flex items-center justify-between gap-3">
								<div className="flex flex-col min-w-0">
									<span className="text-xs aip-hero font-semibold">
										{t('Keep screenshots on this device')}
									</span>
									<span className="aip-meta leading-snug mt-0.5">
										{t(
											'Use a local vision model (Ollama) only. Cloud vision is never called.',
										)}
									</span>
								</div>
								<AipSwitch
							...
							{visionLocalOnly && !localFallbackAvailable && (
								<div className="aip-inline-warn flex items-start gap-2">
									...
									<span>
										{t(
											'No local vision model is installed. Screenshot questions will be refused rather than sent to the cloud. Install a vision-capable model under Local & Gateways.',
										)}
									</span>
								</div>
							)}
							...
							<div
								...
								<div className="flex flex-col min-w-0">
									<span
										className={`text-xs font-semibold ${visionLocalOnly ? 'aip-faint' : 'aip-hero'}`}>
									...
									<span className="aip-meta leading-snug mt-0.5">
										{visionLocalOnly
											...
											: t(
													'Fail with a clear error instead of quietly answering without the screenshot.',
												)}
									</span>
								</div>
								<AipSwitch
                        ...
                        by which engine owns it. */}
							<div
								...
								<div className="flex flex-col min-w-0">
									<span className="text-xs aip-hero font-semibold">
										{t('High-resolution capture for code')}
									</span>
                                ...
                                that does nothing on the hotkey/attachment capture path. */}
									<span className="aip-meta leading-snug mt-0.5">
										{t(
											'In technical interview and coding modes, captures at the highest-resolution profile so small code text stays legible. Costs more tokens per screenshot.',
										)}
									</span>
								</div>
								<AipSwitch
									...
									onChange={(next) => {
										...
										if (api?.setTechnicalInterviewVisionFirst) {
											...
											window.electronAPI?.setTechnicalInterviewDirectVision?.(
												next,
											);
										}
									}}
							...
							{!visionLocalOnly && providerDataScopes.screenshots === false && (
								<div
									...
									<Info
										size={12}
										strokeWidth={1.75}
										className="aip-faint shrink-0 mt-0.5"
										aria-hidden="true"
									/>
									<p className="aip-meta leading-relaxed">
										{localFallbackAvailable
											? t(
													'Screenshots are already blocked from cloud providers by the data scope below, so your local vision model handles them.',
												)
											: visionRequired
												? t(
														'Screenshots are blocked from cloud providers by the data scope below, and no local vision model is installed. Screenshot questions will be refused rather than answered without the image.',
													)
												: t(
														'Screenshots are blocked from cloud providers by the data scope below, and no local vision model is installed — so the screenshot is discarded and the question is answered without it.',
													)}
									</p>
								</div>
							)}
                ...
                reading any row, and the footnote only when it carries new information. */}
					<div className="space-y-5">
						<div className="flex items-end justify-between gap-3">
							<div className="min-w-0">
								<h3 className="text-sm font-bold aip-hero mb-1">
									{t('Cloud provider data scopes')}
								</h3>
								<p className="text-xs aip-muted">
									{t('What cloud AI providers are allowed to receive.')}
								</p>
							</div>
							<span className="aip-meta tabular-nums shrink-0 pb-0.5">
								{SCOPE_ROWS.length - disabledScopeCount}/{SCOPE_ROWS.length}{' '}
								{t('shared')}
							</span>
						</div>
								...
								return (
									<div key={key} className="flex items-center gap-3">
										<Icon
											...
											strokeWidth={1.75}
											className={
												allowed ? 'aip-faint shrink-0' : 'aip-warn-fg shrink-0'
											}
											aria-hidden="true"
										/>
										<span className="text-xs aip-hero min-w-0 truncate">
											{label}
										</span>
													...
													<span
														...
														title={
															rowLocal
																...
																: isKillSwitch
																	? t(
																			'Cloud requests are refused entirely — there is no local model to fall back to.',
																		)
																	: t(
																			'Omitted from context — no local model to fall back to.',
																		)
														}>
														{rowLocal ? (
															<Laptop
														...
														<span className="aip-badge-label">
															{rowLocal
																? t('On-device')
																: isKillSwitch
																	? t('Blocks cloud')
                    ...
                    permanent restatement of the per-row text. */}
						{providerDataScopes.transcript === false &&
							!localFallbackFor('transcript') && (
								<div
									className="aip-inline-warn flex items-start gap-2"
									role="status">
									<AlertCircle
										size={12}
										strokeWidth={1.75}
                            ...
                            plainly here because nothing else in the UI says it. */}
									<span>
										{t(
											'With Transcripts off and no local model selected, cloud requests are refused entirely — answers will fail rather than run without the transcript. Select Ollama under Local & Gateways to keep answering on-device.',
										)}
									</span>
								</div>
							)}
						{disabledScopeCount > 0 &&
							!localFallbackFor('reference_files') &&
							providerDataScopes.transcript !== false && (
								<div
									className="aip-inline-warn flex items-start gap-2"
									role="status">
									...
									<span>
										{t(
											'Disabled types are dropped from context, not handled on-device — select a local model under Local & Gateways to keep them.',
										)}
									</span>
								</div>
							)}
