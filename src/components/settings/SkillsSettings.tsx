...
const BRIDGE_MISSING_MSG =
	'Skills IPC bridge not detected on window.electronAPI — preload may be missing.';
...
export const SkillsSettings: React.FC = () => {
	...
	const runUpload = useCallback(
		async (
			payload: SkillUploadPayload,
			autoInstall: boolean,
		): Promise<UploadSkillOutcome | null> => {
			if (typeof window.electronAPI?.skillsUpload !== 'function') {
				setStatus(BRIDGE_MISSING_MSG);
				return null;
			}
			try {
				const outcome = await window.electronAPI.skillsUpload(payload, {
					autoInstall,
				});
	...
	const handleInstall = async () => {
		...
		try {
			...
			if (outcome?.stage === 'installed') {
				setSuccess(
					`Installed "${outcome.preview.name}" to ${outcome.installedPath}`,
				);
	...
	return (
		<div
			className="space-y-5 animated fadeIn select-text pb-4"
			...
			<div
				...
				<div className="flex items-center justify-between gap-4">
					<div className="min-w-0">
						...
						{uploading && (
							<p className="text-[11px] text-text-tertiary animate-pulse mt-2">
								{t('Uploading…')}
							</p>
						)}
					</div>
					...
					<div className="flex items-center gap-3 flex-wrap text-[11px] text-text-tertiary">
						<span>
							<span className="text-text-secondary font-medium">
								{preview.preview.referenceCount}
							</span>{' '}
							{t('reference')}
						</span>
						<span>
							<span className="text-text-secondary font-medium">
								{preview.preview.assetCount}
							</span>{' '}
							{t('asset')}
						</span>
						<span>
							<span className="text-text-secondary font-medium">
								{preview.preview.scriptCount}
							</span>{' '}
							{t('script')}
						</span>
				...
				<div className="space-y-1.5">
									...
									{skill.source !== 'builtin' &&
										(confirmingId === skill.id ? (
											<div
												...
												<button
													onClick={() =>
														commitDeleteSkill(skill.id, skill.name)
													}
					...
					{!loading && skills.length === 0 && (
						<div className="bg-bg-card rounded-lg border border-dashed border-border-subtle p-5 text-center">
							<FileCode
								size={18}
								className="mx-auto mb-1.5 text-text-tertiary"
							/>
