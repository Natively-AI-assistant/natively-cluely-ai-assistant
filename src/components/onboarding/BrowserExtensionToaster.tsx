...
export const BrowserExtensionToaster: React.FC<Props> = ({ isOpen, onDismiss, onSkip }) => {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      // Close immediately; the user is in the Chrome store now. Not a
      // permanent dismiss, so they can return next launch if they didn't install.
      onDismiss();
    }
  };
  ...
  useEffect(() => {
    if (!visible) { setPlateHover(false); setCtaActive(false); setOpening(false); }
  }, [visible]);
