...
const NativelyInterface: React.FC<NativelyInterfaceProps> = ({
  ...
  const [micLevel, setMicLevel] = useState(0); // 0-100 range
  ...
  // Listen for real-time audio levels (for Voice Tester and Warning Banner)
  useEffect(() => {
    const unsub = window.electronAPI?.onAudioLevel?.((data: { channel: 'mic' | 'system', level: number }) => {
      if (data.channel === 'mic') {
  ...
  // Screen Recording / Mic Permission Warning Banner
  const [audioWarning, setAudioWarning] = useState<{ message: string; channel: 'system' | 'mic' } | null>(null);
  useEffect(() => {
    const unsub = window.electronAPI?.onSystemAudioPermissionDenied?.((message: string) => {
      setAudioWarning({ message, channel: 'system' });
  ...
  useEffect(() => {
    const unsubCaptureFailed = window.electronAPI?.onAudioCaptureFailed?.((payload) => {
      // Allow both system and mic failures if they are terminal/stuck
      if (payload.terminal || payload.stuck) {
        setAudioWarning({ message: payload.message, channel: payload.channel });
        setIsExpanded(true);
      }
    });

    const unsubCaptureSuccess = window.electronAPI?.onAudioCaptureSuccess?.((_payload) => {
      // If we get audio flowing on either channel, clear any capture warning
      // Only clear if the success channel matches the warning channel (or if we want to be permissive)
      setAudioWarning(null);
    });

    // FAIL-SAFE: If we see actual transcript text appearing, the audio IS working.
    // Clear any stuck warnings immediately.
    if (rollingTranscript && rollingTranscript.length > 3) {
      setAudioWarning(null);
    }
  ...
  useEffect(() => {
    setModelNotConfigured(!currentModel);
  }, [currentModel]);
  useEffect(() => {
    let mounted = true;
        ...
        {isExpanded && (
          ...
          >
            ...
            >
              ...
              {/* System Audio / Mic Permission Warning Banner */}
              {audioWarning && (
                <div className="flex items-center justify-between mx-4 mt-3 mb-1 px-3.5 py-2.5 bg-yellow-500/10 border border-yellow-500/20 rounded-[12px] shadow-sm relative no-drag group/warning">
                  <div className="flex flex-col gap-1 pr-3">
                    <div className="flex items-center gap-2 text-[12.5px] text-yellow-600 dark:text-yellow-400/90 font-medium leading-tight">
                      ...
                      <span>
                        {audioWarning.channel === 'mic' 
                          ? 'Microphone Silence Detected' 
                          : (window.electronAPI.platform === 'darwin' ? 'Screen Recording Permission Denied' : 'Audio Capture Permission Required')}
                      </span>
                    </div>
                    {audioWarning.channel === 'mic' && (
                      <div className="pl-[26px] pr-4 py-1">
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-[10px] text-yellow-600/60 dark:text-yellow-400/50">Input Monitor</span>
                          <span className={`text-[10px] font-medium ${micLevel > 1 ? 'text-green-500' : 'text-yellow-600/50'}`}>
                            {micLevel > 1 ? 'Sound detected' : 'No sound'}
                          </span>
                        </div>
                        <div className="h-1 bg-yellow-500/10 rounded-full overflow-hidden border border-yellow-500/10">
                          <motion.div 
              ...
              {/* AI Provider Not Configured Warning Banner */}
              {modelNotConfigured && (
                <div className="flex items-center justify-between mx-4 mt-3 mb-1 px-3.5 py-2.5 bg-blue-500/10 border border-blue-500/20 rounded-[12px] shadow-sm relative no-drag group/model-warning">
                  <div className="flex flex-col gap-1 pr-3">
                    <div className="flex items-center gap-2 text-[12.5px] text-blue-600 dark:text-blue-400/90 font-medium leading-tight">
                      <div className="shrink-0 p-1 bg-blue-500/20 rounded-full">
                        <svg
                    ...
                    <p className="text-[11px] text-blue-600/70 dark:text-blue-400/60 leading-snug pl-[26px]">
                      No AI model selected. Open Settings → AI Provider to pick one.
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-blue-500/15 hover:bg-blue-500/25 text-blue-700 dark:text-blue-500 text-[11px] font-semibold transition-all active:scale-95 border border-blue-500/20 shadow-sm"
                    >
                      Open Settings
                    </button>
                    <button
                      onClick={() => setModelNotConfigured(false)}
                      className="p-1.5 rounded-full hover:bg-black/5 dark:hover:bg-white/10 text-blue-600/50 hover:text-blue-700 dark:text-blue-500/50 dark:hover:text-blue-400 transition-colors absolute top-1 right-1 opacity-0 group-hover/model-warning:opacity-100"
                      title="Dismiss"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  ...
                  >
                    <span className="overlay-text-primary flex-1">
                      {window.electronAPI.platform === 'darwin' 
                        ? 'Stealth typing needs Accessibility access. Grant it in System Settings, then restart Natively.'
                        : 'Stealth typing requires system permissions to capture keystrokes. Grant permissions in Settings, then restart.'}
                    </span>
                    <button
                      onClick={() => {
